...
#ifdef CONFIG_SIM_BTHWSIM
#define VHCI_BTHWSIM_BR_HANDLE_BASE 0x0050
#define VHCI_BTHWSIM_LE_HANDLE_BASE 0x0100

static void vhci_bthwsim_event(struct hci_dev *hdev, u8 event,
			       const u8 *param, size_t param_len)
{
	struct sk_buff *evt_skb;
	u8 *evt;

	evt_skb = bt_skb_alloc(2 + param_len, GFP_KERNEL);
	if (!evt_skb)
		return;

	evt = skb_put(evt_skb, 2 + param_len);
	if (!evt) {
		kfree_skb(evt_skb);
		return;
	}
	...
	hci_skb_pkt_type(evt_skb) = HCI_EVENT_PKT;
	if (hci_get_drvdata(hdev))
		skb_queue_tail(&((struct vhci_data *)hci_get_drvdata(hdev))->eventq,
			       evt_skb);
	else
...
int linux_bt_compat_vhci_pump_deferred_events(struct hci_dev *hdev)
{
	...
	while ((skb = skb_dequeue(&data->eventq)) != NULL) {
...
static void vhci_bthwsim_cmd_status_event(struct hci_dev *hdev, __u16 opcode,
					  u8 status)
{
	u8 ev[4];
	...
	ev[0] = status;
	ev[1] = 1;
	put_unaligned_le16(opcode, &ev[2]);

	vhci_bthwsim_event(hdev, HCI_EV_CMD_STATUS, ev, sizeof(ev));
}

static __u16 vhci_bthwsim_handle(u16 base, const bdaddr_t *bdaddr)
{
	const u8 *addr = (const u8 *)bdaddr;

	return base + addr[0];
}

static u16 vhci_bthwsim_role(void)
{
...
static void vhci_bthwsim_ctrl_connect(u16 peer, u16 handle,
				      const char *transport)
{
	...
	len = snprintf(payload, sizeof(payload),
		       "HCI_CMD_CONNECT src=%u dst=%u transport=%s handle=0x%04x",
		       vhci_bthwsim_role(), peer, transport, handle);
	if (len <= 0)
		return;
	...
	linux_bt_hwsim_send(LINUX_BT_HWSIM_TYPE_CTRL, peer, payload, len);
}

static void vhci_bthwsim_ctrl_disconnect(u16 peer, u16 handle, u8 reason,
					 const char *transport)
{
	char payload[128];
	int len;
	...
	len = snprintf(payload, sizeof(payload),
		       "HCI_CMD_DISCONNECT src=%u dst=%u transport=%s "
		       "handle=0x%04x reason=0x%02x",
		       vhci_bthwsim_role(), peer, transport, handle, reason);
	if (len <= 0)
		return;
...
static void vhci_bthwsim_conn_complete(struct hci_dev *hdev,
				       const struct sk_buff *cmd_skb,
				       __u16 opcode)
{
	if (opcode == HCI_OP_CREATE_CONN) {
		const struct hci_cp_create_conn *cp;
		struct hci_ev_conn_complete ev;

		if (cmd_skb->len < 4 + sizeof(*cp))
			return;

		cp = (const void *)&cmd_skb->data[4];
		memset(&ev, 0, sizeof(ev));
		ev.status = 0;
		ev.handle = cpu_to_le16(vhci_bthwsim_handle(
				VHCI_BTHWSIM_BR_HANDLE_BASE, &cp->bdaddr));
		memcpy(&ev.bdaddr, &cp->bdaddr, sizeof(ev.bdaddr));
		ev.link_type = ACL_LINK;
		ev.encr_mode = 0;

		vhci_bthwsim_ctrl_connect(cp->bdaddr.b[0],
					  le16_to_cpu(ev.handle), "bredr");
		vhci_bthwsim_cmd_status_event(hdev, opcode, 0);
		vhci_bthwsim_event(hdev, HCI_EV_CONN_COMPLETE, (u8 *)&ev,
				   sizeof(ev));
	} else if (opcode == HCI_OP_LE_CREATE_CONN) {
		const struct hci_cp_le_create_conn *cp;
		u8 le[1 + sizeof(struct hci_ev_le_conn_complete)];
		...
		ev->status = 0;
		ev->handle = cpu_to_le16(vhci_bthwsim_handle(
				VHCI_BTHWSIM_LE_HANDLE_BASE, &cp->peer_addr));
		ev->role = 0;
		ev->bdaddr_type = cp->peer_addr_type;
		memcpy(&ev->bdaddr, &cp->peer_addr, sizeof(ev->bdaddr));
		ev->interval = cpu_to_le16(0x0018);
		ev->latency = cpu_to_le16(0x0000);
		ev->supervision_timeout = cpu_to_le16(0x01f4);
		ev->clk_accurancy = 0;

		vhci_bthwsim_ctrl_connect(cp->peer_addr.b[0],
					  le16_to_cpu(ev->handle), "le");
		vhci_bthwsim_event(hdev, HCI_EV_LE_META, le, sizeof(le));
	} else if (opcode == HCI_OP_LE_EXT_CREATE_CONN) {
		const struct hci_cp_le_ext_create_conn *cp;
		u8 le[1 + sizeof(struct hci_ev_le_enh_conn_complete)];
		...
		cp = (const void *)&cmd_skb->data[4];
		memset(le, 0, sizeof(le));
		le[0] = HCI_EV_LE_ENHANCED_CONN_COMPLETE;
		ev = (void *)&le[1];
		ev->status = 0;
		ev->handle = cpu_to_le16(vhci_bthwsim_handle(
				VHCI_BTHWSIM_LE_HANDLE_BASE, &cp->peer_addr));
...
static void vhci_bthwsim_remote_complete(struct hci_dev *hdev,
					 const struct sk_buff *cmd_skb,
					 __u16 opcode)
{
	__le16 handle;
	...
	memcpy(&handle, &cmd_skb->data[4], sizeof(handle));

	if (opcode == HCI_OP_READ_REMOTE_FEATURES) {
		...
		vhci_bthwsim_cmd_status_event(hdev, opcode, 0);
		vhci_bthwsim_event(hdev, HCI_EV_REMOTE_FEATURES, (u8 *)&ev,
				   sizeof(ev));
	} else if (opcode == HCI_OP_READ_REMOTE_VERSION) {
		struct hci_ev_remote_version ev;

		memset(&ev, 0, sizeof(ev));
		ev.status = 0;
		ev.handle = handle;
		ev.lmp_ver = 0x09;
		ev.manufacturer = cpu_to_le16(0xffff);
		ev.lmp_subver = cpu_to_le16(0x0001);

		vhci_bthwsim_cmd_status_event(hdev, opcode, 0);
		vhci_bthwsim_event(hdev, HCI_EV_REMOTE_VERSION, (u8 *)&ev,
				   sizeof(ev));
	} else if (opcode == HCI_OP_LE_READ_REMOTE_FEATURES) {
		u8 le[1 + 1 + 2 + 8];

		memset(le, 0, sizeof(le));
		le[0] = HCI_EV_LE_REMOTE_FEAT_COMPLETE;
		le[1] = 0;
		memcpy(&le[2], &handle, sizeof(handle));
		le[4] = 0x01;
...
static void vhci_bthwsim_disconn_complete(struct hci_dev *hdev,
					  const struct sk_buff *cmd_skb)
{
	...
	vhci_bthwsim_ctrl_disconnect(le16_to_cpu(cp->handle) &
				     (VHCI_BTHWSIM_LE_HANDLE_BASE - 1),
				     le16_to_cpu(cp->handle), cp->reason, "le");
	vhci_bthwsim_cmd_status_event(hdev, HCI_OP_DISCONNECT, 0);
	vhci_bthwsim_event(hdev, HCI_EV_DISCONN_COMPLETE, (u8 *)&ev,
			   sizeof(ev));
}

static size_t vhci_bthwsim_cmd_complete_status(__u16 opcode,
					       const struct sk_buff *cmd_skb,
					       u8 *rp, size_t rp_len)
{
	u8 role;
	...
	case HCI_OP_READ_LOCAL_VERSION:
		if (rp_len < 9)
			return 0;
		rp[1] = 0x09;                  /* Bluetooth 5.0 */
		put_unaligned_le16(0x0001, &rp[2]);
		rp[4] = 0x09;                  /* LMP 5.0 */
		put_unaligned_le16(0xffff, &rp[5]);
		put_unaligned_le16(0x0001, &rp[7]);
		return 9;
	...
	case HCI_OP_READ_LOCAL_NAME:
		if (rp_len < 249)
			return 0;
		memcpy(&rp[1], "NuttX Linux BT hwsim", 20);
		return 249;

	case HCI_OP_READ_VOICE_SETTING:
		if (rp_len < 3)
			return 0;
		put_unaligned_le16(0x0060, &rp[1]);
		return 3;
	...
	case HCI_OP_READ_DEF_ERR_DATA_REPORTING:
		if (rp_len < 2)
			return 0;
		rp[1] = ERR_DATA_REPORTING_DISABLED;
		return 2;
...
static void vhci_bthwsim_cmd_complete(struct hci_dev *hdev,
				       const struct sk_buff *cmd_skb)
{
	u8 rp[249];
	u8 *evt;
	...
	if (!cmd_skb || cmd_skb->len < 4 || cmd_skb->data[0] != HCI_COMMAND_PKT)
		return;

	opcode = get_unaligned_le16(&cmd_skb->data[1]);

	switch (opcode) {
	case HCI_OP_CREATE_CONN:
	case HCI_OP_LE_CREATE_CONN:
	case HCI_OP_LE_EXT_CREATE_CONN:
		vhci_bthwsim_conn_complete(hdev, cmd_skb, opcode);
		return;

	case HCI_OP_DISCONNECT:
		vhci_bthwsim_disconn_complete(hdev, cmd_skb);
		return;

	case HCI_OP_READ_REMOTE_FEATURES:
	case HCI_OP_READ_REMOTE_VERSION:
	case HCI_OP_LE_READ_REMOTE_FEATURES:
		vhci_bthwsim_remote_complete(hdev, cmd_skb, opcode);
		return;
	...
	rp_len = vhci_bthwsim_cmd_complete_status(opcode, cmd_skb, rp,
						  sizeof(rp));
	if (!rp_len)
		return;

	evt_len = 3 + rp_len;
	evt = kmalloc(evt_len, GFP_KERNEL);
	if (!evt)
		return;

	evt[0] = 1;
	put_unaligned_le16(opcode, &evt[1]);
	memcpy(&evt[3], rp, rp_len);

	vhci_bthwsim_event(hdev, HCI_EV_CMD_COMPLETE, evt, evt_len);
	kfree(evt);
}
#endif
...
#ifdef CONFIG_SIM_BTHWSIM
	vhci_bthwsim_cmd_complete(hdev, skb);
#endif
...
#ifdef CONFIG_SIM_BTHWSIM
	skb_queue_head_init(&data->eventq);
#endif
