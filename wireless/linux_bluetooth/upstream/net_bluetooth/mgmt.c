...
static struct mgmt_addr_info hwsim_paired_addr;
static struct hci_dev *hwsim_paired_hdev;
...
#ifdef CONFIG_SIM_BTHWSIM
	if (bacmp(&hwsim_paired_addr.bdaddr, &cp->addr.bdaddr) == 0 &&
	    hwsim_paired_addr.type == cp->addr.type) {
		err = mgmt_cmd_complete(sk, hdev->id, MGMT_OP_UNPAIR_DEVICE,
					MGMT_STATUS_SUCCESS, &rp, sizeof(rp));
		device_unpaired(hdev, &cp->addr.bdaddr, cp->addr.type, NULL);
		if (cp->disconnect) {
		...
		hwsim_paired_hdev = NULL;
		memset(&hwsim_paired_addr, 0, sizeof(hwsim_paired_addr));
		goto unlock;
	}
#endif
		      ...
		      u16 len)
{
	...
	mgmt_device_disconnected(hdev, &conn->dst, conn->type,
				 conn->dst_type, MGMT_DEV_DISCONN_LOCAL_HOST,
				 true);
	hci_conn_del(conn);
	goto failed;
#endif
...
#ifdef CONFIG_SIM_BTHWSIM
static struct mgmt_addr_info hwsim_pair_addr;
static struct sock *hwsim_pair_sk;
static struct hci_dev *hwsim_pair_hdev;
static bool hwsim_pair_pending;
static bool hwsim_pair_authenticated;

static void hwsim_fill_ltk(struct mgmt_ev_new_long_term_key *ltk,
			   const struct mgmt_addr_info *addr, bool authenticated)
{
	const u8 *bdaddr = (const u8 *)&addr->bdaddr;
	unsigned int i;

	memset(ltk, 0, sizeof(*ltk));
	ltk->store_hint = 1;
	bacpy(&ltk->key.addr.bdaddr, &addr->bdaddr);
	ltk->key.addr.type = addr->type;
	ltk->key.type = authenticated ? MGMT_LTK_AUTHENTICATED :
					MGMT_LTK_UNAUTHENTICATED;
	ltk->key.initiator = 1;
	ltk->key.enc_size = 16;
	ltk->key.ediv = cpu_to_le16((u16)(bdaddr[0] | (bdaddr[1] << 8)));
	ltk->key.rand = cpu_to_le64(((u64)bdaddr[5] << 40) |
				     ((u64)bdaddr[4] << 32) |
				     ((u64)bdaddr[3] << 24) |
				     ((u64)bdaddr[2] << 16) |
				     ((u64)bdaddr[1] << 8) | bdaddr[0]);

	for (i = 0; i < sizeof(ltk->key.val); i++)
		ltk->key.val[i] = 0xa5 ^ bdaddr[i % 6] ^ i;
}
...
static int hwsim_pair_connect(struct hci_dev *hdev,
			      const struct mgmt_addr_info *addr)
{
	struct hci_conn *conn;

	if (!hdev || !addr || !bdaddr_type_is_valid(addr->type))
		return -EINVAL;

	conn = hci_conn_hash_lookup_ba(hdev, LE_LINK, &addr->bdaddr);
	if (!conn) {
		conn = hci_conn_add(hdev, LE_LINK, (bdaddr_t *)&addr->bdaddr,
				    HCI_ROLE_MASTER, 0x0040);
		if (IS_ERR(conn))
			return PTR_ERR(conn);
	}

	conn->state = BT_CONNECTED;
	conn->dst_type = addr->type == BDADDR_LE_RANDOM ?
...
static int hwsim_pair_complete(struct sock *sk, struct hci_dev *hdev,
			       u16 reply_opcode, const struct mgmt_addr_info *addr,
			       bool success)
{
	struct mgmt_ev_new_long_term_key ltk;
	struct mgmt_rp_pair_device pair_rp;
	struct mgmt_rp_user_confirm_reply confirm_rp;
	int ret;

	if (!hwsim_pair_pending || hwsim_pair_hdev != hdev ||
	    bacmp(&hwsim_pair_addr.bdaddr, &addr->bdaddr) ||
	    hwsim_pair_addr.type != addr->type)
		return mgmt_cmd_status(sk, hdev->id, reply_opcode,
				       MGMT_STATUS_INVALID_PARAMS);

	memset(&confirm_rp, 0, sizeof(confirm_rp));
	memcpy(&confirm_rp.addr, addr, sizeof(confirm_rp.addr));
	ret = mgmt_cmd_complete(sk, hdev->id, reply_opcode,
				MGMT_STATUS_SUCCESS,
				&confirm_rp, sizeof(confirm_rp));

	memset(&pair_rp, 0, sizeof(pair_rp));
	memcpy(&pair_rp.addr, &hwsim_pair_addr, sizeof(pair_rp.addr));
	if (success) {
		hwsim_fill_ltk(&ltk, &hwsim_pair_addr,
			       hwsim_pair_authenticated);
		mgmt_event(MGMT_EV_NEW_LONG_TERM_KEY, hdev, &ltk,
			   sizeof(ltk), NULL);
		hwsim_pair_connect(hdev, &hwsim_pair_addr);
		memcpy(&hwsim_paired_addr, &hwsim_pair_addr,
		       sizeof(hwsim_paired_addr));
		hwsim_paired_hdev = hdev;
	}

	mgmt_cmd_complete(hwsim_pair_sk, hdev->id, MGMT_OP_PAIR_DEVICE,
			  success ? MGMT_STATUS_SUCCESS : MGMT_STATUS_FAILED,
			  &pair_rp, sizeof(pair_rp));

	hwsim_pair_sk = NULL;
	hwsim_pair_hdev = NULL;
	memset(&hwsim_pair_addr, 0, sizeof(hwsim_pair_addr));
	hwsim_pair_pending = false;
	hwsim_pair_authenticated = false;
	return ret;
}

static int hwsim_pair_cancel(struct sock *sk, struct hci_dev *hdev,
			     const struct mgmt_addr_info *addr)
{
	struct mgmt_rp_pair_device pair_rp;
	int ret;
	...
	if (!hwsim_pair_pending || hwsim_pair_hdev != hdev ||
	    bacmp(&hwsim_pair_addr.bdaddr, &addr->bdaddr) ||
	    hwsim_pair_addr.type != addr->type)
		return mgmt_cmd_status(sk, hdev->id, MGMT_OP_CANCEL_PAIR_DEVICE,
				       MGMT_STATUS_INVALID_PARAMS);

	memset(&pair_rp, 0, sizeof(pair_rp));
	memcpy(&pair_rp.addr, &hwsim_pair_addr, sizeof(pair_rp.addr));
	mgmt_cmd_complete(hwsim_pair_sk, hdev->id, MGMT_OP_PAIR_DEVICE,
			  MGMT_STATUS_CANCELLED, &pair_rp, sizeof(pair_rp));

	ret = mgmt_cmd_complete(sk, hdev->id, MGMT_OP_CANCEL_PAIR_DEVICE,
				MGMT_STATUS_SUCCESS, addr, sizeof(*addr));
...
#ifdef CONFIG_SIM_BTHWSIM
	{
		struct mgmt_ev_user_confirm_request confirm;
		struct mgmt_ev_user_passkey_request passkey;
		struct mgmt_ev_new_long_term_key ltk;
		...
		if (cp->io_cap == SMP_IO_NO_INPUT_OUTPUT) {
			hwsim_fill_ltk(&ltk, &cp->addr, false);
			mgmt_event(MGMT_EV_NEW_LONG_TERM_KEY, hdev, &ltk,
				   sizeof(ltk), NULL);
			hwsim_pair_connect(hdev, &cp->addr);
			memcpy(&hwsim_paired_addr, &cp->addr,
			       sizeof(hwsim_paired_addr));
			hwsim_paired_hdev = hdev;
			return mgmt_cmd_complete(sk, hdev->id,
						 MGMT_OP_PAIR_DEVICE,
		...
		hwsim_pair_sk = sk;
		hwsim_pair_hdev = hdev;
		memcpy(&hwsim_pair_addr, &cp->addr, sizeof(hwsim_pair_addr));
		hwsim_pair_pending = true;
		hwsim_pair_authenticated = true;

		if (cp->io_cap == SMP_IO_KEYBOARD_ONLY) {
			memset(&passkey, 0, sizeof(passkey));
			memcpy(&passkey.addr, &cp->addr, sizeof(passkey.addr));
			return mgmt_event(MGMT_EV_USER_PASSKEY_REQUEST, hdev,
					  &passkey, sizeof(passkey), NULL);
		}

		memset(&confirm, 0, sizeof(confirm));
		memcpy(&confirm.addr, &cp->addr, sizeof(confirm.addr));
		confirm.value = cpu_to_le32(123456);
		return mgmt_event(MGMT_EV_USER_CONFIRM_REQUEST, hdev,
				  &confirm, sizeof(confirm), NULL);
	}
#endif
...
#ifdef CONFIG_SIM_BTHWSIM
	if (hwsim_pair_pending && hwsim_pair_hdev == hdev) {
		err = hwsim_pair_cancel(sk, hdev, addr);
		goto unlock;
	}
#endif
...
#ifdef CONFIG_SIM_BTHWSIM
	return hwsim_pair_complete(sk, hdev, MGMT_OP_USER_CONFIRM_REPLY,
				   &cp->addr, true);
#endif
...
#ifdef CONFIG_SIM_BTHWSIM
	return hwsim_pair_complete(sk, hdev, MGMT_OP_USER_CONFIRM_NEG_REPLY,
				   &cp->addr, false);
#endif
...
#ifdef CONFIG_SIM_BTHWSIM
	return hwsim_pair_complete(sk, hdev, MGMT_OP_USER_PASSKEY_REPLY,
				   &cp->addr, true);
#endif
...
#ifdef CONFIG_SIM_BTHWSIM
	return hwsim_pair_complete(sk, hdev, MGMT_OP_USER_PASSKEY_NEG_REPLY,
				   &cp->addr, false);
#endif
