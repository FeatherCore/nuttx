...
static u8 iso_sim_link_type(__u16 handle)
{
...
int iso_sim_attach_connected(struct socket *sock, uint16_t handle)
{
	...
	if (!hcon) {
		hcon = hci_conn_add(hdev, iso_sim_link_type(handle), BDADDR_ANY,
	...
	hcon->mtu = hcon->mtu ? hcon->mtu : ISO_DEFAULT_MTU;
	hcon->iso_qos.ucast.out.sdu = hcon->mtu;
	hcon->iso_qos.ucast.in.sdu = hcon->mtu;
	hcon->iso_qos.ucast.out.phy = 0x01;
	hcon->iso_qos.ucast.in.phy = 0x01;
	hcon->iso_qos.bcast.big = (handle >> 4) & 0x0f;
	hcon->iso_qos.bcast.bis = handle & 0x0f;
