...
int l2cap_compat_clear_bound_psm(bdaddr_t *src, u8 src_type, __le16 psm)
{
	struct l2cap_chan *c;
	int cleared = 0;
...
EXPORT_SYMBOL_GPL(l2cap_compat_clear_bound_psm);
...
static struct l2cap_conn *l2cap_conn_add(struct hci_conn *hcon)
{
    ...
    defined(CONFIG_NET_LINUX_BLUETOOTH_UPSTREAM_L2CAP)
int l2cap_sim_attach_channel(struct hci_dev *hdev, u16 handle,
			     struct l2cap_chan *chan, u16 cid)
{
...
int l2cap_sim_detach_channel(struct l2cap_chan *chan)
{
	if (!chan)
		return -EINVAL;
...
int l2cap_sim_detach_cid(struct hci_dev *hdev, u16 handle, u16 cid)
{
	struct hci_conn *hcon;
	struct l2cap_conn *conn;
	struct l2cap_chan *chan;
	struct l2cap_chan *tmp;
	int detached = 0;
	...
	mutex_lock(&conn->lock);
	list_for_each_entry_safe(chan, tmp, &conn->chan_l, list) {
		if (chan->scid == cid || chan->dcid == cid) {
			l2cap_sim_detach_channel(chan);
			detached++;
		}
	}
	mutex_unlock(&conn->lock);
