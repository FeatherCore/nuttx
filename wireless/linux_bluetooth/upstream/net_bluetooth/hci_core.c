...
static bool hci_compat_deferred_event_pumping;
static bool hci_compat_deferred_cmd_q_ready;
static struct sk_buff_head hci_compat_deferred_cmd_q;

static void hci_compat_deferred_cmd_q_init(void)
{
	if (!hci_compat_deferred_cmd_q_ready) {
		skb_queue_head_init(&hci_compat_deferred_cmd_q);
		hci_compat_deferred_cmd_q_ready = true;
	}
}
#endif
...
#ifdef CONFIG_SIM_BTHWSIM
	if (hci_compat_deferred_event_pumping) {
		hci_compat_deferred_cmd_q_init();
		skb_queue_tail(&hci_compat_deferred_cmd_q, skb);
		return 0;
	}
#endif
...
#ifdef CONFIG_SIM_BTHWSIM
	while ((tx_skb = skb_dequeue(&chan->data_q)) != NULL)
		hci_send_conn_frame(hdev, chan->conn, tx_skb);
	return;
#endif
...
#ifdef CONFIG_SIM_BTHWSIM
		hci_compat_deferred_event_pumping = true;
		linux_bt_compat_vhci_pump_deferred_events(hdev);
		hci_compat_deferred_event_pumping = false;
		hci_compat_deferred_cmd_q_init();
		if (!skb_queue_empty(&hci_compat_deferred_cmd_q))
			skb_queue_splice_tail_init(&hci_compat_deferred_cmd_q,
						   &hdev->cmd_q);
