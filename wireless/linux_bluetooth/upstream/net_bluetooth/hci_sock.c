...
static u32 feather_policy_device_flags;
static bool feather_discovery_active;
static u8 feather_discovery_type;

static int hci_mgmt_feather_queue_event(struct sock *sk, u16 index,
					 u16 event, const void *data,
					 u16 data_len)
{
	struct sk_buff *skb;
	struct mgmt_hdr *hdr;
...
static void hci_mgmt_feather_addr_from_role(u16 role, u8 *addr)
{
	memset(addr, 0, 6);
	addr[0] = role & 0xff;
	addr[1] = role >> 8;
...
static int hci_mgmt_feather_queue_device_found(struct sock *sk, u16 index,
					       u16 src, const char *name)
{
	u8 payload[7 + 1 + 4 + 2 + 31];
	u8 eir[31];
	size_t name_len;
	size_t eir_len;
	size_t pos = 0;
	...
	hci_mgmt_feather_addr_from_role(src, &payload[pos]);
	pos += 6;
	payload[pos++] = BDADDR_LE_PUBLIC;
	payload[pos++] = (u8)-42;
	put_unaligned_le32(0, &payload[pos]);
	pos += 4;
	put_unaligned_le16(eir_len, &payload[pos]);
	pos += 2;
	memcpy(&payload[pos], eir, eir_len);
	pos += eir_len;

	return hci_mgmt_feather_queue_event(sk, index, MGMT_EV_DEVICE_FOUND,
					    payload, pos);
}

static int hci_mgmt_feather_poll_adv(struct sock *sk, u16 index)
{
	u8 payload[160];
	char text[161];
	char name[32];
	...
	while (records < 8) {
		memset(payload, 0, sizeof(payload));
		payload_len = 0;
		ret = linux_bt_hwsim_read_raw(LINUX_BT_HWSIM_TYPE_ADV, &src,
		...
		text[payload_len] = '\0';
		if (!strstr(text, "HCI_LE_ADV_REPORT"))
			continue;
		...
		if (hci_mgmt_feather_queue_device_found(sk, index, src,
							name) == 0)
			found++;
	}
...
static int hci_mgmt_feather_discovery_cmd(struct sock *sk, struct sk_buff *skb)
{
	struct mgmt_hdr *hdr = (void *)skb->data;
	u16 opcode = __le16_to_cpu(hdr->opcode);
	u16 index = __le16_to_cpu(hdr->index);
	u16 len = __le16_to_cpu(hdr->len);
	u8 *data = skb->data + sizeof(*hdr);
	u8 ev[2];
	u8 type;
	int err;
	int poll_err;

	if (skb->len < sizeof(*hdr) || len != skb->len - sizeof(*hdr) ||
	    len != 1)
		return mgmt_cmd_complete(sk, index, opcode,
	...
	if (opcode == MGMT_OP_START_DISCOVERY) {
		feather_discovery_active = true;
		feather_discovery_type = type;
		ev[0] = feather_discovery_type;
		ev[1] = 1;
		hci_mgmt_feather_queue_event(sk, index, MGMT_EV_DISCOVERING,
					     ev, sizeof(ev));
		err = mgmt_cmd_complete(sk, index, opcode,
					MGMT_STATUS_SUCCESS, data, len);
		poll_err = hci_mgmt_feather_poll_adv(sk, index);
		(void)poll_err;
		return err;
	}

	feather_discovery_active = false;
	ev[0] = feather_discovery_type ? feather_discovery_type : type;
	ev[1] = 0;
	hci_mgmt_feather_queue_event(sk, index, MGMT_EV_DISCOVERING,
				     ev, sizeof(ev));
	return mgmt_cmd_complete(sk, index, opcode, MGMT_STATUS_SUCCESS,
				 data, len);
}

static int hci_mgmt_feather_settings_cmd(struct sock *sk, struct sk_buff *skb)
{
	struct mgmt_hdr *hdr = (void *)skb->data;
	u16 opcode = __le16_to_cpu(hdr->opcode);
	u16 index = __le16_to_cpu(hdr->index);
	...
	case MGMT_OP_SET_POWERED:
		err = linux_bt_mgmt_set_powered(val);
		break;
	case MGMT_OP_SET_CONNECTABLE:
		err = linux_bt_mgmt_set_connectable(val);
		break;
	case MGMT_OP_SET_DISCOVERABLE:
		err = linux_bt_mgmt_set_discoverable(val);
		break;
	case MGMT_OP_SET_BONDABLE:
		err = linux_bt_mgmt_set_bondable(val);
		break;
	case MGMT_OP_SET_LE:
		err = linux_bt_mgmt_set_le(val);
		break;
	case MGMT_OP_SET_ADVERTISING:
		err = linux_bt_mgmt_set_advertising(val);
		break;
	case MGMT_OP_SET_BREDR:
		err = linux_bt_mgmt_set_bredr(val);
		break;
	default:
		return -EOPNOTSUPP;
	}
	...
	err = mgmt_cmd_complete(sk, index, opcode, status, &settings,
				sizeof(settings));
	if (err >= 0 && status == MGMT_STATUS_SUCCESS)
		hci_mgmt_feather_queue_event(sk, index, MGMT_EV_NEW_SETTINGS,
					     &settings, sizeof(settings));

	return err;
}

static int hci_mgmt_feather_policy_cmd(struct sock *sk, struct sk_buff *skb)
{
	struct mgmt_hdr *hdr = (void *)skb->data;
	u16 opcode = __le16_to_cpu(hdr->opcode);
	u16 index = __le16_to_cpu(hdr->index);
	u16 len = __le16_to_cpu(hdr->len);
	void *data = skb->data + sizeof(*hdr);

	if (skb->len < sizeof(*hdr) || len != skb->len - sizeof(*hdr))
		return mgmt_cmd_status(sk, index, opcode,
				       MGMT_STATUS_INVALID_PARAMS);

	switch (opcode) {
	case MGMT_OP_ADD_DEVICE:
		if (len != sizeof(struct mgmt_cp_add_device))
			return mgmt_cmd_complete(sk, index, opcode,
						 MGMT_STATUS_INVALID_PARAMS,
						 NULL, 0);
		...
		return mgmt_cmd_complete(sk, index, opcode,
					 MGMT_STATUS_SUCCESS,
					 &((struct mgmt_cp_add_device *)data)->addr,
					 sizeof(struct mgmt_addr_info));

	case MGMT_OP_REMOVE_DEVICE:
		if (len != sizeof(struct mgmt_cp_remove_device))
			return mgmt_cmd_complete(sk, index, opcode,
						 MGMT_STATUS_INVALID_PARAMS,
						 NULL, 0);

		feather_policy_device_flags = 0;
		return mgmt_cmd_complete(sk, index, opcode,
					 MGMT_STATUS_SUCCESS,
					 &((struct mgmt_cp_remove_device *)data)->addr,
					 sizeof(struct mgmt_addr_info));

	case MGMT_OP_BLOCK_DEVICE:
	case MGMT_OP_UNBLOCK_DEVICE:
		if (len != sizeof(struct mgmt_cp_block_device))
			return mgmt_cmd_complete(sk, index, opcode,
						 MGMT_STATUS_INVALID_PARAMS,
						 NULL, 0);
		...
		return mgmt_cmd_complete(sk, index, opcode,
					 MGMT_STATUS_SUCCESS,
					 &((struct mgmt_cp_block_device *)data)->addr,
					 sizeof(struct mgmt_addr_info));
	...
	case MGMT_OP_GET_DEVICE_FLAGS:
		{
			...
			rp.supported_flags = cpu_to_le32(0x00000001);
			rp.current_flags = cpu_to_le32(feather_policy_device_flags);
			return mgmt_cmd_complete(sk, index, opcode,
	...
	case MGMT_OP_SET_DEVICE_FLAGS:
		{
			...
			feather_policy_device_flags =
				__le32_to_cpu(cp->current_flags) & 0x00000001;
			return mgmt_cmd_complete(sk, index, opcode,
						 MGMT_STATUS_SUCCESS,
						 &cp->addr,
						 sizeof(struct mgmt_addr_info));
	...
	case MGMT_OP_UNPAIR_DEVICE:
		{
			...
			hdev = index == MGMT_INDEX_NONE ? NULL : hci_dev_get(index);
			err = mgmt_send_event(MGMT_EV_DEVICE_UNPAIRED, hdev,
					      HCI_CHANNEL_CONTROL, &ev,
					      sizeof(ev), HCI_SOCK_TRUSTED,
					      NULL);
			if (hdev)
				hci_dev_put(hdev);
			    ...
			    size_t len, int flags)
{
	...
	if (hci_pi(sk)->channel == HCI_CHANNEL_CONTROL && feather_discovery_active)
		hci_mgmt_feather_poll_adv(sk, 0);
			    ...
			    size_t len)
{
	...
	default:
		if (hci_pi(sk)->channel == HCI_CHANNEL_CONTROL) {
			...
			case MGMT_OP_SET_ADVERTISING:
			case MGMT_OP_SET_BREDR:
				err = hci_mgmt_feather_settings_cmd(sk, skb);
				goto drop;
			case MGMT_OP_START_DISCOVERY:
			case MGMT_OP_STOP_DISCOVERY:
				err = hci_mgmt_feather_discovery_cmd(sk, skb);
				goto drop;
			case MGMT_OP_UNPAIR_DEVICE:
			case MGMT_OP_BLOCK_DEVICE:
			case MGMT_OP_UNBLOCK_DEVICE:
			...
			case MGMT_OP_GET_DEVICE_FLAGS:
			case MGMT_OP_SET_DEVICE_FLAGS:
				err = hci_mgmt_feather_policy_cmd(sk, skb);
				goto drop;
			default:
				break;
			}
