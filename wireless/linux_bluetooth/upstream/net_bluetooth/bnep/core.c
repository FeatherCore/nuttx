...
#define register_netdev(dev) linux_bt_bnep_netdev_register(dev)
#define unregister_netdev(dev) linux_bt_bnep_netdev_unregister(dev)
#endif
...
static int bnep_rx_frame(struct bnep_session *s, struct sk_buff *skb)
{
	...
	netif_rx(nskb);
	linux_bt_upstream_bnep_note_native_rx_frame(orig_len, 0);
	return 0;

badframe:
	dev->stats.rx_errors++;
	kfree_skb(skb);
	linux_bt_upstream_bnep_note_native_rx_frame(orig_len, -EILSEQ);
	return 0;
}

int bnep_recv_l2cap_payload(__u16 cid, const void *payload,
			    size_t payload_len)
{
	struct bnep_session *s;
	...
	list_for_each_entry(s, &bnep_session_list, list) {
		...
		ret = bnep_rx_frame(s, skb);
		if (ret == 0)
			delivered++;
	}
	up_read(&bnep_session_sem);

	linux_bt_upstream_bnep_note_native_l2cap_rx(payload_len, delivered);
	return delivered;
}
...
send:
	...
	if (len > 0) {
		...
		linux_bt_upstream_bnep_note_native_tx_frame(bnep_len, 0);
		return 0;
	}
...
static int bnep_session(void *arg)
{
	...
	while (1) {
		...
		while ((skb = skb_dequeue(&sk->sk_receive_queue))) {
			...
			linux_bt_upstream_bnep_note_native_l2cap_rx(payload_len,
								    delivered);
		}
	...
	linux_bt_upstream_bnep_note_native_session_stop();
	free_netdev(dev);
...
static struct device *bnep_get_device(struct bnep_session *session)
{
	...
	if (!conn || !conn->hcon || !conn->hcon->hdev)
		return NULL;

	return &conn->hcon->hdev->dev;
}
...
int bnep_del_connection(struct bnep_conndel_req *req)
{
	...
	if (s) {
		linux_bt_upstream_bnep_note_native_session_terminate();
