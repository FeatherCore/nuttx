...
#ifdef CONFIG_SIM_BTHWSIM
	if (ev->link_type == ACL_LINK && !status) {
		int sim_ret;
		u8 role;

		conn = hci_conn_hash_lookup_ba(hdev, ev->link_type,
					       &ev->bdaddr);
		role = conn ? conn->role : HCI_ROLE_SLAVE;
		sim_ret = linux_bt_upstream_hci_sim_br_complete(
			hdev, &ev->bdaddr, role,
			__le16_to_cpu(ev->handle));
		bt_dev_dbg(hdev,
			   "sim peer BR/EDR complete bdaddr %pMR handle 0x%4.4x ret %d",
			   &ev->bdaddr, __le16_to_cpu(ev->handle), sim_ret);
		goto unlock;
	}
#endif
...
#ifdef CONFIG_SIM_BTHWSIM
	if (conn->type == LE_LINK && !ev->status) {
		int sim_ret;

		sim_ret = linux_bt_upstream_hci_sim_le_disconnect(
			hdev, __le16_to_cpu(ev->handle), ev->reason);
		bt_dev_dbg(hdev,
			   "sim LE disconnect complete handle 0x%4.4x reason 0x%2.2x ret %d",
			   __le16_to_cpu(ev->handle), ev->reason, sim_ret);
		goto unlock;
	}
#endif
...
#ifdef CONFIG_SIM_BTHWSIM
	if (!hdev->req_skb && !ev->status) {
		int sim_ret;

		sim_ret = linux_bt_upstream_hci_sim_le_complete(
			hdev, &ev->bdaddr, ev->bdaddr_type, ev->role,
			le16_to_cpu(ev->handle));
		bt_dev_dbg(hdev,
			   "sim peer LE complete bdaddr %pMR handle 0x%4.4x ret %d",
			   &ev->bdaddr, le16_to_cpu(ev->handle), sim_ret);
		return;
	}
#endif
...
#ifdef CONFIG_SIM_BTHWSIM
	if (!req_matched && ev->subevent == HCI_EV_LE_CONN_COMPLETE &&
	    hci_sent_cmd_data(hdev, HCI_OP_LE_CREATE_CONN)) {
		*opcode = HCI_OP_LE_CREATE_CONN;
		hci_req_cmd_complete(hdev, *opcode, 0x00, req_complete,
				     req_complete_skb);
		req_matched = true;
	} else if (!req_matched &&
		   ev->subevent == HCI_EV_LE_ENHANCED_CONN_COMPLETE &&
		   hci_sent_cmd_data(hdev, HCI_OP_LE_EXT_CREATE_CONN)) {
		*opcode = HCI_OP_LE_EXT_CREATE_CONN;
		hci_req_cmd_complete(hdev, *opcode, 0x00, req_complete,
				     req_complete_skb);
		req_matched = true;
	}
