...
static bool hwsim_debug_is_assoc_skb(struct sk_buff *skb)
{
	...
	return ieee80211_is_assoc_req(hdr->frame_control) ||
			       ...
			       struct ieee80211_txq *queue)
{
	...
	while (1) {
		...
		if (hwsim_debug_is_assoc_skb(skb)) {
			nuttx_hwsim_debugf("hwsim-debug: wake_tx dequeue assoc sdata=%s queue=%p sta=%p len=%u flags=0x%x hwq=%d\n",
			       sdata->name, queue, queue->sta, skb->len,
