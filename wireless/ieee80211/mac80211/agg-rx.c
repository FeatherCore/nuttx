				      ...
				      const u8 req_addba_ext_data)
{
	...
	nuttx_hwsim_ampdu_prooff("ampdu-proof: send ADDBA_RESP sta="
				 NUTTX_HWSIM_MAC_FMT " "
				 "tid=%u token=%u status=%u policy=%u buf=%u "
				 "timeout=%u amsdu=%d\n",
				 NUTTX_HWSIM_MAC_ARG(da), tid, dialog_token, status, policy,
				 buf_size, timeout, amsdu ? 1 : 0);
				     ...
				     const u8 addba_ext_data)
{
	...
	lockdep_assert_wiphy(sta->local->hw.wiphy);
	nuttx_hwsim_ampdu_prooff("ampdu-proof: start_rx_ba_session "
				 "sta=" NUTTX_HWSIM_MAC_FMT
				 " tid=%u token=%u ssn=%u policy=%u "
				 "buf=%u timeout=%u tx=%d\n",
				 NUTTX_HWSIM_MAC_ARG(sta->sta.addr), tid, dialog_token,
				 start_seq_num, ba_policy, buf_size, timeout,
				 tx ? 1 : 0);
	...
	if (((ba_policy != 1) &&
		...
		ht_dbg_ratelimited(sta->sdata,
			...
			nuttx_hwsim_ampdu_prooff("ampdu-proof: start_rx_ba_session "
						 "reject bad-param sta="
						 NUTTX_HWSIM_MAC_FMT " tid=%u "
						 "policy=%u buf=%u max=%u\n",
						 NUTTX_HWSIM_MAC_ARG(sta->sta.addr), tid, ba_policy,
						 buf_size, max_buf_size);
		goto end;
	}
	...
	if (ieee80211_hw_check(&local->hw, SUPPORTS_REORDERING_BUFFER)) {
		...
		nuttx_hwsim_ampdu_prooff("ampdu-proof: rx ampdu_action "
					 "sta=" NUTTX_HWSIM_MAC_FMT
					 " tid=%u result=%d buf=%u "
					 "reorder_hw=1\n",
					 NUTTX_HWSIM_MAC_ARG(sta->sta.addr), tid, ret,
					 params.buf_size);
		if (!ret)
			status = WLAN_STATUS_SUCCESS;
		goto end;
	...
	ht_dbg(sta->sdata, "Rx A-MPDU request on %pM tid %d result %d\n",
	       sta->sta.addr, tid, ret);
	nuttx_hwsim_ampdu_prooff("ampdu-proof: rx ampdu_action sta="
				 NUTTX_HWSIM_MAC_FMT " "
				 "tid=%u result=%d buf=%u reorder_hw=0\n",
				 NUTTX_HWSIM_MAC_ARG(sta->sta.addr), tid, ret,
				 params.buf_size);
	if (ret) {
		kfree(tid_agg_rx->reorder_buf);
	...
	nuttx_hwsim_ampdu_prooff("ampdu-proof: start_rx_ba_session done "
				 "sta=" NUTTX_HWSIM_MAC_FMT
				 " tid=%u status=%u buf=%u\n",
				 NUTTX_HWSIM_MAC_ARG(sta->sta.addr), tid,
				 status, buf_size);
				     ...
				     size_t len)
{
	...
	nuttx_hwsim_ampdu_prooff("ampdu-proof: rx ADDBA_REQ sta="
				 NUTTX_HWSIM_MAC_FMT " "
				 "tid=%u token=%u ssn=%u policy=%u buf=%u "
				 "timeout=%u\n",
				 NUTTX_HWSIM_MAC_ARG(sta->sta.addr), tid, dialog_token,
				 start_seq_num, ba_policy, buf_size, timeout);
