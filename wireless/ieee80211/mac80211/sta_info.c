...
static void __sta_info_recalc_tim(struct sta_info *sta, bool ignore_pending)
{
	...
	nuttx_hwsim_ps_prooff("ps-proof: tim sta=" NUTTX_HWSIM_MAC_FMT
			      " aid=%u set=%d total=%d\n",
			      NUTTX_HWSIM_MAC_ARG(sta->sta.addr), id,
			      indicate_tim ? 1 : 0, local->total_ps_buffered);
...
void ieee80211_sta_ps_deliver_wakeup(struct sta_info *sta)
{
	...
	nuttx_hwsim_ps_prooff("ps-proof: wake deliver sta="
			      NUTTX_HWSIM_MAC_FMT " aid=%d filtered=%d "
			      "buffered=%d total=%d num_sta_ps=%d\n",
			      NUTTX_HWSIM_MAC_ARG(sta->sta.addr),
			      sta->sta.aid, filtered, buffered,
			      local->total_ps_buffered,
			      atomic_read(&ps->num_sta_ps));
				  ...
				  enum ieee80211_frame_release_type reason)
{
	...
	released_frames = skb_queue_len(&frames);
	if (released_frames || driver_release_tids || more_data) {
		nuttx_hwsim_ps_prooff("ps-proof: deliver response sta="
				      NUTTX_HWSIM_MAC_FMT " aid=%d reason=%d "
				      "requested=%d frames=%u driver_tids=0x%lx "
				      "more=%d\n",
				      NUTTX_HWSIM_MAC_ARG(sta->sta.addr),
				      sta->sta.aid, reason, n_frames,
				      released_frames, driver_release_tids,
				      more_data ? 1 : 0);
	} else {
		static unsigned int empty_deliver_logs;

		if (empty_deliver_logs < 8 || empty_deliver_logs % 256 == 0) {
			nuttx_hwsim_ps_prooff("ps-proof: deliver response sta="
					      NUTTX_HWSIM_MAC_FMT " aid=%d reason=%d "
					      "requested=%d frames=0 driver_tids=0x0 "
					      "more=0 sample=%u\n",
					      NUTTX_HWSIM_MAC_ARG(sta->sta.addr),
					      sta->sta.aid, reason, n_frames,
					      empty_deliver_logs);
	...
	if (skb_queue_empty(&frames) && !driver_release_tids) {
		...
		drv_allow_buffered_frames(local, sta, tids, num,
					  reason, more_data);
		nuttx_hwsim_ps_prooff("ps-proof: allow buffered sta="
				      NUTTX_HWSIM_MAC_FMT " aid=%d tids=0x%x "
				      "num=%d more=%d\n",
				      NUTTX_HWSIM_MAC_ARG(sta->sta.addr),
				      sta->sta.aid, tids, num,
				      more_data ? 1 : 0);
...
void ieee80211_sta_pspoll_n(struct ieee80211_sta *pubsta, int n_frames)
{
	struct sta_info *sta = container_of(pubsta, struct sta_info, sta);
	static unsigned int bulk_logs;
	u8 ignore_for_response;

	if (n_frames <= 1) {
		ieee80211_sta_pspoll(pubsta);
		return;
	}
	...
	if (bulk_logs < 8 || bulk_logs % 128 == 0) {
		nuttx_hwsim_ps_prooff("ps-proof: pspoll bulk rx sta=" NUTTX_HWSIM_MAC_FMT
				      " aid=%d frames=%d driver_ps=%d sample=%u\n",
				      NUTTX_HWSIM_MAC_ARG(sta->sta.addr),
				      sta->sta.aid, n_frames,
	...
	ignore_for_response = sta->sta.uapsd_queues;
	if (ignore_for_response == BIT(IEEE80211_NUM_ACS) - 1)
		ignore_for_response = 0;

	ieee80211_sta_ps_deliver_response(sta, n_frames, ignore_for_response,
					  IEEE80211_FRAME_RELEASE_PSPOLL);
}
EXPORT_SYMBOL(ieee80211_sta_pspoll_n);
