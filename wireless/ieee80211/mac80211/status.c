				 ...
				 struct sk_buff *skb)
{
...
#ifdef CONFIG_WL_NUTTX_HWSIM
	{
		...
		if (skb->len >= 24 && ieee80211_is_mgmt(hdr->frame_control))
			nuttx_hwsim_debugf("hwsim-debug: mac80211 tx_status_irqsafe fc=0x%04x len=%u flags=0x%x status_idr=%u status_data=%u ack=%d\n",
			       le16_to_cpu(hdr->frame_control), skb->len,
			       info->flags, info->status_data_idr,
			       info->status_data,
				     ...
				     ktime_t ack_hwtstamp)
{
	...
	if (!skb) {
#ifdef CONFIG_WL_NUTTX_HWSIM
		nuttx_hwsim_debugf("hwsim-debug: mac80211 report_ack missing ack_skb status_data=%u acked=%d dropped=%d\n",
		       info->status_data, acked, dropped);
#endif
		return;
	}
...
#ifdef CONFIG_WL_NUTTX_HWSIM
		nuttx_hwsim_debugf("hwsim-debug: mac80211 report_ack fc=0x%04x len=%u cookie=%llu acked=%d sdata=%s flags=0x%x\n",
		       le16_to_cpu(hdr->frame_control), skb->len,
		       (unsigned long long)cookie, acked,
		       sdata ? sdata->name : "(null)", info->flags);
#endif
		if (sdata) {
