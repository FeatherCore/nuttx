					 ...
					 u16 agg_size, u16 timeout)
{
	...
	nuttx_hwsim_ampdu_prooff("ampdu-proof: send ADDBA_REQ sta="
				 NUTTX_HWSIM_MAC_FMT " "
				 "tid=%u token=%u ssn=%u buf=%u timeout=%u\n",
				 NUTTX_HWSIM_MAC_ARG(sta->sta.addr), tid, dialog_token,
				 start_seq_num, agg_size, timeout);
				  ...
				  u16 timeout)
{
	...
	ht_dbg(sdata, "Open BA session requested for %pM tid %u\n",
	       pubsta->addr, tid);
	nuttx_hwsim_ampdu_prooff("ampdu-proof: start_tx_ba_session "
				 "request sta=" NUTTX_HWSIM_MAC_FMT
				 " tid=%u timeout=%u vif_type=%d\n",
				 NUTTX_HWSIM_MAC_ARG(pubsta->addr), tid, timeout,
				 sdata->vif.type);
	...
	if (sdata->vif.type != NL80211_IFTYPE_STATION &&
		...
		nuttx_hwsim_ampdu_prooff("ampdu-proof: start_tx_ba_session "
					 "reject vif_type=%d sta="
					 NUTTX_HWSIM_MAC_FMT " tid=%u\n",
					 sdata->vif.type,
					 NUTTX_HWSIM_MAC_ARG(pubsta->addr), tid);
		return -EINVAL;
	}
	...
	if (test_sta_flag(sta, WLAN_STA_BLOCK_BA)) {
		...
		nuttx_hwsim_ampdu_prooff("ampdu-proof: start_tx_ba_session "
					 "reject blocked sta="
					 NUTTX_HWSIM_MAC_FMT " tid=%u\n",
					 NUTTX_HWSIM_MAC_ARG(sta->sta.addr), tid);
		return -EINVAL;
	}
	...
	if (sta->ampdu_mlme.addba_req_num[tid] > HT_AGG_MAX_RETRIES) {
		...
		nuttx_hwsim_ampdu_prooff("ampdu-proof: start_tx_ba_session "
					 "reject retries sta="
					 NUTTX_HWSIM_MAC_FMT " tid=%u tries=%u\n",
					 NUTTX_HWSIM_MAC_ARG(sta->sta.addr), tid,
					 sta->ampdu_mlme.addba_req_num[tid]);
		goto err_unlock_sta;
	}
	...
	if (tid_tx || sta->ampdu_mlme.tid_start_tx[tid]) {
		...
		ret = -EAGAIN;
		nuttx_hwsim_ampdu_prooff("ampdu-proof: start_tx_ba_session "
					 "reject busy sta=" NUTTX_HWSIM_MAC_FMT
					 " tid=%u tid_tx=%p "
					 "start_tx=%p\n",
					 NUTTX_HWSIM_MAC_ARG(sta->sta.addr), tid, tid_tx,
					 sta->ampdu_mlme.tid_start_tx[tid]);
		goto err_unlock_sta;
	}
	...
	nuttx_hwsim_ampdu_prooff("ampdu-proof: start_tx_ba_session queued "
					 ...
					 struct sta_info *sta, u16 tid)
{
	...
	ht_dbg(sta->sdata, "Aggregation is on for %pM tid %d\n",
	       sta->sta.addr, tid);
	nuttx_hwsim_ampdu_prooff("ampdu-proof: tx operational sta="
				 NUTTX_HWSIM_MAC_FMT " "
				 "tid=%u buf=%u amsdu=%d\n",
				 NUTTX_HWSIM_MAC_ARG(sta->sta.addr), tid, params.buf_size,
				 params.amsdu);
				  ...
				  size_t len)
{
	...
	nuttx_hwsim_ampdu_prooff("ampdu-proof: rx ADDBA_RESP sta="
				 NUTTX_HWSIM_MAC_FMT " "
				 "tid=%u token=%u status=%u raw_buf=%u "
				 "amsdu=%d timeout=%u\n",
				 NUTTX_HWSIM_MAC_ARG(sta->sta.addr), tid,
