	...
	while ((skb = skb_dequeue(&local->skb_queue)) ||
		...
		if (skb->len >= 24) {
			...
			if (ieee80211_is_auth(hdr->frame_control))
				nuttx_hwsim_debugf("hwsim-debug: tasklet dequeued auth len=%u pkt_type=%u local_started=%d\n",
						   skb->len, skb->pkt_type,
						   local->started);
		}
...
int ieee80211_register_hw(struct ieee80211_hw *hw)
{
	...
	if (!local->ops->sched_scan_start) {
		local->hw.wiphy->max_sched_scan_reqs = 0;
		local->hw.wiphy->max_sched_scan_ssids = 0;
		local->hw.wiphy->max_sched_scan_ie_len = 0;
		local->hw.wiphy->max_sched_scan_plans = 0;
		local->hw.wiphy->max_sched_scan_plan_interval = 0;
		local->hw.wiphy->max_sched_scan_plan_iterations = 0;
	}
