			   ...
			   struct ieee80211_mgd_assoc_data *assoc_data)
{
	...
	nuttx_hwsim_debugf("hwsim-debug: fils-req encrypt len=%u capab_off=%td ies_off=%td session_off=%td encr_off=%td aad4=%zu crypt=%zu kek=%zu snonce=%02x%02x anonce=%02x%02x\n",
			   skb->len, capab - skb->data, ies - skb->data,
			   (u8 *)session - skb->data, encr - skb->data,
			   len[4], crypt_len, assoc_data->fils_kek_len,
			   assoc_data->fils_nonces[0],
			   assoc_data->fils_nonces[1],
			   assoc_data->fils_nonces[FILS_NONCE_LEN],
			   assoc_data->fils_nonces[FILS_NONCE_LEN + 1]);
			    ...
			    struct ieee80211_mgd_assoc_data *assoc_data)
{
	...
	nuttx_hwsim_debugf("hwsim-debug: fils-resp decrypt len=%zu capab_off=%td ies_off=%td session_off=%td encr_off=%td aad4=%zu crypt=%zu kek=%zu snonce=%02x%02x anonce=%02x%02x\n",
			   *frame_len, capab - frame, ies - frame,
			   (u8 *)session - frame, encr - frame, len[4],
			   crypt_len, assoc_data->fils_kek_len,
			   assoc_data->fils_nonces[0],
			   assoc_data->fils_nonces[1],
			   assoc_data->fils_nonces[FILS_NONCE_LEN],
			   assoc_data->fils_nonces[FILS_NONCE_LEN + 1]);
