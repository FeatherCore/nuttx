...
/*
 * The NuttX simulator hwsim path currently relays frames through hostfs and
 * NuttX work queues, so the Linux default 100-200 ms MLME retry timers can
 * expire before a just-received management frame is drained from the queue.
 */
#define IEEE80211_AUTH_TIMEOUT		(HZ * 2)
#define IEEE80211_AUTH_TIMEOUT_LONG	(HZ * 3)
#define IEEE80211_AUTH_TIMEOUT_SHORT	(HZ)
#else
...
#define IEEE80211_ASSOC_TIMEOUT_LONG	(HZ * 3)
#define IEEE80211_ASSOC_TIMEOUT_SHORT	(HZ)
#else
...
static int ieee80211_send_assoc(struct ieee80211_sub_if_data *sdata)
{
	...
	*pos++ = assoc_data->ssid_len;
	...
	nuttx_hwsim_debugf("hwsim-debug: send_assoc tx sdata=%s len=%u fc=0x%04x sta=%p ap=%02x:%02x:%02x:%02x:%02x:%02x chanctx=%p flags=0x%x\n",
	       sdata->name, skb->len, le16_to_cpu(mgmt->frame_control), sta,
	       sdata->vif.cfg.ap_addr[0], sdata->vif.cfg.ap_addr[1],
	       sdata->vif.cfg.ap_addr[2], sdata->vif.cfg.ap_addr[3],
	       sdata->vif.cfg.ap_addr[4], sdata->vif.cfg.ap_addr[5],
					...
					bool assoc)
{
	...
	static int destroy_auth_debug_logs;

	lockdep_assert_wiphy(sdata->local->hw.wiphy);

	if (destroy_auth_debug_logs < 24) {
		nuttx_hwsim_debugf("hwsim-debug: destroy_auth_data sdata=%s auth_data=%p assoc=%d ap=%02x:%02x:%02x:%02x:%02x:%02x\n",
		       sdata->name, auth_data, assoc,
		       auth_data ? auth_data->ap_addr[0] : 0,
		       auth_data ? auth_data->ap_addr[1] : 0,
		       auth_data ? auth_data->ap_addr[2] : 0,
		       auth_data ? auth_data->ap_addr[3] : 0,
		       auth_data ? auth_data->ap_addr[4] : 0,
		       auth_data ? auth_data->ap_addr[5] : 0);
		destroy_auth_debug_logs++;
	}
...
static bool ieee80211_mark_sta_auth(struct ieee80211_sub_if_data *sdata)
{
	...
	if (!sta) {
		nuttx_hwsim_debugf("hwsim-debug: mark_sta_auth missing sta sdata=%s ap=%02x:%02x:%02x:%02x:%02x:%02x auth_data=%p\n",
		       sdata->name,
		       ap_addr[0], ap_addr[1], ap_addr[2],
		       ap_addr[3], ap_addr[4], ap_addr[5],
		       ifmgd->auth_data);
		WARN_ONCE(1, "%s: STA %pM not found", sdata->name, ap_addr);
		return false;
	}
	if (sta_info_move_state(sta, IEEE80211_STA_AUTH)) {
		nuttx_hwsim_debugf("hwsim-debug: mark_sta_auth move failed sdata=%s sta=%p ap=%02x:%02x:%02x:%02x:%02x:%02x\n",
		       sdata->name, sta,
		       ap_addr[0], ap_addr[1], ap_addr[2],
		       ap_addr[3], ap_addr[4], ap_addr[5]);
				   ...
				   struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	static int auth_debug_logs;

	lockdep_assert_wiphy(sdata->local->hw.wiphy);

	if (len < 24 + 6) {
		if (auth_debug_logs < 24) {
			nuttx_hwsim_debugf("hwsim-debug: ieee80211_rx_mgmt_auth short len=%zu\n",
			       len);
			auth_debug_logs++;
		}
		return;
	...
	if (!ifmgd->auth_data || ifmgd->auth_data->done) {
		if (auth_debug_logs < 24) {
			nuttx_hwsim_debugf("hwsim-debug: ieee80211_rx_mgmt_auth no-auth-data auth_data=%p done=%d sa=%02x:%02x:%02x:%02x:%02x:%02x bssid=%02x:%02x:%02x:%02x:%02x:%02x\n",
			       ifmgd->auth_data,
			       ifmgd->auth_data ? ifmgd->auth_data->done : -1,
	...
	if (!ether_addr_equal(ifmgd->auth_data->ap_addr, mgmt->bssid)) {
		if (auth_debug_logs < 24) {
			nuttx_hwsim_debugf("hwsim-debug: ieee80211_rx_mgmt_auth bssid-miss ap=%02x:%02x:%02x:%02x:%02x:%02x bssid=%02x:%02x:%02x:%02x:%02x:%02x\n",
			       ifmgd->auth_data->ap_addr[0],
			       ifmgd->auth_data->ap_addr[1],
			       ifmgd->auth_data->ap_addr[2],
			       ifmgd->auth_data->ap_addr[3],
			       ifmgd->auth_data->ap_addr[4],
			       ifmgd->auth_data->ap_addr[5],
			       mgmt->bssid[0], mgmt->bssid[1],
			       mgmt->bssid[2], mgmt->bssid[3],
			       mgmt->bssid[4], mgmt->bssid[5]);
			auth_debug_logs++;
	...
	if (auth_debug_logs < 24) {
		nuttx_hwsim_debugf("hwsim-debug: ieee80211_rx_mgmt_auth alg=%u expected_alg=%u trans=%u expected_trans=%u status=%u sa=%02x:%02x:%02x:%02x:%02x:%02x\n",
		       auth_alg, ifmgd->auth_data->algorithm,
				  ...
				  struct sk_buff *skb)
{
	...
	if ((fc & IEEE80211_FCTL_STYPE) == IEEE80211_STYPE_AUTH) {
		static int queued_auth_logs;

		if (queued_auth_logs < 24) {
			nuttx_hwsim_debugf("hwsim-debug: ieee80211_sta_rx_queued_mgmt auth len=%u sa=%02x:%02x:%02x:%02x:%02x:%02x da=%02x:%02x:%02x:%02x:%02x:%02x bssid=%02x:%02x:%02x:%02x:%02x:%02x auth_data=%p\n",
			       skb->len,
			       mgmt->sa[0], mgmt->sa[1], mgmt->sa[2],
			       mgmt->sa[3], mgmt->sa[4], mgmt->sa[5],
...
static int ieee80211_do_assoc(struct ieee80211_sub_if_data *sdata)
{
	...
	nuttx_hwsim_debugf("hwsim-debug: do_assoc send ret=%d sdata=%s tries=%d ap=%02x:%02x:%02x:%02x:%02x:%02x\n",
	       ret, sdata->name, assoc_data->tries,
	       assoc_data->ap_addr[0], assoc_data->ap_addr[1],
	       assoc_data->ap_addr[2], assoc_data->ap_addr[3],
	       assoc_data->ap_addr[4], assoc_data->ap_addr[5]);
	if (ret)
		return ret;
...
void ieee80211_sta_work(struct ieee80211_sub_if_data *sdata)
{
	...
	if (ifmgd->auth_data && ifmgd->auth_data->timeout_started &&
		...
		if (ifmgd->auth_data->done || ifmgd->auth_data->waiting) {
			 ...
			 */
			nuttx_hwsim_debugf("hwsim-debug: auth_data timeout cleanup sdata=%s auth_data=%p done=%d waiting=%d\n",
			       sdata->name, ifmgd->auth_data,
			       ifmgd->auth_data->done,
			       ifmgd->auth_data->waiting);
			ieee80211_destroy_auth_data(sdata, false);
			...
			nuttx_hwsim_debugf("hwsim-debug: auth retry failed cleanup sdata=%s auth_data=%p ap=%02x:%02x:%02x:%02x:%02x:%02x\n",
			       sdata->name, ifmgd->auth_data,
			       ap_addr[0], ap_addr[1], ap_addr[2],
			       ap_addr[3], ap_addr[4], ap_addr[5]);
			ieee80211_destroy_auth_data(sdata, false);
				     ...
				     unsigned long *userspace_selectors)
{
	...
	if (new_sta) {
		...
		nuttx_hwsim_debugf("hwsim-debug: prep_connection inserted sta sdata=%s ap=%02x:%02x:%02x:%02x:%02x:%02x assoc=%d\n",
		       sdata->name,
		       ap_mld_addr[0], ap_mld_addr[1], ap_mld_addr[2],
		       ap_mld_addr[3], ap_mld_addr[4], ap_mld_addr[5],
		       assoc);
	} else
		       ...
		       struct cfg80211_auth_request *req)
{
	...
	nuttx_hwsim_debugf("hwsim-debug: mgd_auth set auth_data sdata=%s auth_data=%p ap=%02x:%02x:%02x:%02x:%02x:%02x bss=%p\n",
	       sdata->name, auth_data,
	       auth_data->ap_addr[0], auth_data->ap_addr[1],
	       auth_data->ap_addr[2], auth_data->ap_addr[3],
	       auth_data->ap_addr[4], auth_data->ap_addr[5],
	       auth_data->bss);
 ...
 err_clear:
	...
	nuttx_hwsim_debugf("hwsim-debug: mgd_auth err_clear sdata=%s auth_data=%p err=%d\n",
	       sdata->name, auth_data, err);
	kfree(auth_data);
	return err;
}
...
#ifdef CONFIG_WL_NUTTX_HWSIM
	/*
	 * The NuttX timer compatibility layer can run same-jiffy timers
	 * before jiffies advances, which makes time_after(jiffies, timeout)
	 * false for the initial association timeout. Queue the STA work
	 * directly so the first association request is emitted.
	 */
	nuttx_hwsim_debugf("hwsim-debug: mgd_assoc queue initial sta_work sdata=%s timeout=%lu jiffies=%lu\n",
	       sdata->name, assoc_data->timeout, jiffies);
	wiphy_work_queue(sdata->local->hw.wiphy, &sdata->work);
#endif
