				...
				u32 mask, u32 set)
{
	...
	nuttx_hwsim_debugf("hwsim-debug: mac80211 sta_apply_auth_flags sdata=%s sta=%02x:%02x:%02x:%02x:%02x:%02x mask=0x%x set=0x%x before auth=%d assoc=%d authorized=%d\n",
	       sta->sdata->name, sta->sta.addr[0], sta->sta.addr[1],
	       sta->sta.addr[2], sta->sta.addr[3], sta->sta.addr[4],
	       sta->sta.addr[5], mask, set,
	...
	if (mask & BIT(NL80211_STA_FLAG_AUTHENTICATED) &&
		...
		if (ret) {
#ifdef CONFIG_WL_NUTTX_HWSIM
			nuttx_hwsim_debugf("hwsim-debug: mac80211 sta auth move failed ret=%d\n",
			       ret);
#endif
			return ret;
		}
	...
	if (mask & BIT(NL80211_STA_FLAG_ASSOCIATED) &&
		...
		if (ret) {
#ifdef CONFIG_WL_NUTTX_HWSIM
			nuttx_hwsim_debugf("hwsim-debug: mac80211 sta assoc move failed ret=%d\n",
			       ret);
#endif
			return ret;
		}
	...
	if (mask & BIT(NL80211_STA_FLAG_AUTHORIZED)) {
		...
		if (ret) {
#ifdef CONFIG_WL_NUTTX_HWSIM
			nuttx_hwsim_debugf("hwsim-debug: mac80211 sta authorized move failed ret=%d\n",
			       ret);
#endif
			return ret;
		}
...
#ifdef CONFIG_WL_NUTTX_HWSIM
	nuttx_hwsim_debugf("hwsim-debug: mac80211 sta_apply_auth_flags done sdata=%s sta=%02x:%02x:%02x:%02x:%02x:%02x auth=%d assoc=%d authorized=%d\n",
	       sta->sdata->name, sta->sta.addr[0], sta->sta.addr[1],
	       sta->sta.addr[2], sta->sta.addr[3], sta->sta.addr[4],
	       sta->sta.addr[5],
	       test_sta_flag(sta, WLAN_STA_AUTH),
...
#ifdef CONFIG_WL_NUTTX_HWSIM
	nuttx_hwsim_debugf("hwsim-debug: mac80211 sta_apply_parameters sdata=%s sta=%02x:%02x:%02x:%02x:%02x:%02x mask=0x%x set=0x%x initial_auth=%d initial_assoc=%d initial_authorized=%d\n",
	       sdata->name, sta->sta.addr[0], sta->sta.addr[1],
	       sta->sta.addr[2], sta->sta.addr[3], sta->sta.addr[4],
	       sta->sta.addr[5], mask, set,
	       test_sta_flag(sta, WLAN_STA_AUTH),
	       test_sta_flag(sta, WLAN_STA_ASSOC),
...
#ifdef CONFIG_WL_NUTTX_HWSIM
	nuttx_hwsim_debugf("hwsim-debug: mac80211 change_station sdata=%s mac=%02x:%02x:%02x:%02x:%02x:%02x mask=0x%x set=0x%x current_auth=%d current_assoc=%d current_authorized=%d\n",
	       sdata->name, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5],
