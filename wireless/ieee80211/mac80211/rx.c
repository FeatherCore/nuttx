...
static bool ieee80211_is_control_port_protocol(
	struct ieee80211_sub_if_data *sdata, __be16 proto);
...
ieee80211_rx_h_check(struct ieee80211_rx_data *rx)
{
	...
	if (unlikely((ieee80211_is_data(hdr->frame_control) ||
		...
		if (rx->sta && rx->sdata->vif.type == NL80211_IFTYPE_STATION &&
			...
			if (ieee80211_is_control_port_protocol(rx->sdata,
							       ethertype))
				return RX_CONTINUE;
		}
...
static void sta_ps_start(struct sta_info *sta)
{
	...
	nuttx_hwsim_ps_prooff("ps-proof: sta sleep sta=" NUTTX_HWSIM_MAC_FMT
			      " aid=%d num_sta_ps=%d txq_tids=0x%lx\n",
			      NUTTX_HWSIM_MAC_ARG(sta->sta.addr),
			      sta->sta.aid, atomic_read(&ps->num_sta_ps),
...
static void sta_ps_end(struct sta_info *sta)
{
	ps_dbg(sta->sdata, "STA %pM aid %d exits power save mode\n",
	       sta->sta.addr, sta->sta.aid);
	nuttx_hwsim_ps_prooff("ps-proof: sta wake sta=" NUTTX_HWSIM_MAC_FMT
			      " aid=%d driver_ps=%d\n",
			      NUTTX_HWSIM_MAC_ARG(sta->sta.addr),
			      sta->sta.aid,
			      test_sta_flag(sta, WLAN_STA_PS_DRIVER) ? 1 : 0);
...
void ieee80211_sta_pspoll(struct ieee80211_sta *pubsta)
{
	...
	nuttx_hwsim_ps_prooff("ps-proof: pspoll rx sta=" NUTTX_HWSIM_MAC_FMT
			      " aid=%d driver_ps=%d\n",
			      NUTTX_HWSIM_MAC_ARG(sta->sta.addr),
			      sta->sta.aid,
			      test_sta_flag(sta, WLAN_STA_PS_DRIVER) ? 1 : 0);
...
__ieee80211_data_to_8023(struct ieee80211_rx_data *rx, bool *port_control)
{
	...
	*port_control = false;
	...
	if (ieee80211_is_control_port_protocol(rx->sdata, ehdr->h_proto))
		*port_control = true;
...
static bool ieee80211_is_control_port_protocol(
	struct ieee80211_sub_if_data *sdata, __be16 proto)
{
	return proto == sdata->control_port_protocol ||
	       ntohs(proto) == be16_to_cpu(sdata->control_port_protocol);
}
...
static bool ieee80211_frame_allowed(struct ieee80211_rx_data *rx, __le16 fc)
{
	 ...
	 */
	if (unlikely(ieee80211_is_control_port_protocol(rx->sdata,
							ehdr->h_proto)))
						 ...
						 struct ieee80211_rx_data *rx)
{
	...
	if (unlikely((ieee80211_is_control_port_protocol(sdata,
							 skb->protocol) ||
		 ...
		 */
		if (unlikely(ieee80211_is_control_port_protocol(
				     sdata, skb->protocol) &&
...
ieee80211_deliver_skb(struct ieee80211_rx_data *rx)
{
...
#ifdef CONFIG_WL_NUTTX_HWSIM
	if (hwsim_deliver_logs < 24) {
		nuttx_hwsim_debugf("hwsim-debug: mac80211 deliver data sdata=%s iftype=%d len=%u proto=0x%04x dst=%02x:%02x:%02x:%02x:%02x:%02x src=%02x:%02x:%02x:%02x:%02x:%02x sta=%p\n",
		       sdata->name, sdata->vif.type, skb->len,
		       ntohs(ehdr->h_proto),
		       ehdr->h_dest[0], ehdr->h_dest[1], ehdr->h_dest[2],
		       ehdr->h_dest[3], ehdr->h_dest[4], ehdr->h_dest[5],
		       ehdr->h_source[0], ehdr->h_source[1],
		       ehdr->h_source[2], ehdr->h_source[3],
		       ehdr->h_source[4], ehdr->h_source[5], rx->sta);
		hwsim_deliver_logs++;
	}
#endif
	...
	if ((sdata->vif.type == NL80211_IFTYPE_AP ||
	    ...
	    !ieee80211_is_control_port_protocol(rx->sdata, ehdr->h_proto) &&
	    (sdata->vif.type != NL80211_IFTYPE_AP_VLAN || !sdata->u.vlan.sta)) {
...
ieee80211_rx_h_data(struct ieee80211_rx_data *rx)
{
...
#ifdef CONFIG_WL_NUTTX_HWSIM
	if (hwsim_data_logs < 24) {
		nuttx_hwsim_debugf("hwsim-debug: mac80211 rx_h_data enter sdata=%s iftype=%d len=%u fc=0x%04x sta=%p link_sta=%p dst=%02x:%02x:%02x:%02x:%02x:%02x src=%02x:%02x:%02x:%02x:%02x:%02x bssid=%02x:%02x:%02x:%02x:%02x:%02x\n",
		       sdata->name, sdata->vif.type, rx->skb->len,
		       le16_to_cpu(fc), rx->sta, rx->link_sta,
...
#ifdef CONFIG_WL_NUTTX_HWSIM
		nuttx_hwsim_debugf("hwsim-debug: mac80211 rx_h_data drop data_to_8023 res=%u sdata=%s\n",
		       (__force u32)res, sdata->name);
#endif
		return res;
	}
	...
	if (res != RX_CONTINUE) {
#ifdef CONFIG_WL_NUTTX_HWSIM
		nuttx_hwsim_debugf("hwsim-debug: mac80211 rx_h_data drop mesh res=%u sdata=%s\n",
		       (__force u32)res, sdata->name);
#endif
		return res;
	}
	...
	if (!ieee80211_frame_allowed(rx, fc)) {
#ifdef CONFIG_WL_NUTTX_HWSIM
		nuttx_hwsim_debugf("hwsim-debug: mac80211 rx_h_data drop frame_allowed sdata=%s sta=%p port_control=%d proto=0x%04x\n",
		       sdata->name, rx->sta, port_control,
		       ntohs(((struct ethhdr *)rx->skb->data)->h_proto));
#endif
...
ieee80211_rx_h_mgmt_check(struct ieee80211_rx_data *rx)
{
	...
	if (ieee80211_is_auth(mgmt->frame_control))
		nuttx_hwsim_debugf("hwsim-debug: rx_h_mgmt_check auth sdata=%s iftype=%d len=%u fc=0x%04x flags=0x%x\n",
				   rx->sdata->name, rx->sdata->vif.type,
				   rx->skb->len, le16_to_cpu(mgmt->frame_control),
				   rx->flags);
...
ieee80211_rx_h_userspace_mgmt(struct ieee80211_rx_data *rx)
{
	...
	if (ieee80211_is_auth(mgmt->frame_control))
		nuttx_hwsim_debugf("hwsim-debug: userspace_mgmt auth enter sdata=%s iftype=%d len=%u freq_khz=%u rx_flags=0x%x\n",
				   rx->sdata->name, rx->sdata->vif.type,
				   rx->skb->len, info.freq, status->rx_flags);

	if (cfg80211_rx_mgmt_ext(&rx->sdata->wdev, &info)) {
		if (ieee80211_is_auth(mgmt->frame_control))
			nuttx_hwsim_debugf("hwsim-debug: userspace_mgmt auth queued sdata=%s\n",
					   rx->sdata->name);
		if (rx->sta)
			rx->link_sta->rx_stats.packets++;
		dev_kfree_skb(rx->skb);
	...
	if (ieee80211_is_auth(mgmt->frame_control))
		nuttx_hwsim_debugf("hwsim-debug: userspace_mgmt auth continue sdata=%s\n",
				   rx->sdata->name);
...
ieee80211_rx_h_mgmt(struct ieee80211_rx_data *rx)
{
	...
	if (!ieee80211_vif_is_mesh(&sdata->vif) &&
	    ...
	    sdata->vif.type != NL80211_IFTYPE_STATION) {
		if (stype == cpu_to_le16(IEEE80211_STYPE_AUTH))
			nuttx_hwsim_debugf("hwsim-debug: rx_h_mgmt drop auth unhandled sdata=%s iftype=%d\n",
...
static bool nuttx_hwsim_debug_rx_auth(struct sk_buff *skb)
{
	struct ieee80211_hdr *hdr;
...
static bool ieee80211_accept_frame(struct ieee80211_rx_data *rx)
{
	...
	bool debug_auth = nuttx_hwsim_debug_rx_auth(skb);

	if (debug_auth)
		nuttx_hwsim_debugf("hwsim-debug: accept auth enter sdata=%s iftype=%d len=%u freq=%u bssid=%02x:%02x:%02x:%02x:%02x:%02x a1=%02x:%02x:%02x:%02x:%02x:%02x a2=%02x:%02x:%02x:%02x:%02x:%02x a3=%02x:%02x:%02x:%02x:%02x:%02x\n",
				   sdata->name, sdata->vif.type, skb->len,
				   status->freq,
	...
	case NL80211_IFTYPE_AP:
		...
		if (debug_auth)
			nuttx_hwsim_debugf("hwsim-debug: accept auth pass ap sdata=%s link_id=%d\n",
					   sdata->name, rx->link_id);
				       ...
				       struct sk_buff *skb, bool consume)
{
	...
	if (nuttx_hwsim_debug_rx_auth(skb))
		nuttx_hwsim_debugf("hwsim-debug: rx_for_interface auth sdata=%s iftype=%d consume=%d sta=%p link_id=%d\n",
				   rx->sdata->name, rx->sdata->vif.type,
				   consume, sta, link_id);
					 ...
					 struct list_head *list)
{
	...
	if (nuttx_hwsim_debug_rx_auth(skb))
		nuttx_hwsim_debugf("hwsim-debug: rx_handle_packet auth enter len=%u band=%u freq=%u rate_idx=%u local_started=%d interfaces_empty=%d\n",
				   skb->len, status->band, status->freq,
				   status->rate_idx, local->started,
	...
	list_for_each_entry_rcu(sdata, &local->interfaces, list) {
		if (nuttx_hwsim_debug_rx_auth(skb))
			nuttx_hwsim_debugf("hwsim-debug: rx_handle_packet auth consider sdata=%s iftype=%d running=%d\n",
					   sdata->name, sdata->vif.type,
		       ...
		       struct sk_buff *skb, struct list_head *list)
{
	...
	if (debug_auth)
		nuttx_hwsim_debugf("hwsim-debug: rx_list auth enter len=%u band=%u freq=%u freq_offset=%u encoding=%u rate_idx=%u nss=%u flag=0x%x\n",
				   skb->len, status->band, status->freq,
				   status->freq_offset, status->encoding,
				   status->rate_idx, status->nss, status->flag);

	if (WARN_ON(status->band >= NUM_NL80211_BANDS)) {
		if (debug_auth)
			nuttx_hwsim_debugf("hwsim-debug: rx_list auth drop bad_band=%u\n",
					   status->band);
		goto drop;
	}
	...
	if (WARN_ON(!sband)) {
		if (debug_auth)
			nuttx_hwsim_debugf("hwsim-debug: rx_list auth drop no_sband band=%u\n",
					   status->band);
		goto drop;
	}
	...
	if (unlikely(local->quiescing || local->suspended)) {
		if (debug_auth)
			nuttx_hwsim_debugf("hwsim-debug: rx_list auth drop quiescing=%d suspended=%d\n",
					   local->quiescing, local->suspended);
		goto drop;
	}
	...
	if (unlikely(local->in_reconfig)) {
		if (debug_auth)
			nuttx_hwsim_debugf("hwsim-debug: rx_list auth drop in_reconfig\n");
		goto drop;
	}
	...
	if (WARN_ON(!local->started)) {
		if (debug_auth)
			nuttx_hwsim_debugf("hwsim-debug: rx_list auth drop local_not_started\n");
		goto drop;
	}
	...
	if (likely(!(status->flag & RX_FLAG_FAILED_PLCP_CRC) &&
		...
		case RX_ENC_LEGACY:
			if (WARN_ON(status->rate_idx >= sband->n_bitrates)) {
				if (debug_auth)
					nuttx_hwsim_debugf("hwsim-debug: rx_list auth drop bad_legacy_rate rate_idx=%u n_bitrates=%u band=%u\n",
							   status->rate_idx,
							   sband->n_bitrates,
							   status->band);
				goto drop;
	...
	if (WARN_ON_ONCE(status->link_id >= IEEE80211_LINK_UNSPECIFIED)) {
		if (debug_auth)
			nuttx_hwsim_debugf("hwsim-debug: rx_list auth drop bad_link_id=%u\n",
					   status->link_id);
		goto drop;
	}
	...
	if (skb) {
		if (debug_auth)
			nuttx_hwsim_debugf("hwsim-debug: rx_list auth dispatch flag=0x%x\n",
					   status->flag);
 ...
 drop:
	if (debug_auth)
		nuttx_hwsim_debugf("hwsim-debug: rx_list auth drop final\n");
	kfree_skb(skb);
}
