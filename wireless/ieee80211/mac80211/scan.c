				  ...
				  struct cfg80211_scan_request *req)
{
 ...
 again:
	...
	nuttx_hwsim_debugf("hwsim-debug: start_scan hw_scan=%d ops=%p hw_scan_op=%p channels=%u ssids=%u\n",
	       hw_scan, local->ops, local->ops ? local->ops->hw_scan : NULL,
	       req->n_channels, req->n_ssids);
	if (hw_scan) {
		bool prepared = ieee80211_prep_hw_scan(sdata);

		nuttx_hwsim_debugf("hwsim-debug: prep_hw_scan prepared=%d hw_req=%p hw_channels=%u\n",
		       prepared, local->hw_scan_req,
		       local->hw_scan_req ? local->hw_scan_req->req.n_channels : 0);
		WARN_ON(!prepared);
		rc = drv_hw_scan(local, sdata, local->hw_scan_req);
		nuttx_hwsim_debugf("hwsim-debug: drv_hw_scan rc=%d\n", rc);
	} else {
		rc = ieee80211_start_sw_scan(local, sdata);
		nuttx_hwsim_debugf("hwsim-debug: start_sw_scan rc=%d\n", rc);
	}
...
void ieee80211_scan_work(struct wiphy *wiphy, struct wiphy_work *work)
{
	...
	nuttx_hwsim_debugf("hwsim-debug: ieee80211_scan_work enter local=%p scanning=0x%lx\n",
	       local, local->scanning);
