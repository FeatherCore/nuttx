		...
		if (roc->mgmt_tx_cookie) {
			if (!WARN_ON(!roc->frame)) {
				struct ieee80211_hdr *hdr =
					(struct ieee80211_hdr *)roc->frame->data;

				if (roc->frame->len >= sizeof(*hdr) &&
				    ieee80211_is_probe_resp(hdr->frame_control))
					nuttx_hwsim_debugf("hwsim-debug: roc tx probe_resp sdata=%s freq=%u len=%u flags=0x%x\n",
					       roc->sdata->name,
					       roc->chan ? roc->chan->center_freq : 0,
					       roc->frame->len,
					       IEEE80211_SKB_CB(roc->frame)->flags);
