...
static bool hwsim_debug_is_assoc_skb(struct sk_buff *skb)
{
...
ieee80211_tx_h_unicast_ps_buf(struct ieee80211_tx_data *tx)
{
	...
	if (unlikely((test_sta_flag(sta, WLAN_STA_PS_STA) ||
		...
		info->control.jiffies = jiffies;
			...
			{
				static unsigned int ps_buf_logs;
				static unsigned int ps_full_logs;
				unsigned int qlen = skb_queue_len(&sta->ps_tx_buf[ac]);
				bool log_ps_buf = false;

				if (qlen == STA_MAX_TX_BUFFER) {
					log_ps_buf = ps_full_logs < 16 ||
						     ps_full_logs % 128 == 0;
					ps_full_logs++;
				} else if (qlen <= 8) {
					log_ps_buf = ps_buf_logs < 16 ||
						     ps_buf_logs % 128 == 0;
					ps_buf_logs++;
				}

				if (log_ps_buf) {
					nuttx_hwsim_ps_prooff("ps-proof: buffer unicast sta="
							      NUTTX_HWSIM_MAC_FMT " aid=%d ac=%d "
							      "queue=%u total=%d low_sample=%u "
							      "full_sample=%u\n",
			  ...
			  struct sta_info *sta, struct sk_buff *skb)
{
	...
	static bool reported_no_ref;
	static bool reported_skip;
	static bool reported_existing;
	static bool reported_start;
	u16 tid;

	if (!ref || !(ref->ops->capa & RATE_CTRL_CAPA_AMPDU_TRIGGER)) {
		if (!reported_no_ref) {
			nuttx_hwsim_ampdu_prooff("ampdu-proof: aggr_check skip no "
						 "rate_ctrl trigger ref=%p capa=0x%x\n",
						 ref, ref ? ref->ops->capa : 0);
			reported_no_ref = true;
		}
		return;
	}
	...
	if (!sta ||
		...
		if (!reported_skip) {
			nuttx_hwsim_ampdu_prooff("ampdu-proof: aggr_check skip "
						 "sta=%p ht=%d s1g=%d wme=%d "
						 "queue=%u proto=0x%04x control=0x%04x\n",
						 sta,
						 sta ? sta->sta.deflink.ht_cap.ht_supported : 0,
						 sta ? sta->sta.deflink.s1g_cap.s1g : 0,
						 sta ? sta->sta.wme : 0,
						 skb_get_queue_mapping(skb),
						 ntohs(skb->protocol),
						 ntohs(sdata->control_port_protocol));
			reported_skip = true;
		}
		return;
	}
	...
	tid = skb->priority & IEEE80211_QOS_CTL_TID_MASK;
		if (likely(sta->ampdu_mlme.tid_tx[tid])) {
			if (!reported_existing) {
				nuttx_hwsim_ampdu_prooff("ampdu-proof: aggr_check existing "
							 "tid=%u sta=" NUTTX_HWSIM_MAC_FMT "\n",
							 tid, NUTTX_HWSIM_MAC_ARG(sta->sta.addr));
		...
		if (!reported_start) {
			nuttx_hwsim_ampdu_prooff("ampdu-proof: aggr_check start tid=%u "
				...
				struct sk_buff *skb)
{
	...
	if (hwsim_debug_is_assoc_skb(skb)) {
		nuttx_hwsim_debugf("hwsim-debug: tx queue assoc vif=%p sta=%p uploaded=%d txq=%p ac=%d len=%u flags=0x%x hwq=%d\n",
		       vif, sta, sta ? sta->uploaded : -1, &txqi->txq,
		       txqi->txq.ac, skb->len, info->flags,
		       info->hw_queue);
	}
			       ...
			       bool txpending)
{
	...
	skb_queue_walk_safe(skbs, skb, tmp) {
		...
		if (hwsim_debug_is_assoc_skb(skb)) {
			...
			nuttx_hwsim_debugf("hwsim-debug: drv_tx mgmt fc=0x%04x vif=%p sta=%p len=%u flags=0x%x hwq=%d\n",
			       le16_to_cpu(hdr->frame_control), vif,
			       control.sta, skb->len, info->flags,
			       info->hw_queue);
		}
			 ...
			 bool txpending)
{
	...
	struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
	bool result = true;
	bool debug_assoc = hwsim_debug_is_assoc_skb(skb);
	...
	if (debug_assoc) {
		nuttx_hwsim_debugf("hwsim-debug: tx assoc prepare res=%d sdata=%s sta_in=%p txsta=%p uploaded=%d len=%u flags=0x%x\n",
		       res_prepare, sdata->name, sta, tx.sta,
		       tx.sta ? tx.sta->uploaded : -1, skb->len, info->flags);
	}
	...
	if (invoke_tx_handlers_early(&tx)) {
		if (debug_assoc) {
			nuttx_hwsim_debugf("hwsim-debug: tx assoc early handler consumed/drop sdata=%s flags=0x%x\n",
			       sdata->name, info->flags);
		}
		return true;
	}
	...
	if (ieee80211_queue_skb(local, sdata, tx.sta, tx.skb)) {
		if (debug_assoc) {
			nuttx_hwsim_debugf("hwsim-debug: tx assoc queued sdata=%s sta=%p flags=0x%x hwq=%d\n",
			       sdata->name, tx.sta, info->flags, info->hw_queue);
		}
		return true;
	}
	...
	if (!invoke_tx_handlers_late(&tx)) {
		if (debug_assoc) {
			nuttx_hwsim_debugf("hwsim-debug: tx assoc direct sdata=%s sta=%p flags=0x%x hwq=%d\n",
			       sdata->name, tx.sta, info->flags, info->hw_queue);
		}
		result = __ieee80211_tx(local, &tx.skbs, tx.sta, txpending);
	} else if (debug_assoc) {
		nuttx_hwsim_debugf("hwsim-debug: tx assoc late handler consumed/drop sdata=%s flags=0x%x\n",
		       sdata->name, info->flags);
	}
...
static unsigned int g_nuttx_hwsim_amsdu_probe_budget = 96;

#define nuttx_hwsim_amsdu_probe(_fmt, ...) \
	do { \
		if (g_nuttx_hwsim_amsdu_probe_budget > 0) { \
			g_nuttx_hwsim_amsdu_probe_budget--; \
			nuttx_hwsim_amsdu_prooff("amsdu-proof: probe " _fmt "\n", \
						 ##__VA_ARGS__); \
		} \
	} while (0)
#else
#define nuttx_hwsim_amsdu_probe(_fmt, ...) do { } while (0)
#endif
				      ...
				      const u8 *da, const u8 *sa)
{
	...
	if (!ieee80211_hw_check(&local->hw, TX_AMSDU)) {
		nuttx_hwsim_amsdu_probe("skip no-tx-amsdu tid=%u len=%u",
					tid, skb->len);
		return false;
	}

	if (sdata->vif.offload_flags & IEEE80211_OFFLOAD_ENCAP_ENABLED) {
		nuttx_hwsim_amsdu_probe("skip offload-encap tid=%u len=%u",
					tid, skb->len);
		return false;
	}

	if (ieee80211_vif_is_mesh(&sdata->vif)) {
		nuttx_hwsim_amsdu_probe("skip mesh tid=%u len=%u", tid,
					skb->len);
		return false;
	}

	if (skb_is_gso(skb)) {
		nuttx_hwsim_amsdu_probe("skip gso tid=%u len=%u", tid,
					skb->len);
		return false;
	}

	if (!txq) {
		nuttx_hwsim_amsdu_probe("skip no-txq tid=%u len=%u", tid,
					skb->len);
		return false;
	}
	...
	if (test_bit(IEEE80211_TXQ_NO_AMSDU, &txqi->flags)) {
		nuttx_hwsim_amsdu_probe("skip no-amsdu-flag tid=%u len=%u",
					tid, skb->len);
		return false;
	}
	...
	if (!head || skb_is_gso(head)) {
		nuttx_hwsim_amsdu_probe("skip no-head tid=%u len=%u max=%d "
					"tin_packets=%u tin_bytes=%u head=%p",
					tid, skb->len, max_amsdu_len,
					tin->backlog_packets,
					tin->backlog_bytes, head);
		goto out;
	}
	...
	if (skb->len + head->len > max_amsdu_len) {
		nuttx_hwsim_amsdu_probe("skip too-large tid=%u skb_len=%u "
					"head_len=%u max=%d",
					tid, skb->len, head->len,
					max_amsdu_len);
		goto out;
	}
	...
	if (max_subframes && n > max_subframes) {
		nuttx_hwsim_amsdu_probe("skip max-subframes tid=%u n=%d max=%u",
					tid, n, max_subframes);
		goto out;
	}

	if (max_frags && nfrags > max_frags) {
		nuttx_hwsim_amsdu_probe("skip max-frags tid=%u nfrags=%d max=%d",
					tid, nfrags, max_frags);
		goto out;
	}

	if (!drv_can_aggregate_in_amsdu(local, head, skb)) {
		nuttx_hwsim_amsdu_probe("skip driver-refused tid=%u len=%u "
					"head_len=%u",
					tid, skb->len, head->len);
		goto out;
	}
	...
	if (!ieee80211_amsdu_prepare_head(sdata, fast_tx, head)) {
		nuttx_hwsim_amsdu_probe("skip prepare-head tid=%u len=%u "
					"head_len=%u",
					tid, skb->len, head->len);
		goto out;
	}
	...
	if (!ieee80211_amsdu_realloc_pad(local, skb, sizeof(rfc1042_header) +
						     2 + pad)) {
		nuttx_hwsim_amsdu_probe("skip realloc-pad tid=%u len=%u pad=%d",
					tid, skb->len, pad);
		goto out_recalc;
	}
	...
	*frag_tail = skb;
	...
	nuttx_hwsim_amsdu_prooff("amsdu-proof: aggregate tid=%u sta="
				 NUTTX_HWSIM_MAC_FMT " da="
				 NUTTX_HWSIM_MAC_FMT " sa="
				 NUTTX_HWSIM_MAC_FMT " subframes=%d "
				 "orig_head_len=%u added_len=%u total_len=%u "
				 "max_len=%d data_len=%u\n",
				 tid, NUTTX_HWSIM_MAC_ARG(sta->sta.addr),
				 NUTTX_HWSIM_MAC_ARG(da),
				 NUTTX_HWSIM_MAC_ARG(sa), n, orig_len,
				 skb->len, head->len, max_amsdu_len,
				 head->data_len);
				 ...
				 enum nl80211_band band)
{
	...
	if (hwsim_debug_is_assoc_skb(skb)) {
		nuttx_hwsim_debugf("hwsim-debug: tx_skb_tid assoc sdata=%s tid=%d link=%u band=%d len=%u flags=0x%x\n",
		       sdata->name, tid, link, band, skb->len,
