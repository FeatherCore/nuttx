			      ...
			      int flags, struct nl80211_dump_wiphy_state *state)
{
	...
	case 3:
		...
		for (band = state->band_start;
			...
			case 0:
				if (nl80211_send_band_rateinfo(msg, sband, true))
					goto nla_put_failure;
				state->chan_start++;
				if (state->split)
					break;
	...
	if (wdev->valid_links) {
		...
		for_each_valid_link(wdev, link_id) {
			...
			if (rdev->ops->get_tx_power) {
				...
				txpwr_ret = rdev_get_tx_power(rdev, wdev, -1, link_id, &dbm);
				if (txpwr_ret == 0 &&
				    nla_put_u32(msg, NL80211_ATTR_WIPHY_TX_POWER_LEVEL,
						DBM_TO_MBM(dbm)))
					goto nla_put_failure;
				      ...
				      struct cfg80211_he_bss_color *he_bss_color)
{
	...
	err = nla_parse_nested_deprecated(tb, NL80211_HE_BSS_COLOR_ATTR_MAX,
					  attrs, he_bss_color_policy, NULL);
	if (err)
		return err;
...
static int nl80211_tx_mgmt(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (err)
		return err;
	params.n_csa_offsets = n_csa_offsets;
			      ...
			      struct genl_info *info)
{
	...
	if (internal_flags & NL80211_FLAG_CLEAR_SKB) {
		...
		if (nlh && nlh->nlmsg_len >= NLMSG_HDRLEN &&
		    nlh->nlmsg_len <= skb->end - skb->head)
...
static struct genl_family nl80211_metadata_fam __ro_after_init = {
				...
				enum nl80211_commands cmd)
{
	...
	if (!genl_has_listeners(&nl80211_fam, wiphy_net(&rdev->wiphy),
				NL80211_MCGRP_CONFIG))
		return;
...
#ifdef CONFIG_WIRELESS_IEEE80211_NL80211_METADATA_ONLY
	err = genl_register_family(&nl80211_metadata_fam);
#else
