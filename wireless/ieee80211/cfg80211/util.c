				  ...
				  u8 data_offset, bool is_amsdu)
{
	...
	if (likely(!is_amsdu && iftype != NL80211_IFTYPE_MESH_POINT &&
		   ...
		   ieee80211_get_8023_tunnel_proto(&payload, &proto))) {
			      ...
			      u8 mesh_control)
{
	...
	while (!last) {
		...
		len = ieee80211_amsdu_subframe_length(&proto, hdr.flags,
						      mesh_control);
		subframe_len = sizeof(struct ethhdr) + len;
		padding = (4 - subframe_len) & 0x3;
		...
		if (likely(iftype != NL80211_IFTYPE_MESH_POINT &&
			   ieee80211_get_8023_tunnel_proto(frame->data, &proto))) {
			hdr.eth.h_proto = proto;
