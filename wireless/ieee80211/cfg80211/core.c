...
void wiphy_delayed_work_timer(struct timer_list *t)
{
	...
	dwork->timer.active = false;
	wiphy_work_queue(dwork->wiphy, &dwork->work);
}
EXPORT_SYMBOL(wiphy_delayed_work_timer);

static void wiphy_delayed_work_nuttx(void *arg)
{
	struct wiphy_delayed_work *dwork = arg;

	wiphy_delayed_work_timer(&dwork->timer);
}
			      ...
			      unsigned long delay)
{
	...
	if (!delay) {
		work_cancel(CFG80211_COMPAT_WORKQ, &dwork->nuttx_work);
	...
	dwork->timer.active = true;
	work_cancel(CFG80211_COMPAT_WORKQ, &dwork->nuttx_work);
	work_queue(CFG80211_COMPAT_WORKQ, &dwork->nuttx_work,
		   wiphy_delayed_work_nuttx, dwork, delay);
}
			       ...
			       struct wiphy_delayed_work *dwork)
{
	...
	work_cancel_sync(CFG80211_COMPAT_WORKQ, &dwork->nuttx_work);
