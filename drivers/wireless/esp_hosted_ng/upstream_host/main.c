...
static bool esp_eapol_has_l2_header(struct sk_buff *skb)
{
...
static struct sk_buff *esp_eapol_ensure_l2(struct esp_wifi_device *priv,
					   struct sk_buff *skb)
{
	...
	if (!priv || !priv->ndev || !skb)
		return skb;

	if (esp_eapol_has_l2_header(skb))
		return skb;

	if (skb_headroom(skb) >= ETH_HLEN) {
		ethhdr = (struct ethhdr *)skb_push(skb, ETH_HLEN);
	} else {
		new_skb = esp_if_alloc_skb(priv->adapter, skb->len + ETH_HLEN);
		if (!new_skb) {
			dev_kfree_skb_any(skb);
			return NULL;
		}

		pos = skb_put(new_skb, skb->len + ETH_HLEN);
		ethhdr = (struct ethhdr *)pos;
		memcpy(pos + ETH_HLEN, skb->data, skb->len);
		dev_kfree_skb_any(skb);
		skb = new_skb;
	}

	ether_addr_copy(ethhdr->h_dest, priv->ndev->dev_addr);
	ether_addr_copy(ethhdr->h_source, ap_bssid);
	ethhdr->h_proto = htons(ETH_P_PAE);

	return skb;
}

struct multicast_list mcast_list = {0};
struct esp_adapter g_adapter;
...
static int process_tx_packet(struct sk_buff *skb)
{
	...
	if (g_adapter.capabilities & ESP_CHECKSUM_ENABLED)
...
static int esp_set_mac_address(struct net_device *ndev, void *data)
{
	...
	ret = cmd_set_mac(priv, (uint8_t *)sa->sa_data);
...
int esp_start_ota(struct esp_adapter *adapter, char *fw_ota_file)
{
#ifdef __NuttX__
	(void)adapter;
	(void)fw_ota_file;
	...
	file = filp_open(fw_ota_file, O_RDONLY, 0);
...
static void process_rx_packet(struct esp_adapter *adapter, struct sk_buff *skb)
{
	...
	if (payload_header->if_type == ESP_STA_IF || payload_header->if_type == ESP_AP_IF) {
		...
		if (payload_header->packet_type == PACKET_TYPE_EAPOL) {
			skb = esp_eapol_ensure_l2(priv, skb);
			if (!skb)
				return;
...
static void esp_events_work(struct work_struct *work)
{
	...
	while ((skb = skb_dequeue(&g_adapter.events_skb_q)) != NULL) {
		if (skb->data) {
			process_internal_event(&g_adapter, skb);
		}
		dev_kfree_skb_any(skb);
	}
}

static void esp_poll_bootup_event(struct esp_adapter *adapter)
{
	int attempt;
	...
	for (attempt = 0; attempt < 100; attempt++) {
		...
		esp_get_packets(adapter);
		esp_events_work(&adapter->events_work);
	...
	esp_warn("ESP boot-up event not received yet; wlan0 registration pending\n");
}
...
static struct esp_adapter *init_adapter(void)
{
	...
	g_adapter.if_rx_workqueue = alloc_workqueue("ESP_IF_RX_WORK_QUEUE", 0, 0);

	if (!g_adapter.if_rx_workqueue) {
		deinit_adapter();
		return NULL;
	}

	INIT_WORK(&g_adapter.if_rx_work, esp_if_rx_work);

	skb_queue_head_init(&g_adapter.events_skb_q);

	g_adapter.events_wq = alloc_workqueue("ESP_EVENTS_WORKQUEUE", WQ_HIGHPRI, 0);

	if (!g_adapter.events_wq) {
		deinit_adapter();
		return NULL;
	}

	INIT_WORK(&g_adapter.events_work, esp_events_work);

	INIT_WORK(&g_adapter.mac_flter_work, update_mac_filter);
...
static void deinit_adapter(void)
{
	if (g_adapter.if_context)
		atomic_set(&g_adapter.state, ESP_CONTEXT_DISABLED);

	skb_queue_purge(&g_adapter.events_skb_q);

	if (g_adapter.events_wq)
		destroy_workqueue(g_adapter.events_wq);

	if (g_adapter.if_rx_workqueue)
		destroy_workqueue(g_adapter.if_rx_workqueue);
}
...
#endif
	for (iface_idx = 0; iface_idx < ESP_MAX_INTERFACE; iface_idx++) {
		cmd_deinit_interface(g_adapter.priv[iface_idx]);
	}
	clear_bit(ESP_DRIVER_ACTIVE, &g_adapter.state_flags);
