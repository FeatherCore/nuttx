...
static const struct ieee80211_regdomain esp_regdom = {
	.n_reg_rules = 5,
	.alpha2 = "99",
	.reg_rules = {
		REG_RULE(2412 - 10, 2472 + 10, 40, 0, 20, 0),
		REG_RULE(5180 - 10, 5240 + 10, 80, 0, 20, 0),
		REG_RULE(5260 - 10, 5320 + 10, 80, 0, 20, 0),
		REG_RULE(5500 - 10, 5720 + 10, 160, 0, 20, 0),
		REG_RULE(5745 - 10, 5885 + 10, 160, 0, 20, 0),
	},
};
...
static const struct wiphy_wowlan_support esp_wowlan_support __maybe_unused = {
...
/*	struct wireless_dev *wdev = NULL;*/
	...
	ret = cmd_init_interface(esp_wdev);
	if (ret) {
		esp_err("cmd_init_interface(%s) failed: %d\n", ndev->name, ret);
		goto free_and_return;
	}

	ret = cmd_get_mac(esp_wdev);
	if (ret) {
		esp_err("cmd_get_mac(%s) failed: %d\n", ndev->name, ret);
		goto free_and_return;
	}
	...
	if (ret) {
		esp_err("cfg80211_register_netdevice(%s) failed: %d\n",
			ndev->name, ret);
		goto free_and_return;
	}

	esp_info("registered %s mac=" MACSTR " iftype=%u\n",
		 ndev->name, MAC2STR(esp_wdev->mac_address), type);
...
static int esp_cfg80211_dump_survey(struct wiphy *wiphy,
				    struct net_device *ndev,
				    int idx, struct survey_info *survey)
{
	...
	priv = netdev_priv(ndev);
	if (priv && priv->bss && priv->bss->channel == survey->channel)
		survey->filled |= SURVEY_INFO_IN_USE;

	return 0;
}

static int esp_cfg80211_flush_pmksa(struct wiphy *wiphy,
				    struct net_device *ndev)
{
	if (!wiphy || !ndev)
		return -EINVAL;

	/* ESP-Hosted NG does not expose a firmware PMKSA cache command.
	 * Flushing an empty host-side cache is a no-op, matching Linux drivers
	 * that can accept FLUSH_PMKSA while doing WPA key management in
	 * userspace.
	 */
	return 0;
}
					     ...
					     int *dbm)
{
	...
	/* nl80211 may query TX power while NEW_INTERFACE is being emitted
	 * during ESP boot handling. Avoid re-entering the command path until
	 * the firmware init flow is complete; the cached default is valid
	 * enough for interface capability reporting.
	 */
	if (adapter && test_bit(ESP_INIT_DONE, &adapter->state_flags)) {
		if (cmd_get_tx_power(priv))
			esp_warn("failed to refresh TX power, using cached value\n");
	}
...
static struct cfg80211_ops esp_cfg80211_ops = {
	...
	.dump_survey = esp_cfg80211_dump_survey,
	.flush_pmksa = esp_cfg80211_flush_pmksa,
...
int esp_add_wiphy(struct esp_adapter *adapter)
{
	...
	wiphy_apply_custom_regulatory(wiphy, &esp_regdom);
