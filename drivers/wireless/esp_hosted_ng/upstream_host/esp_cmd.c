...
#define ESP_WIFI_REASON_HANDSHAKE_TIMEOUT 204
...
static bool esp_mac_is_zero(const u8 *addr)
{
	static const u8 zero[MAC_ADDR_LEN] = { 0 };

	return !addr || !memcmp(addr, zero, MAC_ADDR_LEN);
}
...
static int decode_common_resp(struct command_node *cmd_node)
{
	...
	case CMD_RESPONSE_BUSY:
		return -EBUSY;
	case CMD_RESPONSE_UNSUPPORTED:
		return -EOPNOTSUPP;
	case CMD_RESPONSE_INVALID:
		return -EINVAL;
	case CMD_RESPONSE_FAIL:
	default:
		esp_info("[0x%x] Command failed status=%u\n",
			 cmd_node->cmd_code, cmd->cmd_status);
		return -EIO;
	}
}
		...
		struct command_node *cmd_node)
{
	...
	unsigned int timeout_ms = jiffies_to_msecs(COMMAND_RESPONSE_TIMEOUT);
	int ret = 0;
	...
	while (adapter->cmd_resp != cmd_node->cmd_code &&
	       elapsed_ms < timeout_ms) {
		if (adapter->cur_cmd != cmd_node)
			esp_cmd_work(&adapter->cmd_work);
	...
	ret = adapter->cmd_resp == cmd_node->cmd_code ? 1 : 0;
...
int process_cmd_resp(struct esp_adapter *adapter, struct sk_buff *skb)
{
	...
	if (header->cmd_code != adapter->cur_cmd->cmd_code) {
		esp_err("Command response mismatch expected=%d got=%d\n",
...
static void process_deauth_event(struct esp_wifi_device *priv, struct disconnect_event *event)
{
	...
	cfg80211_rx_mlme_mgmt(priv->ndev, (u8 *)&mgmt,
		...
		struct disconnect_event *event)
{
	...
	if (event->reason == ESP_WIFI_REASON_HANDSHAKE_TIMEOUT &&
	    !priv->pairwise_key_installed) {
		priv->disconnect_event_pending = 1;
		priv->pending_disconnect_reason = event->reason;
		esp_info("firmware reported reason 204 before key install; "
			 "defer userspace disconnect while key install may still race\n");
		return;
	}

	priv->pairwise_key_installed = 0;
	priv->disconnect_event_pending = 0;
	priv->pending_disconnect_reason = 0;
		...
		struct assoc_event *event)
{
	...
	memcpy(priv->bssid, event->bssid, MAC_ADDR_LEN);
	memcpy(ap_bssid, event->bssid, MAC_ADDR_LEN);
		...
		struct cfg80211_connect_params *params)
{
	...
	if (params->bssid) {
		...
		memcpy(priv->bssid, params->bssid, MAC_ADDR_LEN);
		...
		const u8 *mac_addr, struct key_params *params)
{
	...
	if (params->key_len > sizeof(((struct wifi_sec_key *)0)->data)) {
		esp_err("Too long key length (%u)\n", params->key_len);
		return -EINVAL;
	}

	if (params->seq_len > sizeof(((struct wifi_sec_key *)0)->seq)) {
	...
	if (priv->disconnect_event_pending) {
		esp_info("allow add_key idx=%u pairwise=%u after deferred "
			 "disconnect reason=%u\n",
			 key_index, pairwise, priv->pending_disconnect_reason);
	}

	if (pairwise) {
		mac = mac_addr;
	} else if (!esp_mac_is_zero(priv->bssid)) {
		mac = priv->bssid;
	} else if (!esp_mac_is_zero(ap_bssid)) {
		mac = ap_bssid;
	} else {
		esp_err("missing AP BSSID for group key\n");
		return -EINVAL;
	}
	if (mac) {
	...
	key->algo = esp_hosted_wpa_cipher_to_alg(params->cipher);
#if 0
	...
	if (ret) {
		esp_err("add_key idx=%u pairwise=%u failed: %d\n",
			key_index, pairwise, ret);
		return ret;
	}

	if (pairwise) {
		priv->pairwise_key_installed = 1;
		priv->disconnect_event_pending = 0;
		priv->pending_disconnect_reason = 0;
	}
