...
#define NUTTX_HWSIM_MEDIUM_BSS_PATH "/h/hwsim-bss.bin"
#define NUTTX_HWSIM_MEDIUM_FRAME_PATH "/h/hwsim-frames.bin"
#define NUTTX_HWSIM_MEDIUM_FRAME_LOCK_PATH "/h/hwsim-frames.lock"
#define NUTTX_HWSIM_DCM_PROOF_PATH "/h/hwsim-dcm-proof"
#define NUTTX_HWSIM_MEDIUM_MAGIC 0x4e485753
#define NUTTX_HWSIM_MEDIUM_VERSION 1
#define NUTTX_HWSIM_MEDIUM_MAX_FRAME 4096
#define NUTTX_HWSIM_MEDIUM_MAX_BSS_RECORDS 16
#define NUTTX_HWSIM_MEDIUM_MAX_RECORDS_PER_POLL 512
#define NUTTX_HWSIM_MEDIUM_POLL_MS 20
#define NUTTX_HWSIM_MEDIUM_LISTEN_MS 10000
#define NUTTX_HWSIM_MEDIUM_LOCK_RETRIES 1000
#define NUTTX_HWSIM_AUTO_PS_POLL_MS 10
#define NUTTX_HWSIM_AUTO_PS_RELEASE_FRAMES 512
#define NUTTX_HWSIM_AUTO_PS_RX_BUDGET 8192
#define NUTTX_HWSIM_AUTO_PS_RX_LOG_GRAN 256
#define NUTTX_HWSIM_DYNPS_IDLE_MS 500
#define NUTTX_HWSIM_TWT_MIN_DELAY_MS 1
#define NUTTX_HWSIM_TWT_MIN_WAKE_MS 500
#define NUTTX_HWSIM_TWT_MIN_INTERVAL_MS 1000
#define NUTTX_HWSIM_TWT_ACTION_CATEGORY 23

#ifndef CONFIG_WL_NUTTX_HWSIM_MEDIUM_MAX_BYTES
#define CONFIG_WL_NUTTX_HWSIM_MEDIUM_MAX_BYTES (64 * 1024 * 1024)
#endif

#define NUTTX_HWSIM_MEDIUM_MAX_BYTES CONFIG_WL_NUTTX_HWSIM_MEDIUM_MAX_BYTES

#ifndef CONFIG_WL_NUTTX_HWSIM_RADIO_BASE
#define CONFIG_WL_NUTTX_HWSIM_RADIO_BASE 0
#endif

#ifndef CONFIG_WL_NUTTX_HWSIM_RADIOS
#define CONFIG_WL_NUTTX_HWSIM_RADIOS 2
#endif

#ifndef CONFIG_WL_NUTTX_HWSIM_CHANNELS
#define CONFIG_WL_NUTTX_HWSIM_CHANNELS 2
#endif

#ifdef CONFIG_WL_NUTTX_HWSIM_DCM_PROOF
#define HWSIM_HE_DCM_PHY_CAPS						\
	.phy_cap_info[3] =						\
		IEEE80211_HE_PHY_CAP3_DCM_MAX_CONST_TX_BPSK |		\
		IEEE80211_HE_PHY_CAP3_DCM_MAX_CONST_RX_BPSK,		\
	.phy_cap_info[8] = IEEE80211_HE_PHY_CAP8_DCM_MAX_RU_484,
#else
#define HWSIM_HE_DCM_PHY_CAPS
#endif
...
static int radios = CONFIG_WL_NUTTX_HWSIM_RADIOS;
module_param(radios, int, 0444);
MODULE_PARM_DESC(radios, "Number of simulated radios");

static int channels = CONFIG_WL_NUTTX_HWSIM_CHANNELS;
...
#ifdef CONFIG_WL_NUTTX_HWSIM_MLO_SUPPORT
static bool mlo = true;
#else
static bool mlo;
#endif
...
#ifdef CONFIG_WL_NUTTX_HWSIM_MULTI_RADIO_SUPPORT
static bool multi_radio = true;
#else
...
/*
 * NuttX sim does not currently have the Linux module-parameter/regdb userspace
 * path wired up. Use hwsim's custom world regulatory domain by default so AP
 * validation on 5 GHz/6 GHz channels is not blocked by the built-in world
 * domain NO_IR flags.
 */
static int regtest =
...
static const struct ieee80211_regdomain hwsim_world_regdom_custom_03 = {
	...
	.reg_rules = {
		...
		REG_RULE(2412 - 10, 2462 + 10, 40, 0, 20, 0),
		REG_RULE(2484 - 10, 2484 + 10, 40, 0, 20, 0),
		REG_RULE(5150 - 10, 5320 + 10, 160, 0, 30, NL80211_RRF_AUTO_BW),
...
static bool mac80211_hwsim_linux_initialized;
...
struct mac80211_hwsim_data {
	...
	struct delayed_work medium_rx;
	struct delayed_work auto_ps_poll;
	struct delayed_work dyn_ps_idle;
	struct delayed_work twt_sp;
	off_t medium_frame_offset;
	struct ieee80211_channel *medium_listen_chan;
	unsigned long medium_listen_until;
	...
	unsigned int ps_poll_budget;
	bool dynps_sleeping;
	unsigned long dynps_idle_deadline;
	bool twt_sp_open;
	bool twt_auto_enabled;
	bool twt_restore_ps;
	bool twt_sp_phase_open;
	unsigned long twt_interval_jiffies;
	unsigned long twt_wake_jiffies;
	unsigned int twt_sp_iterations;
	u8 twt_flowid;
	u8 twt_dialog_token;
...
static void hwsim_send_ps_poll(void *dat, u8 *mac, struct ieee80211_vif *vif)
{
	...
	pspoll->frame_control = cpu_to_le16(IEEE80211_FTYPE_CTL |
					    ...
					    IEEE80211_FCTL_PM |
					    (data->ps == PS_AUTO_POLL ?
					     IEEE80211_FCTL_RETRY : 0));
...
static void hwsim_dyn_ps_idle_work(struct work_struct *work)
{
	...
	if (!data->started || data->ps != PS_DYNAMIC_IDLE)
		return;

	if (time_before(jiffies, data->dynps_idle_deadline)) {
		delay = data->dynps_idle_deadline - jiffies;
		ieee80211_queue_delayed_work(data->hw, &data->dyn_ps_idle,
					     delay);
		return;
	}

	if (data->dynps_sleeping)
		return;

	data->dynps_sleeping = true;
	nuttx_hwsim_ps_prooff("ps-proof: hwsim dynps idle sleep timeout_ms=%u\n",
			      NUTTX_HWSIM_DYNPS_IDLE_MS);
	local_bh_disable();
...
static void hwsim_dyn_ps_tx_activity(struct mac80211_hwsim_data *data,
				     struct sk_buff *skb)
{
	const struct ieee80211_hdr *hdr = (const void *)skb->data;

	if (data->ps != PS_DYNAMIC_IDLE ||
	...
	if (data->dynps_sleeping) {
		data->dynps_sleeping = false;
		nuttx_hwsim_ps_prooff("ps-proof: hwsim dynps tx wake len=%u\n",
				      skb->len);
		local_bh_disable();
		ieee80211_iterate_active_interfaces_atomic(
			data->hw, IEEE80211_IFACE_ITER_NORMAL,
		...
		nuttx_hwsim_ps_prooff("ps-proof: hwsim dynps tx activity len=%u\n",
				      skb->len);
	}

	data->dynps_idle_deadline =
		jiffies + msecs_to_jiffies(NUTTX_HWSIM_DYNPS_IDLE_MS);
	ieee80211_queue_delayed_work(data->hw, &data->dyn_ps_idle,
				     msecs_to_jiffies(NUTTX_HWSIM_DYNPS_IDLE_MS));
}

static bool hwsim_tx_should_force_pm(struct mac80211_hwsim_data *data)
{
	if (data->twt_sp_open || data->ps == PS_DISABLED)
		return false;

	if (data->ps == PS_DYNAMIC_IDLE)
		return data->dynps_sleeping;

	return true;
}

static unsigned long hwsim_twt_usec_to_jiffies(u64 usec)
{
	u64 msec;

	if (!usec)
		usec = NUTTX_HWSIM_TWT_MIN_DELAY_MS * 1000ULL;

	msec = (usec + 999) / 1000;
	if (!msec)
		msec = NUTTX_HWSIM_TWT_MIN_DELAY_MS;
	if (msec > UINT_MAX)
		msec = UINT_MAX;

	return max_t(unsigned long, 1, msecs_to_jiffies((unsigned int)msec));
}

static unsigned long hwsim_twt_msecs_to_jiffies(unsigned int msec)
{
	return max_t(unsigned long, 1, msecs_to_jiffies(msec));
}

static void hwsim_twt_send_pm(struct mac80211_hwsim_data *data, bool awake)
{
	local_bh_disable();
	ieee80211_iterate_active_interfaces_atomic(
		data->hw, IEEE80211_IFACE_ITER_NORMAL,
		awake ? hwsim_send_nullfunc_no_ps : hwsim_send_nullfunc_ps,
		data);
	local_bh_enable();
}

static void hwsim_twt_set_sp(struct mac80211_hwsim_data *data, bool open,
			     bool force, const char *reason)
{
	if (!force && data->twt_sp_open == open)
		return;

	data->twt_sp_open = open;
	data->twt_sp_phase_open = open;
	nuttx_hwsim_ps_prooff("twt-proof: hwsim auto service period %s reason=%s flow=%u iter=%u\n",
			      open ? "open" : "closed", reason,
			      data->twt_flowid, data->twt_sp_iterations);
	hwsim_twt_send_pm(data, open);
}

static void hwsim_twt_disable_auto(struct mac80211_hwsim_data *data,
				   bool restore_ps, const char *reason)
{
	bool was_open = data->twt_sp_open;

	cancel_delayed_work_sync(&data->twt_sp);

	if (!data->twt_auto_enabled && !was_open)
		return;

	data->twt_auto_enabled = false;
	data->twt_sp_open = false;
	data->twt_sp_phase_open = false;
	data->twt_interval_jiffies = 0;
	data->twt_wake_jiffies = 0;
	data->twt_sp_iterations = 0;
	nuttx_hwsim_ps_prooff("twt-proof: hwsim auto disabled reason=%s restore_ps=%d\n",
			      reason, restore_ps && data->twt_restore_ps);

	if (restore_ps && data->twt_restore_ps && data->ps == PS_ENABLED) {
		data->ps = PS_DISABLED;
		hwsim_twt_send_pm(data, true);
	} else if (data->ps == PS_ENABLED) {
		hwsim_twt_send_pm(data, false);
	}

	data->twt_restore_ps = false;
}

static void hwsim_twt_sp_work(struct work_struct *work)
{
	struct mac80211_hwsim_data *data =
		container_of(work, struct mac80211_hwsim_data, twt_sp.work);
	unsigned long delay;

	if (!data->started || !data->twt_auto_enabled ||
	    data->ps != PS_ENABLED)
		return;

	if (data->twt_sp_phase_open) {
		hwsim_twt_set_sp(data, false, false, "period-end");
		if (data->twt_interval_jiffies > data->twt_wake_jiffies)
			delay = data->twt_interval_jiffies -
				data->twt_wake_jiffies;
		else
			delay = data->twt_interval_jiffies;
	} else {
		data->twt_sp_iterations++;
		hwsim_twt_set_sp(data, true, false, "period-start");
		delay = data->twt_wake_jiffies;
	}

	ieee80211_queue_delayed_work(data->hw, &data->twt_sp,
				     max_t(unsigned long, 1, delay));
}

static void hwsim_twt_install_auto(struct mac80211_hwsim_data *data,
				   struct ieee80211_twt_setup *twt)
{
	struct ieee80211_twt_params *params = (void *)twt->params;
	u16 req_type = le16_to_cpu(params->req_type);
	u8 setup_cmd = FIELD_GET(IEEE80211_TWT_REQTYPE_SETUP_CMD, req_type);
	u8 exponent = FIELD_GET(IEEE80211_TWT_REQTYPE_WAKE_INT_EXP, req_type);
	u8 flowid = FIELD_GET(IEEE80211_TWT_REQTYPE_FLOWID, req_type);
	u16 mantissa = le16_to_cpu(params->mantissa);
	u64 interval_usec;
	u64 wake_usec;
	unsigned long first_delay;

	if (setup_cmd != TWT_SETUP_CMD_ACCEPT || !mantissa ||
	    !params->min_twt_dur)
		return;

	if (exponent >= 32)
		interval_usec = (u64)UINT_MAX * 1000ULL;
	else
		interval_usec = (u64)mantissa << exponent;

	wake_usec = (u64)params->min_twt_dur *
		    ((twt->control & IEEE80211_TWT_CONTROL_WAKE_DUR_UNIT) ?
		     1024ULL : 256ULL);

	hwsim_twt_disable_auto(data, false, "reinstall");

	data->twt_auto_enabled = true;
	data->twt_restore_ps = data->ps == PS_DISABLED;
	data->twt_flowid = flowid;
	data->twt_dialog_token = twt->dialog_token;
	data->twt_interval_jiffies =
		hwsim_twt_usec_to_jiffies(interval_usec);
	data->twt_wake_jiffies = hwsim_twt_usec_to_jiffies(wake_usec);
	data->twt_wake_jiffies =
		max_t(unsigned long, data->twt_wake_jiffies,
		      hwsim_twt_msecs_to_jiffies(NUTTX_HWSIM_TWT_MIN_WAKE_MS));
	data->twt_interval_jiffies =
		max_t(unsigned long, data->twt_interval_jiffies,
		      hwsim_twt_msecs_to_jiffies(NUTTX_HWSIM_TWT_MIN_INTERVAL_MS));
	data->twt_interval_jiffies =
		max_t(unsigned long, data->twt_interval_jiffies,
		      data->twt_wake_jiffies + 1);

	cancel_delayed_work_sync(&data->auto_ps_poll);
	cancel_delayed_work_sync(&data->dyn_ps_idle);
	data->ps = PS_ENABLED;
	data->ps_poll_pending = false;
	data->ps_poll_budget = 0;
	data->dynps_sleeping = false;

	hwsim_twt_set_sp(data, false, true, "install");
	first_delay = data->twt_interval_jiffies > data->twt_wake_jiffies ?
		      data->twt_interval_jiffies - data->twt_wake_jiffies :
		      data->twt_interval_jiffies;
	nuttx_hwsim_ps_prooff("twt-proof: hwsim auto installed dialog=%u flow=%u exponent=%u mantissa=%u min_twt=%u interval_jiffies=%lu wake_jiffies=%lu first_delay=%lu\n",
			      data->twt_dialog_token, data->twt_flowid,
			      exponent, mantissa, params->min_twt_dur,
			      data->twt_interval_jiffies,
			      data->twt_wake_jiffies, first_delay);
	ieee80211_queue_delayed_work(data->hw, &data->twt_sp,
				     max_t(unsigned long, 1, first_delay));
}

static void hwsim_auto_ps_poll_work(struct work_struct *work)
{
	struct mac80211_hwsim_data *data =
		container_of(work, struct mac80211_hwsim_data,
			     auto_ps_poll.work);
	unsigned long delay = max_t(unsigned long, 1,
				    msecs_to_jiffies(NUTTX_HWSIM_AUTO_PS_POLL_MS));
	static unsigned int auto_poll_logs;

	if (!data->started || data->ps != PS_AUTO_POLL)
		return;

	data->ps_poll_pending = true;
	data->ps_poll_budget = NUTTX_HWSIM_AUTO_PS_RX_BUDGET;
	if (auto_poll_logs < 8 || auto_poll_logs % 128 == 0) {
		nuttx_hwsim_ps_prooff("ps-proof: hwsim auto poll tx budget=%u release=%u interval_ms=%u sample=%u\n",
				      data->ps_poll_budget,
				      NUTTX_HWSIM_AUTO_PS_RELEASE_FRAMES,
				      NUTTX_HWSIM_AUTO_PS_POLL_MS,
				      auto_poll_logs);
	}
	auto_poll_logs++;
	local_bh_disable();
	...
	ieee80211_queue_delayed_work(data->hw, &data->auto_ps_poll, delay);
}
...
static int hwsim_fops_ps_write(void *dat, u64 val)
{
	struct mac80211_hwsim_data *data = dat;
	enum ps_mode old_ps;
	bool old_dynps_sleeping;

	if (val != PS_DISABLED && val != PS_ENABLED && val != PS_AUTO_POLL &&
	    val != PS_MANUAL_POLL && val != PS_DYNAMIC_IDLE)
		return -EINVAL;

	if (data->twt_auto_enabled && val != PS_ENABLED)
		hwsim_twt_disable_auto(data, false, "ps-change");

	if (val == PS_MANUAL_POLL) {
		if (data->ps != PS_ENABLED)
			return -EINVAL;
		cancel_delayed_work_sync(&data->auto_ps_poll);
		data->ps = PS_MANUAL_POLL;
		data->ps_poll_pending = true;
		nuttx_hwsim_ps_prooff("ps-proof: hwsim manual poll armed\n");
		local_bh_disable();
		ieee80211_iterate_active_interfaces_atomic(
			data->hw, IEEE80211_IFACE_ITER_NORMAL,
			hwsim_send_ps_poll, data);
	...
	old_ps = data->ps;
	old_dynps_sleeping = data->dynps_sleeping;
	data->ps = val;
	data->ps_poll_pending = false;
	data->ps_poll_budget = 0;
	if (val == PS_DISABLED) {
	...
	local_bh_disable();
	if (val == PS_DYNAMIC_IDLE && old_ps != PS_DYNAMIC_IDLE) {
		ieee80211_iterate_active_interfaces_atomic(
			data->hw, IEEE80211_IFACE_ITER_NORMAL,
			hwsim_send_nullfunc_no_ps, data);
		...
		ieee80211_iterate_active_interfaces_atomic(
			data->hw, IEEE80211_IFACE_ITER_NORMAL,
			hwsim_send_nullfunc_no_ps, data);
		} else if (old_ps == PS_DYNAMIC_IDLE && old_dynps_sleeping &&
			   val != PS_DYNAMIC_IDLE) {
			ieee80211_iterate_active_interfaces_atomic(
				data->hw, IEEE80211_IFACE_ITER_NORMAL,
				hwsim_send_nullfunc_ps, data);
	...
	if (val == PS_AUTO_POLL) {
		...
		nuttx_hwsim_ps_prooff("ps-proof: hwsim auto poll armed\n");
		ieee80211_queue_delayed_work(
			data->hw, &data->auto_ps_poll, delay);
	} else if (val == PS_DYNAMIC_IDLE) {
		cfg80211_compat_refresh_jiffies();
		data->dynps_sleeping = false;
		data->dynps_idle_deadline =
			jiffies + msecs_to_jiffies(NUTTX_HWSIM_DYNPS_IDLE_MS);
		nuttx_hwsim_ps_prooff("ps-proof: hwsim dynps armed timeout_ms=%u\n",
				      NUTTX_HWSIM_DYNPS_IDLE_MS);
		ieee80211_queue_delayed_work(
			data->hw, &data->dyn_ps_idle,
			msecs_to_jiffies(NUTTX_HWSIM_DYNPS_IDLE_MS));
		} else {
			cancel_delayed_work_sync(&data->auto_ps_poll);
		}

	if (val != PS_DYNAMIC_IDLE)
		cancel_delayed_work_sync(&data->dyn_ps_idle);
				     ...
				     struct ieee80211_vif *vif)
{
	...
	if (memcmp(vif->addr, md->addr, ETH_ALEN) == 0) {
		md->ret = true;
		return;
	}

	if (md->include_bss && !is_zero_ether_addr(vif->cfg.ap_addr) &&
	    memcmp(vif->cfg.ap_addr, md->addr, ETH_ALEN) == 0) {
		md->ret = true;
		return;
	}

	if (md->include_bss && vif->bss_conf.bssid &&
	    memcmp(vif->bss_conf.bssid, md->addr, ETH_ALEN) == 0) {
		md->ret = true;
		return;
	}
	...
	for (i = 0; i < ARRAY_SIZE(vif->link_conf); i++) {
		...
		if (md->include_bss && conf->bssid &&
		    memcmp(conf->bssid, md->addr, ETH_ALEN) == 0) {
			md->ret = true;
			return;
		}
...
static bool mac80211_hwsim_rx_addr_match(struct mac80211_hwsim_data *data,
					  const u8 *addr)
{
	struct mac80211_hwsim_addr_match_data md = {
		.ret = false,
...
static bool mac80211_hwsim_own_addr_match(struct mac80211_hwsim_data *data,
					  const u8 *addr)
{
	int i;
	...
	for (i = 0; i < ARRAY_SIZE(data->addresses); i++) {
		if (memcmp(data->addresses[i].addr, addr, ETH_ALEN) == 0)
			return true;
	}
...
static bool hwsim_own_twt_action(struct mac80211_hwsim_data *data,
				 struct sk_buff *skb)
{
	const struct ieee80211_mgmt *mgmt = (const void *)skb->data;
	static unsigned int twt_action_logs;

	if (skb->len >= IEEE80211_MIN_ACTION_SIZE + 1 &&
	    ieee80211_is_action(mgmt->frame_control) &&
	    (mgmt->u.action.category == WLAN_CATEGORY_S1G ||
	     mgmt->u.action.category == NUTTX_HWSIM_TWT_ACTION_CATEGORY) &&
	    twt_action_logs < 32) {
		nuttx_hwsim_ps_prooff("twt-proof: hwsim action probe cat=%u action=%u own=%d da=%02x:%02x:%02x:%02x:%02x:%02x len=%u\n",
				      mgmt->u.action.category,
				      mgmt->u.action.u.s1g.action_code,
				      mac80211_hwsim_own_addr_match(data, mgmt->da),
	...
	return mac80211_hwsim_own_addr_match(data, mgmt->da);
}

static void hwsim_handle_twt_action(struct mac80211_hwsim_data *data,
				    struct sk_buff *skb)
{
	struct ieee80211_mgmt *mgmt = (void *)skb->data;
	struct ieee80211_twt_setup *twt;

	if (!hwsim_own_twt_action(data, skb))
		return;

	switch (mgmt->u.action.u.s1g.action_code) {
	case WLAN_S1G_TWT_SETUP:
		if (skb->len < IEEE80211_MIN_ACTION_SIZE + 1 +
			       sizeof(*twt) + sizeof(struct ieee80211_twt_params))
			return;

		twt = (void *)mgmt->u.action.u.s1g.variable;
		if (twt->element_id != WLAN_EID_S1G_TWT ||
		    twt->length < sizeof(struct ieee80211_twt_params) + 1)
			return;

		hwsim_twt_install_auto(data, twt);
		break;
	case WLAN_S1G_TWT_TEARDOWN:
		hwsim_twt_disable_auto(data, true, "teardown");
		break;
	default:
		break;
	}
...
static void hwsim_handle_tx_twt_action(struct mac80211_hwsim_data *data,
				       struct sk_buff *skb)
{
	struct ieee80211_mgmt *mgmt = (void *)skb->data;
	...
	if (skb->len < IEEE80211_MIN_ACTION_SIZE + 1 ||
	    ...
	    (mgmt->u.action.category != WLAN_CATEGORY_S1G &&
	     mgmt->u.action.category != NUTTX_HWSIM_TWT_ACTION_CATEGORY) ||
	    mgmt->u.action.u.s1g.action_code != WLAN_S1G_TWT_TEARDOWN ||
	    !mac80211_hwsim_own_addr_match(data, mgmt->sa))
		return;

	hwsim_twt_disable_auto(data, true, "tx-teardown");
}
			   ...
			   struct sk_buff *skb)
{
	...
	case PS_ENABLED:
		...
		own_addr = skb->len >= 10 &&
			   mac80211_hwsim_own_addr_match(data, hdr->addr1);
		sp_rx_addr = own_addr || is_multicast_ether_addr(hdr->addr1);
		if (data->twt_sp_open && sp_rx_addr) {
			nuttx_hwsim_ps_prooff("twt-proof: hwsim service period rx len=%u\n",
					      skb->len);
			return true;
		}

		nuttx_hwsim_ps_prooff("twt-proof: hwsim service period reject open=%d own=%d mcast=%d len=%u\n",
				      data->twt_sp_open, own_addr,
	...
	case PS_AUTO_POLL:
		...
		own_addr = skb->len >= 10 &&
			   mac80211_hwsim_own_addr_match(data, hdr->addr1);
		if (data->ps_poll_pending && own_addr &&
		    data->ps_poll_budget > 0) {
			data->ps_poll_budget--;
			if (data->ps_poll_budget == 0)
				data->ps_poll_pending = false;
			if (data->ps_poll_budget == 0 ||
			    data->ps_poll_budget < 8 ||
			    data->ps_poll_budget % NUTTX_HWSIM_AUTO_PS_RX_LOG_GRAN == 0)
				nuttx_hwsim_ps_prooff("ps-proof: hwsim auto poll rx budget=%u\n",
						      data->ps_poll_budget);
			return true;
		}

		nuttx_hwsim_ps_prooff("ps-proof: hwsim auto poll reject pending=%d own=%d budget=%u len=%u\n",
				      data->ps_poll_pending, own_addr,
				      data->ps_poll_budget, skb->len);
		return false;
	case PS_DYNAMIC_IDLE:
		if (!data->dynps_sleeping)
			return true;
		...
		own_addr = skb->len >= 10 &&
			   mac80211_hwsim_own_addr_match(data, hdr->addr1);
		nuttx_hwsim_ps_prooff("ps-proof: hwsim dynps reject sleeping=%d own=%d len=%u\n",
				      data->dynps_sleeping, own_addr, skb->len);
		return false;
	case PS_MANUAL_POLL:
		...
		if (data->ps_poll_pending && own_addr) {
			data->ps_poll_pending = false;
			data->ps = PS_ENABLED;
			nuttx_hwsim_ps_prooff("ps-proof: hwsim manual poll rx one\n");
			return true;
		}
		nuttx_hwsim_ps_prooff("ps-proof: hwsim manual poll reject pending=%d own=%d len=%u\n",
				      data->ps_poll_pending, own_addr, skb->len);
		return false;
	}
				       ...
				       struct ieee80211_channel *channel)
{
	...
	hwsim_dyn_ps_tx_activity(data, my_skb);
	if (hwsim_tx_should_force_pm(data))
...
static void
mac80211_hwsim_ap_link_ps_rx(struct mac80211_hwsim_data *data,
			     struct sk_buff *skb)
{
	...
	static int no_sta_logs;
	bool start;
	int ret;

	if (!ieee80211_hw_check(data->hw, AP_LINK_PS))
		return;
	...
	rcu_read_lock();
		...
		if (skb->len >= sizeof(struct ieee80211_pspoll) &&
			...
			struct ieee80211_pspoll *pspoll = (void *)skb->data;
			bool auto_poll = ieee80211_has_retry(hdr->frame_control);

			sta = ieee80211_find_sta_by_link_addrs(data->hw,
							       pspoll->ta,
						       pspoll->bssid,
						       &link_id);
		if (!sta) {
			sta = ieee80211_find_sta_by_ifaddr(data->hw,
							   pspoll->ta,
							   pspoll->bssid);
			link_id = 0;
		}
			if (sta) {
				static unsigned int auto_pspoll_logs;

				if (!auto_poll || auto_pspoll_logs < 8 ||
				    auto_pspoll_logs % 128 == 0) {
					nuttx_hwsim_ps_prooff("ap-link-ps-proof: pspoll sta="
							      NUTTX_HWSIM_MAC_FMT
							      " link=%u auto=%d sample=%u\n",
							      NUTTX_HWSIM_MAC_ARG(sta->addr),
							      link_id, auto_poll ? 1 : 0,
							      auto_pspoll_logs);
				}
				if (auto_poll)
					auto_pspoll_logs++;
	...
	sta = ieee80211_find_sta_by_link_addrs(data->hw, hdr->addr2,
					       hdr->addr1, &link_id);
	if (!sta) {
		sta = ieee80211_find_sta_by_ifaddr(data->hw, hdr->addr2,
						   hdr->addr1);
		link_id = 0;
	}
	if (!sta) {
		if (no_sta_logs < 16) {
			nuttx_hwsim_ps_prooff("ap-link-ps-proof: no-sta fc=0x%04x "
					      "sta=" NUTTX_HWSIM_MAC_FMT " "
					      "local=" NUTTX_HWSIM_MAC_FMT "\n",
					      le16_to_cpu(hdr->frame_control),
					      NUTTX_HWSIM_MAC_ARG(hdr->addr2),
					      NUTTX_HWSIM_MAC_ARG(hdr->addr1));
			no_sta_logs++;
		}
		rcu_read_unlock();
		return;
	...
	start = ieee80211_has_pm(hdr->frame_control);
	if (sp->ap_link_ps == start) {
		rcu_read_unlock();
		return;
	}

	ret = ieee80211_sta_ps_transition_ni(sta, start);
	if (ret == 0)
		sp->ap_link_ps = start;

	nuttx_hwsim_ps_prooff("ap-link-ps-proof: transition sta="
			      NUTTX_HWSIM_MAC_FMT " link=%u start=%d ret=%d\n",
			      NUTTX_HWSIM_MAC_ARG(sta->addr), link_id,
			      start ? 1 : 0, ret);
			      ...
			      struct sk_buff *skb)
{
	...
	mac80211_hwsim_ap_link_ps_rx(data, skb);
...
struct nuttx_hwsim_medium_bss {
	...
	u32 band;
	s32 signal;
	u8 frame[NUTTX_HWSIM_MEDIUM_MAX_FRAME];
};

struct nuttx_hwsim_medium_frame {
	u32 magic;
	u16 version;
	u16 len;
	u32 freq;
...
static bool nuttx_hwsim_medium_lock(void);
static void nuttx_hwsim_medium_unlock(void);
static void
nuttx_hwsim_apply_s1g_rx_status(struct ieee80211_rx_status *status,
				const char *path);

static void nuttx_hwsim_medium_publish_bss(struct ieee80211_rx_status *status,
					   struct sk_buff *skb)
{
	struct ieee80211_hdr *hdr = (void *)skb->data;
	struct ieee80211_mgmt *new_mgmt;
	struct ieee80211_mgmt *old_mgmt;
	struct nuttx_hwsim_medium_bss *records;
	struct nuttx_hwsim_medium_bss *record;
	static int publish_logs;
	bool updated = false;
	size_t max_records;
	size_t nr_records = 0;
	size_t i;
	...
	if (skb->len < 24 || skb->len > NUTTX_HWSIM_MEDIUM_MAX_FRAME)
		return;
	...
	record = kmm_zalloc(sizeof(*record));
	if (!record)
		return;

	record->magic = NUTTX_HWSIM_MEDIUM_MAGIC;
	record->version = NUTTX_HWSIM_MEDIUM_VERSION;
	record->len = skb->len;
	record->freq = status->freq;
	record->freq_offset = status->freq_offset;
	record->band = status->band;
	record->signal = status->signal;
	memcpy(record->frame, skb->data, skb->len);

	records = kmm_zalloc(sizeof(*records) *
			     NUTTX_HWSIM_MEDIUM_MAX_BSS_RECORDS);
	if (!records) {
		kmm_free(record);
		return;
	}

	if (!nuttx_hwsim_medium_lock()) {
		kmm_free(records);
		kmm_free(record);
		return;
	}

	max_records = NUTTX_HWSIM_MEDIUM_MAX_BSS_RECORDS;
	fd = open(NUTTX_HWSIM_MEDIUM_BSS_PATH, O_RDWR | O_CREAT, 0666);
	if (fd < 0) {
		nuttx_hwsim_medium_unlock();
		kmm_free(records);
		kmm_free(record);
		return;
	}

	nread = read(fd, records, sizeof(*records) * max_records);
	if (nread > 0)
		nr_records = nread / sizeof(*records);

	if (nr_records > max_records)
		nr_records = max_records;

	new_mgmt = (struct ieee80211_mgmt *)record->frame;
	for (i = 0; i < nr_records; i++) {
		if (records[i].magic != NUTTX_HWSIM_MEDIUM_MAGIC ||
		    records[i].version != NUTTX_HWSIM_MEDIUM_VERSION ||
		    records[i].len < 24 ||
		    records[i].len > NUTTX_HWSIM_MEDIUM_MAX_FRAME)
			continue;

		old_mgmt = (struct ieee80211_mgmt *)records[i].frame;
		if (ether_addr_equal(old_mgmt->bssid, new_mgmt->bssid)) {
	...
	if (!updated) {
		if (nr_records < max_records) {
			memcpy(&records[nr_records], record, sizeof(*record));
			nr_records++;
		} else {
			memmove(&records[0], &records[1],
				sizeof(*records) * (max_records - 1));
			memcpy(&records[max_records - 1], record,
			       sizeof(*record));
			nr_records = max_records;
		}
	}
	...
	nwrite = write(fd, records, sizeof(*records) * nr_records);
	close(fd);
	nuttx_hwsim_medium_unlock();

	if (nwrite == sizeof(*records) * nr_records && publish_logs < 48) {
		nuttx_hwsim_debugf("hwsim-debug: published BSS frame freq=%u len=%u records=%zu src=%02x:%02x:%02x:%02x:%02x:%02x bssid=%02x:%02x:%02x:%02x:%02x:%02x path=%s\n",
		       record->freq, record->len,
		       nr_records,
		       hdr->addr2[0], hdr->addr2[1], hdr->addr2[2],
		       hdr->addr2[3], hdr->addr2[4], hdr->addr2[5],
		       hdr->addr3[0], hdr->addr3[1], hdr->addr3[2],
		       hdr->addr3[3], hdr->addr3[4], hdr->addr3[5],
		       NUTTX_HWSIM_MEDIUM_BSS_PATH);
		publish_logs++;
	}
...
static void nuttx_hwsim_medium_inject_scan_bss(struct mac80211_hwsim_data *data)
{
	struct nuttx_hwsim_medium_bss *records;
	struct ieee80211_rx_status rx_status;
	struct cfg80211_inform_bss inform = {};
	struct cfg80211_bss *bss;
	struct sk_buff *skb;
	struct ieee80211_mgmt *mgmt;
	static const u8 no_bssid[ETH_ALEN];
	const u8 *req_bssid = NULL;
	static int inject_logs;
	static int inform_logs;
	...
	fd = open(NUTTX_HWSIM_MEDIUM_BSS_PATH, O_RDONLY);
	if (fd < 0) {
		kmm_free(records);
		return;
	}
	...
	if (data->hw_scan_request)
		req_bssid = data->hw_scan_request->bssid;

	for (i = 0; i < nr_records; i++) {
		struct nuttx_hwsim_medium_bss *record = &records[i];

		if (record->magic != NUTTX_HWSIM_MEDIUM_MAGIC ||
		    record->version != NUTTX_HWSIM_MEDIUM_VERSION ||
		    record->len < 24 ||
		    record->len > NUTTX_HWSIM_MEDIUM_MAX_FRAME ||
		    record->freq != data->tmp_chan->center_freq)
			continue;

		mgmt = (struct ieee80211_mgmt *)record->frame;
		if (req_bssid && !is_zero_ether_addr(req_bssid) &&
		    !is_broadcast_ether_addr(req_bssid) &&
		    !ether_addr_equal(mgmt->bssid, req_bssid)) {
			if (skip_logs < 24) {
				nuttx_hwsim_debugf("hwsim-debug: skipped shared BSS scan_req_bssid=%02x:%02x:%02x:%02x:%02x:%02x frame_bssid=%02x:%02x:%02x:%02x:%02x:%02x src=%02x:%02x:%02x:%02x:%02x:%02x freq=%u len=%u\n",
				       req_bssid[0], req_bssid[1],
				       req_bssid[2], req_bssid[3],
				       req_bssid[4], req_bssid[5],
			...
			skb = dev_alloc_skb(record->len);
			if (!skb)
				continue;

			skb_put_data(skb, record->frame, record->len);
			if (!hwsim_ps_rx_ok(data, skb)) {
				dev_kfree_skb(skb);
				continue;
			}
			hwsim_handle_twt_action(data, skb);

			memset(&rx_status, 0, sizeof(rx_status));
		rx_status.flag |= RX_FLAG_MACTIME_START;
		rx_status.freq = record->freq;
		rx_status.freq_offset = record->freq_offset;
		rx_status.band = record->band;
		rx_status.signal = record->signal;
		rx_status.boottime_ns = ktime_get_boottime_ns();
		rx_status.mactime = mac80211_hwsim_get_tsf_raw() +
				     data->tsf_offset;
		nuttx_hwsim_apply_s1g_rx_status(&rx_status, "shared-bss");

		memset(&inform, 0, sizeof(inform));
		inform.chan = data->tmp_chan;
		inform.boottime_ns = rx_status.boottime_ns;
		inform.signal = record->signal * 100;
		bss = cfg80211_inform_bss_frame_data(data->hw->wiphy, &inform,
						(struct ieee80211_mgmt *)record->frame,
						record->len, GFP_ATOMIC);
		if (bss) {
			const u8 *print_req_bssid = req_bssid ? req_bssid :
						    no_bssid;

			(void)print_req_bssid;
			if (inform_logs < 48) {
				nuttx_hwsim_debugf("hwsim-debug: informed shared BSS freq=%u len=%u src=%02x:%02x:%02x:%02x:%02x:%02x bssid=%02x:%02x:%02x:%02x:%02x:%02x req_bssid=%02x:%02x:%02x:%02x:%02x:%02x\n",
				       record->freq, record->len,
				       mgmt->sa[0], mgmt->sa[1],
				       mgmt->sa[2], mgmt->sa[3],
				       mgmt->sa[4], mgmt->sa[5],
			...
			cfg80211_put_bss(data->hw->wiphy, bss);
		}

		if (inject_logs < 12) {
			nuttx_hwsim_debugf("hwsim-debug: injected shared BSS frame freq=%u len=%u record=%zu/%zu path=%s\n",
			       record->freq, record->len, i + 1, nr_records,
			       NUTTX_HWSIM_MEDIUM_BSS_PATH);
			inject_logs++;
		}

		mac80211_hwsim_rx(data, &rx_status, skb);
	}
...
static bool
nuttx_hwsim_medium_channel_match(struct mac80211_hwsim_data *data, u32 freq)
{
	...
	if (data->tmp_chan && data->tmp_chan->center_freq == freq)
		return true;

	if (data->channel && data->channel->center_freq == freq)
		return true;

	if (data->medium_listen_chan &&
	    data->medium_listen_chan->center_freq == freq &&
	    time_before(jiffies, data->medium_listen_until))
		return true;
	...
	tx_iter_data.receive = false;
	tx_iter_data.channel = channel;
	ieee80211_iterate_active_interfaces_atomic(data->hw,
						   IEEE80211_IFACE_ITER_NORMAL,
						   mac80211_hwsim_tx_iter,
						   &tx_iter_data);

	return tx_iter_data.receive;
}

static off_t nuttx_hwsim_medium_frame_eof(void)
{
	off_t end;
	int fd;

	fd = open(NUTTX_HWSIM_MEDIUM_FRAME_PATH, O_RDONLY);
	if (fd < 0)
		return 0;
				...
				const char *path)
{
	...
	if (status->band != NL80211_BAND_S1GHZ ||
	...
	status->rate_idx = 0;
	status->nss = 1;
	status->bw = RATE_INFO_BW_1;

	if (s1g_logs < 32) {
		nuttx_hwsim_debugf("hwsim-debug: s1g rx status path=%s "
				   "freq=%u encoding=VHT-as-S1G mcs=%u "
				   "nss=%u bw=%u\n",
				   path, status->freq, status->rate_idx,
				   status->nss, status->bw);
		s1g_logs++;
	}
}

static void
nuttx_hwsim_apply_dcm_rx_status(struct ieee80211_rx_status *status,
				...
				const char *path)
{
	...
	if (access(NUTTX_HWSIM_DCM_PROOF_PATH, F_OK) < 0)
		return;
	...
	status->he_gi = NL80211_RATE_INFO_HE_GI_0_8;
	status->he_dcm = 1;

	if (dcm_logs < 32) {
		nuttx_hwsim_dcm_prooff("dcm-proof: rx status path=%s "
				       "freq=%u bw=%u encoding=HE mcs=%u "
				       "nss=%u he_gi=%u he_dcm=%u "
				       "src=" NUTTX_HWSIM_MAC_FMT " "
				       "dst=" NUTTX_HWSIM_MAC_FMT "\n",
				       path, status->freq, status->bw,
				       status->rate_idx, status->nss,
				       status->he_gi, status->he_dcm,
				       NUTTX_HWSIM_MAC_ARG(hdr->addr2),
				       NUTTX_HWSIM_MAC_ARG(hdr->addr1));
		dcm_logs++;
	}
...
static bool nuttx_hwsim_medium_lock(void)
{
	...
	for (i = 0; i < NUTTX_HWSIM_MEDIUM_LOCK_RETRIES; i++) {
		int fd;

		fd = open(NUTTX_HWSIM_MEDIUM_FRAME_LOCK_PATH,
...
static void nuttx_hwsim_medium_unlock(void)
{
	unlink(NUTTX_HWSIM_MEDIUM_FRAME_LOCK_PATH);
}

static bool
nuttx_hwsim_medium_publish_frame(struct ieee80211_rx_status *status,
				 struct sk_buff *skb)
{
	struct ieee80211_hdr *hdr = (void *)skb->data;
	struct ieee80211_pspoll *pspoll = (void *)skb->data;
	struct nuttx_hwsim_medium_frame *record;
	static int publish_logs;
	static int publish_auth_logs;
	static int publish_data_logs;
	static int truncate_logs;
	off_t end;
	u16 fc;
	int fd;
	ssize_t nwrite;
	...
	if (skb->len < sizeof(hdr->frame_control) ||
	    skb->len > NUTTX_HWSIM_MEDIUM_MAX_FRAME)
		return false;

	fc = le16_to_cpu(hdr->frame_control);

	if (ieee80211_is_pspoll(hdr->frame_control)) {
		if (skb->len < sizeof(*pspoll))
			return false;
	} else if (skb->len < 24) {
		return false;
	}
	...
	record->signal = status->signal;
	if (ieee80211_is_pspoll(hdr->frame_control)) {
		memcpy(record->src, pspoll->ta, ETH_ALEN);
		memcpy(record->dst, pspoll->bssid, ETH_ALEN);
		memcpy(record->bssid, pspoll->bssid, ETH_ALEN);
	} else {
		memcpy(record->src, hdr->addr2, ETH_ALEN);
		memcpy(record->dst, hdr->addr1, ETH_ALEN);
		memcpy(record->bssid, hdr->addr3, ETH_ALEN);
	}
	memcpy(record->frame, skb->data, skb->len);
	...
	fd = open(NUTTX_HWSIM_MEDIUM_FRAME_PATH,
	...
	if (end > NUTTX_HWSIM_MEDIUM_MAX_BYTES) {
		if (truncate_logs < 8) {
			nuttx_hwsim_debugf("hwsim-debug: truncating shared frame medium size=%ld limit=%lu path=%s\n",
			       (long)end,
			       (unsigned long)NUTTX_HWSIM_MEDIUM_MAX_BYTES,
			       NUTTX_HWSIM_MEDIUM_FRAME_PATH);
			truncate_logs++;
		}
	...
	nwrite = write(fd, record, sizeof(*record));
	close(fd);
	nuttx_hwsim_medium_unlock();

	if (nwrite == sizeof(*record) && publish_logs < 12) {
		nuttx_hwsim_debugf("hwsim-debug: published shared frame fc=0x%04x freq=%u len=%u src=%02x:%02x:%02x:%02x:%02x:%02x dst=%02x:%02x:%02x:%02x:%02x:%02x path=%s\n",
		       fc, record->freq,
		       record->len,
		       record->src[0], record->src[1], record->src[2],
		       record->src[3], record->src[4], record->src[5],
		       record->dst[0], record->dst[1], record->dst[2],
		       record->dst[3], record->dst[4], record->dst[5],
		       NUTTX_HWSIM_MEDIUM_FRAME_PATH);
		publish_logs++;
	}
	...
	if (nwrite == sizeof(*record) &&
	    ...
	    publish_auth_logs < 24) {
		nuttx_hwsim_debugf("hwsim-debug: published shared auth fc=0x%04x freq=%u len=%u src=%02x:%02x:%02x:%02x:%02x:%02x dst=%02x:%02x:%02x:%02x:%02x:%02x bssid=%02x:%02x:%02x:%02x:%02x:%02x path=%s\n",
		       fc, record->freq, record->len,
		       record->src[0], record->src[1], record->src[2],
		       record->src[3], record->src[4], record->src[5],
		       record->dst[0], record->dst[1], record->dst[2],
		       record->dst[3], record->dst[4], record->dst[5],
		       record->bssid[0], record->bssid[1],
		       record->bssid[2], record->bssid[3],
		       record->bssid[4], record->bssid[5],
		       NUTTX_HWSIM_MEDIUM_FRAME_PATH);
		publish_auth_logs++;
	}

	if (nwrite == sizeof(*record) && ieee80211_is_data(hdr->frame_control) &&
	    publish_data_logs < 24) {
		nuttx_hwsim_debugf("hwsim-debug: published shared data fc=0x%04x freq=%u len=%u src=%02x:%02x:%02x:%02x:%02x:%02x dst=%02x:%02x:%02x:%02x:%02x:%02x bssid=%02x:%02x:%02x:%02x:%02x:%02x path=%s\n",
		       fc, record->freq, record->len,
		       record->src[0], record->src[1], record->src[2],
		       record->src[3], record->src[4], record->src[5],
		       record->dst[0], record->dst[1], record->dst[2],
	...
	kmm_free(record);
	return nwrite == sizeof(*record);
}

static bool
nuttx_hwsim_medium_should_rx_frame(struct mac80211_hwsim_data *data,
				   struct nuttx_hwsim_medium_frame *record)
{
	struct ieee80211_hdr *hdr = (void *)record->frame;
	u16 fc = le16_to_cpu(hdr->frame_control);
	bool is_pspoll;
	bool src_local;
	bool dst_local;
	bool dst_mcast;
	static int auth_drop_logs;
	...
	if (record->len < sizeof(hdr->frame_control))
		return false;

	is_pspoll = ieee80211_is_pspoll(hdr->frame_control);
	if (is_pspoll) {
		if (record->len < sizeof(struct ieee80211_pspoll))
			return false;
	} else if (record->len < 24) {
		return false;
	}

	src_local = mac80211_hwsim_addr_match(data, record->src);
	dst_local = mac80211_hwsim_rx_addr_match(data, record->dst);
	dst_mcast = is_multicast_ether_addr(record->dst);

	if (!nuttx_hwsim_medium_channel_match(data, record->freq))
	  {
		if ((fc & IEEE80211_FCTL_STYPE) == IEEE80211_STYPE_AUTH &&
		    auth_drop_logs < 24) {
			nuttx_hwsim_debugf("hwsim-debug: drop shared auth reason=channel freq=%u chan=%u tmp=%u idle=%d scanning=%d src_local=%d dst_local=%d dst_mcast=%d src=%02x:%02x:%02x:%02x:%02x:%02x dst=%02x:%02x:%02x:%02x:%02x:%02x\n",
			       record->freq,
			       data->channel ? data->channel->center_freq : 0,
			       data->tmp_chan ? data->tmp_chan->center_freq : 0,
			       data->idle, data->scanning, src_local, dst_local,
			       dst_mcast,
			       record->src[0], record->src[1], record->src[2],
			       record->src[3], record->src[4], record->src[5],
			       record->dst[0], record->dst[1], record->dst[2],
	...
	if (!dst_local &&
	    (fc & IEEE80211_FCTL_STYPE) == IEEE80211_STYPE_AUTH &&
	    auth_drop_logs < 24) {
		nuttx_hwsim_debugf("hwsim-debug: drop shared auth reason=addr freq=%u chan=%u tmp=%u idle=%d scanning=%d src_local=%d dst_local=%d src=%02x:%02x:%02x:%02x:%02x:%02x dst=%02x:%02x:%02x:%02x:%02x:%02x\n",
		       record->freq,
		       data->channel ? data->channel->center_freq : 0,
		       data->tmp_chan ? data->tmp_chan->center_freq : 0,
		       data->idle, data->scanning, src_local, dst_local,
...
static void
nuttx_hwsim_medium_consume_frames(struct mac80211_hwsim_data *data)
{
	struct nuttx_hwsim_medium_frame *record;
	static int inject_logs;
	static int inject_data_logs;
	...
	if (data->medium_frame_offset > end)
		data->medium_frame_offset = 0;

	if (lseek(fd, data->medium_frame_offset, SEEK_SET) < 0) {
		close(fd);
		kmm_free(record);
		return;
	}
	...
	for (;;) {
		...
		if (records++ >= NUTTX_HWSIM_MEDIUM_MAX_RECORDS_PER_POLL)
			break;
		...
		data->medium_frame_offset += sizeof(*record);

		if ((le16_to_cpu(((struct ieee80211_hdr *)record->frame)->frame_control) &
		     IEEE80211_FCTL_STYPE) == IEEE80211_STYPE_AUTH) {
			static int auth_read_logs;

			if (auth_read_logs < 24) {
				nuttx_hwsim_debugf("hwsim-debug: read shared auth offset=%ld freq=%u chan=%u tmp=%u idle=%d scanning=%d src=%02x:%02x:%02x:%02x:%02x:%02x dst=%02x:%02x:%02x:%02x:%02x:%02x\n",
				       (long)data->medium_frame_offset,
				       record->freq,
				       data->channel ? data->channel->center_freq : 0,
				       data->tmp_chan ? data->tmp_chan->center_freq : 0,
				       data->idle, data->scanning,
				       record->src[0], record->src[1],
				       record->src[2], record->src[3],
				       record->src[4], record->src[5],
				       record->dst[0], record->dst[1],
				       record->dst[2], record->dst[3],
				       record->dst[4], record->dst[5]);
				auth_read_logs++;
			}
		}

			if (!nuttx_hwsim_medium_should_rx_frame(data, record))
				continue;
		...
		rx_status.mactime = mac80211_hwsim_get_tsf_raw() +
				     data->tsf_offset;
		nuttx_hwsim_apply_s1g_rx_status(&rx_status, "shared");
		nuttx_hwsim_apply_dcm_rx_status(&rx_status,
						(void *)record->frame,
						"shared");
		...
		if (inject_logs < 12) {
			...
			nuttx_hwsim_debugf("hwsim-debug: injected shared frame fc=0x%04x freq=%u len=%u src=%02x:%02x:%02x:%02x:%02x:%02x dst=%02x:%02x:%02x:%02x:%02x:%02x path=%s\n",
			       le16_to_cpu(hdr->frame_control), record->freq,
			       record->len,
			       record->src[0], record->src[1], record->src[2],
			       record->src[3], record->src[4], record->src[5],
			       record->dst[0], record->dst[1], record->dst[2],
		...
		if (ieee80211_is_data(((struct ieee80211_hdr *)record->frame)->frame_control) &&
		    inject_data_logs < 24) {
			struct ieee80211_hdr *hdr = (void *)record->frame;

			nuttx_hwsim_debugf("hwsim-debug: injected shared data fc=0x%04x freq=%u len=%u src=%02x:%02x:%02x:%02x:%02x:%02x dst=%02x:%02x:%02x:%02x:%02x:%02x bssid=%02x:%02x:%02x:%02x:%02x:%02x path=%s\n",
			       le16_to_cpu(hdr->frame_control), record->freq,
			       record->len,
			       record->src[0], record->src[1], record->src[2],
			       record->src[3], record->src[4], record->src[5],
...
static void nuttx_hwsim_medium_rx_work(struct work_struct *work)
{
	struct mac80211_hwsim_data *data =
		container_of(work, struct mac80211_hwsim_data, medium_rx.work);

	if (!data->started)
		return;

	nuttx_hwsim_medium_consume_frames(data);
	ieee80211_queue_delayed_work(data->hw, &data->medium_rx,
				     msecs_to_jiffies(NUTTX_HWSIM_MEDIUM_POLL_MS));
}
					  ...
					  struct ieee80211_channel *chan)
{
	...
	nuttx_hwsim_apply_s1g_rx_status(&rx_status, "direct");
	nuttx_hwsim_apply_dcm_rx_status(&rx_status, hdr, "direct");
	/* TODO: simulate optional packet loss */
	rx_status.signal = data->rx_rssi;
	if (info->control.vif)
		rx_status.signal += info->control.vif->bss_conf.txpower;

	hwsim_dyn_ps_tx_activity(data, skb);
	if (hwsim_tx_should_force_pm(data))
		hdr->frame_control |= cpu_to_le16(IEEE80211_FCTL_PM);
	...
	nuttx_hwsim_medium_publish_bss(&rx_status, skb);
	if (nuttx_hwsim_medium_publish_frame(&rx_status, skb) &&
	    !is_multicast_ether_addr(hdr->addr1))
		ack = true;
	if (ieee80211_is_mgmt(hdr->frame_control)) {
		data->medium_listen_chan = chan;
		data->medium_listen_until =
			jiffies + msecs_to_jiffies(NUTTX_HWSIM_MEDIUM_LISTEN_MS);
	}
	...
	list_for_each_entry(data2, &hwsim_radios, list) {
		...
		if (skb->len < PAGE_SIZE && paged_rx) {
			...
			hwsim_handle_twt_action(data2, nskb);
...
static bool nuttx_hwsim_debug_is_assoc_skb(struct sk_buff *skb)
{
	struct ieee80211_hdr *hdr;
			      ...
			      struct sk_buff *skb)
{
	...
	u32 _portid, i;
	bool debug_assoc = nuttx_hwsim_debug_is_assoc_skb(skb);
	...
	if (vif && vif->type == NL80211_IFTYPE_NAN && !data->tmp_chan) {
		...
		if (!vif) {
			if (debug_assoc)
				nuttx_hwsim_debugf("hwsim-debug: hwsim_tx assoc drop no-vif len=%u flags=0x%x hwq=%d\n",
				       skb->len, txi->flags, txi->hw_queue);
			ieee80211_free_txskb(hw, skb);
			return;
		}
		...
		if (unlikely(!bss_conf)) {
			...
			if (debug_assoc)
				nuttx_hwsim_debugf("hwsim-debug: hwsim_tx assoc drop no-bss-conf vif=%p sta=%p link=%u len=%u flags=0x%x hwq=%d\n",
				       vif, sta, link, skb->len, txi->flags,
				       txi->hw_queue);
			ieee80211_free_txskb(hw, skb);
			return;
		}
	...
	if (WARN(!channel, "TX w/o channel - queue = %d\n", txi->hw_queue)) {
		if (debug_assoc)
			nuttx_hwsim_debugf("hwsim-debug: hwsim_tx assoc drop no-channel vif=%p sta=%p tmp=%p data_chan=%p use_chanctx=%d len=%u flags=0x%x hwq=%d\n",
			       vif, control->sta, data->tmp_chan, data->channel,
			       data->use_chanctx, skb->len, txi->flags,
			       txi->hw_queue);
		ieee80211_free_txskb(hw, skb);
		return;
	}
	...
	if (data->idle && !data->tmp_chan) {
		if (debug_assoc)
			nuttx_hwsim_debugf("hwsim-debug: hwsim_tx assoc drop idle chan=%u len=%u flags=0x%x hwq=%d\n",
			       channel->center_freq, skb->len, txi->flags,
			       txi->hw_queue);
	...
	if (debug_assoc) {
		nuttx_hwsim_debugf("hwsim-debug: hwsim_tx assoc enter vif=%p sta=%p chan=%u len=%u flags=0x%x hwq=%d src=%02x:%02x:%02x:%02x:%02x:%02x dst=%02x:%02x:%02x:%02x:%02x:%02x\n",
		       vif, control->sta, channel->center_freq, skb->len,
		       txi->flags, txi->hw_queue,
		       hdr->addr2[0], hdr->addr2[1], hdr->addr2[2],
		       hdr->addr2[3], hdr->addr2[4], hdr->addr2[5],
	...
	hwsim_handle_tx_twt_action(data, skb);
	...
	for (i = 0; i < ARRAY_SIZE(txi->control.rates); i++) {
		...
		if (WARN_ON(hwsim_get_chanwidth(bw) > hwsim_get_chanwidth(confbw))) {
			if (debug_assoc)
				nuttx_hwsim_debugf("hwsim-debug: hwsim_tx assoc drop width bw=%d confbw=%d bw_mhz=%d confbw_mhz=%d rate_idx=%d rflags=0x%x\n",
				       bw, confbw, hwsim_get_chanwidth(bw),
				       hwsim_get_chanwidth(confbw),
				       txi->control.rates[i].idx, rflags);
			return;
		}
	}
...
static int mac80211_hwsim_start(struct ieee80211_hw *hw)
{
	...
	data->started = true;
	data->medium_frame_offset = nuttx_hwsim_medium_frame_eof();
	ieee80211_queue_delayed_work(hw, &data->medium_rx,
				     msecs_to_jiffies(NUTTX_HWSIM_MEDIUM_POLL_MS));
	return 0;
}
...
static void mac80211_hwsim_stop(struct ieee80211_hw *hw, bool suspend)
{
	...
	data->started = false;
	cancel_delayed_work_sync(&data->medium_rx);
	cancel_delayed_work_sync(&data->auto_ps_poll);
	cancel_delayed_work_sync(&data->dyn_ps_idle);
	hwsim_twt_disable_auto(data, false, "stop");
...
static void
nuttx_hwsim_medium_publish_beacon_template(struct mac80211_hwsim_data *data,
					   struct ieee80211_hw *hw,
					   struct ieee80211_vif *vif,
					   struct ieee80211_bss_conf *link_conf)
{
	...
	if (link_conf->chanctx_conf)
		chan = rcu_dereference(link_conf->chanctx_conf)->def.chan;
	if (!chan)
		chan = data->channel;
	if (!chan)
		return;

	skb = ieee80211_beacon_get(hw, vif, link_conf->link_id);
	if (!skb)
		return;
	...
	data->abs_bcn_ts = now;
	mgmt = (struct ieee80211_mgmt *)skb->data;
	if (skb->len >= 32 && !ieee80211_is_s1g_beacon(mgmt->frame_control))
					     ...
					     u64 changed)
{
	...
	if (changed & BSS_CHANGED_BEACON_ENABLED) {
		...
		if (info->enable_beacon)
			nuttx_hwsim_medium_publish_beacon_template(data, hw,
								  vif, info);
	}
...
static const struct nla_policy hwsim_testmode_policy[HWSIM_TM_ATTR_MAX + 1] = {
	...
	[HWSIM_TM_ATTR_TWT_SP] = { .type = NLA_U32 },
};
				       ...
				       void *data, int len)
{
	...
	case HWSIM_TM_CMD_WAKE_QUEUES:
		...
		return 0;
	case HWSIM_TM_CMD_SET_TWT_SP:
		if (!tb[HWSIM_TM_ATTR_TWT_SP])
			return -EINVAL;
		twt_sp = nla_get_u32(tb[HWSIM_TM_ATTR_TWT_SP]);
		if (twt_sp > 1)
			return -EINVAL;
		hwsim->twt_sp_open = twt_sp;
		nuttx_hwsim_ps_prooff("twt-proof: hwsim service period %s\n",
				      twt_sp ? "open" : "closed");
		local_bh_disable();
		ieee80211_iterate_active_interfaces_atomic(
			hwsim->hw, IEEE80211_IFACE_ITER_NORMAL,
			twt_sp ? hwsim_send_nullfunc_no_ps :
				 hwsim_send_nullfunc_ps,
			hwsim);
		local_bh_enable();
		return 0;
	case HWSIM_TM_CMD_GET_TWT_SP:
		skb = cfg80211_testmode_alloc_reply_skb(hw->wiphy,
					nla_total_size(sizeof(u32)));
		if (!skb)
			return -ENOMEM;
		if (nla_put_u32(skb, HWSIM_TM_ATTR_TWT_SP,
				hwsim->twt_sp_open ? 1 : 0))
			goto nla_put_failure;
		return cfg80211_testmode_reply(skb);
	default:
		return -EOPNOTSUPP;
				       ...
				       struct ieee80211_ampdu_params *params)
{
	...
	if (sta)
		nuttx_hwsim_ampdu_prooff("ampdu-proof: hwsim ampdu_action "
					 "action=%d tid=%u sta="
					 NUTTX_HWSIM_MAC_FMT " buf=%u "
					 "amsdu=%d ssn=%u timeout=%u\n",
					 action, tid, NUTTX_HWSIM_MAC_ARG(sta->addr),
					 params->buf_size,
					 params->amsdu ? 1 : 0, params->ssn,
					 params->timeout);
	else
		nuttx_hwsim_ampdu_prooff("ampdu-proof: hwsim ampdu_action "
					 "action=%d tid=%u sta=NULL buf=%u "
					 "amsdu=%d ssn=%u timeout=%u\n",
					 action, tid, params->buf_size,
					 params->amsdu ? 1 : 0, params->ssn,
					 params->timeout);
...
static void hw_scan_work(struct work_struct *work)
{
	...
	nuttx_hwsim_debugf("hwsim-debug: hw_scan_work enter req=%p idx=%d\n",
	       req, hwsim->scan_chan_idx);

	mutex_lock(&hwsim->mutex);
	/*
	 * The simulated medium is file-backed and the peer process may publish a
	 * Probe Response while this radio is dwelling on the previous channel.
	 * Sample it before advancing/completing the scan so late responses are
	 * visible to cfg80211.
	 */
	nuttx_hwsim_medium_inject_scan_bss(hwsim);
	if (hwsim->scan_chan_idx >= req->n_channels) {
		struct cfg80211_scan_info info = {
			.aborted = false,
		};

		nuttx_hwsim_debugf("hwsim-debug: hw_scan_work complete channels=%d\n",
		       req->n_channels);
				  ...
				  struct ieee80211_scan_request *hw_req)
{
	...
	nuttx_hwsim_debugf("hwsim-debug: mac80211_hwsim_hw_scan queue req=%p chans=%d\n",
	       req, req->n_channels);
...
static void mac80211_hwsim_add_twt_setup(struct ieee80211_hw *hw,
					 struct ieee80211_sta *sta,
					 struct ieee80211_twt_setup *twt)
{
	struct ieee80211_twt_params *params = (void *)twt->params;
	...
	nuttx_hwsim_ps_prooff("twt-proof: hwsim add_twt_setup sta="
			      NUTTX_HWSIM_MAC_FMT
			      " dialog=%u setup_cmd=%u flow=%u exponent=%u mantissa=%u min_twt=%u\n",
			      NUTTX_HWSIM_MAC_ARG(sta->addr),
			      twt->dialog_token,
			      (unsigned int)FIELD_GET(IEEE80211_TWT_REQTYPE_SETUP_CMD,
						      req_type),
			      (unsigned int)FIELD_GET(IEEE80211_TWT_REQTYPE_FLOWID,
						      req_type),
			      (unsigned int)FIELD_GET(IEEE80211_TWT_REQTYPE_WAKE_INT_EXP,
						      req_type),
			      le16_to_cpu(params->mantissa),
			      params->min_twt_dur);
}

static void mac80211_hwsim_twt_teardown_request(struct ieee80211_hw *hw,
						struct ieee80211_sta *sta,
						u8 flowid)
{
	nuttx_hwsim_ps_prooff("twt-proof: hwsim twt_teardown_request sta="
			      NUTTX_HWSIM_MAC_FMT " flow=%u\n",
			      NUTTX_HWSIM_MAC_ARG(sta->addr), flowid);
}
...
#define HWSIM_COMMON_OPS					\
	...
	.add_twt_setup = mac80211_hwsim_add_twt_setup,		\
	.twt_teardown_request = mac80211_hwsim_twt_teardown_request, \
		...
		.he_cap = {
			...
			.he_cap_elem = {
				...
				.phy_cap_info[1] =
					...
					HWSIM_HE_DCM_PHY_CAPS

					/* Leave all the other PHY capability bytes
					 * unset, as beam forming and PPE
				    ...
				    struct hwsim_new_radio_params *param)
{
	...
	if (!param->perm_addr) {
		unsigned int macidx = idx + CONFIG_WL_NUTTX_HWSIM_RADIO_BASE;
	...
	if (data->use_chanctx) {
		...
		/* DPP public action exchanges can request a 2000 ms wait. */
	...
	INIT_DELAYED_WORK(&data->medium_rx, nuttx_hwsim_medium_rx_work);
	INIT_DELAYED_WORK(&data->auto_ps_poll, hwsim_auto_ps_poll_work);
	INIT_DELAYED_WORK(&data->dyn_ps_idle, hwsim_dyn_ps_idle_work);
	INIT_DELAYED_WORK(&data->twt_sp, hwsim_twt_sp_work);
...
int mac80211_hwsim_linux_initialize(void)
{
	...
	if (mac80211_hwsim_linux_initialized)
		return 0;

	ret = init_mac80211_hwsim();
	if (ret == 0)
		mac80211_hwsim_linux_initialized = true;
...
void mac80211_hwsim_linux_uninitialize(void)
{
	...
	exit_mac80211_hwsim();
	mac80211_hwsim_linux_initialized = false;
}
