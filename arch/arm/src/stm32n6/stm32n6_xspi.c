...
#define STM32N6_XSPI_NOR_RESET_DELAY_MS  100u
...
static uint8_t g_nor_jedec[3] =
{
  MX66UW1G45G_JEDEC0,
  MX66UW1G45G_JEDEC1,
  MX66UW1G45G_JEDEC2
};
static bool g_nor_jedec_valid;

/****************************************************************************
 * Private Functions
 ****************************************************************************/

static uint32_t stm32n6_xspi_prescaler(uint32_t source_hz,
                                       uint32_t target_hz)
{
  uint32_t div;
...
static uint32_t stm32n6_xspi_effective_hz(uint32_t source_hz,
                                          uint32_t prescaler)
{
...
static bool stm32n6_xspi_is_mapped(uintptr_t base)
{
...
static STM32N6_RAMFUNC int stm32n6_xspi_wait_idle(uintptr_t base)
{
...
static void stm32n6_xspi_config_gpio(uintptr_t portbase, uint32_t pins,
                                     uint32_t af)
{
  uint32_t regval;
  int pin;
...
static int stm32n6_xspi_common_setup(void)
{
...
static STM32N6_RAMFUNC int stm32n6_xspi_command(uintptr_t base,
...
static STM32N6_RAMFUNC int stm32n6_xspi_command_addr(
...
static int stm32n6_xspi_readid(uint8_t id[3])
{
  ...
  modifyreg32(STM32N6_XSPI_CR(base), XSPI_CR_FMODE_MASK, 0);
  putreg32(2, STM32N6_XSPI_DLR(base));
  putreg32(0, STM32N6_XSPI_TCR(base));
  putreg32(XSPI_CCR_IMODE_1_LINE | XSPI_CCR_DMODE_1_LINE,
           STM32N6_XSPI_CCR(base));
  putreg32(MX66UW1G45G_READ_ID_CMD, STM32N6_XSPI_IR(base));
...
static bool stm32n6_xspi_id_blank(FAR const uint8_t id[3])
{
  return (id[0] == 0x00 && id[1] == 0x00 && id[2] == 0x00) ||
         (id[0] == 0xff && id[1] == 0xff && id[2] == 0xff);
}

static STM32N6_RAMFUNC int stm32n6_xspi_read_data(
  uintptr_t base, uint32_t instruction, uint32_t address, uint32_t ccr,
  uint32_t tcr, size_t nbytes, uint8_t *value)
{
  volatile uint8_t *dr = (volatile uint8_t *)STM32N6_XSPI_DR(base);
  size_t i;
  int ret;
...
static STM32N6_RAMFUNC int stm32n6_xspi_write_data(
  uintptr_t base, uint32_t instruction, uint32_t address, uint32_t ccr,
...
static STM32N6_RAMFUNC int stm32n6_xspi_write_buffer(
  uintptr_t base, uint32_t instruction, uint32_t address, uint32_t ccr,
...
static void stm32n6_xspi2_gpio_config(void)
{
...
static void stm32n6_xspi1_gpio_config(void)
{
...
static void stm32n6_xspi_controller_config(uintptr_t base, uint32_t reset,
                                           ...
                                           uint32_t csbound)
{
  modifyreg32(STM32N6_RCC_AHB5RSTR, 0, reset);
  modifyreg32(STM32N6_RCC_AHB5RSTR, reset, 0);

  putreg32(0, STM32N6_XSPI_CR(base));
  putreg32(memory_type | XSPI_DCR1_DEVSIZE(device_size) | XSPI_DCR1_CSHT(csht),
           STM32N6_XSPI_DCR1(base));
  putreg32(XSPI_DCR2_PRESCALER(prescaler), STM32N6_XSPI_DCR2(base));
  putreg32(XSPI_DCR3_CSBOUND(csbound), STM32N6_XSPI_DCR3(base));
  putreg32(0, STM32N6_XSPI_DCR4(base));
  putreg32((fifo_threshold - 1u) << XSPI_CR_FTHRES_SHIFT,
           STM32N6_XSPI_CR(base));
  modifyreg32(STM32N6_XSPI_CR(base), 0, XSPI_CR_EN);
}

static STM32N6_RAMFUNC int stm32n6_xspi_set_prescaler(uintptr_t base,
                                                      uint32_t prescaler)
{
  int ret;
...
static STM32N6_RAMFUNC int stm32n6_xspi_abort_memory_mapped(uintptr_t base)
{
  int ret;
...
static int stm32n6_nor_read_cfg2_spi(uint32_t address, uint8_t *value)
{
  uint32_t ccr;
...
static int stm32n6_nor_read_status_spi(uint8_t *status)
{
  uint32_t ccr;
...
static STM32N6_RAMFUNC int stm32n6_nor_read_status_octal(uint8_t *status)
{
  uint32_t ccr;
...
static STM32N6_RAMFUNC int stm32n6_nor_wait_status(
...
static int stm32n6_nor_wait_wip_spi(void)
{
  return stm32n6_nor_wait_status(stm32n6_nor_read_status_spi,
                                 MX66UW1G45G_SR_WIP, 0);
}

static STM32N6_RAMFUNC int stm32n6_nor_wait_wip_octal(void)
{
  return stm32n6_nor_wait_status(stm32n6_nor_read_status_octal,
                                 MX66UW1G45G_SR_WIP, 0);
}

static int stm32n6_nor_write_enable_spi(void)
{
  int ret;
...
static STM32N6_RAMFUNC int stm32n6_nor_write_enable_octal(void)
{
  int ret;
...
static int stm32n6_nor_write_cfg2_spi(uint32_t address, uint8_t value,
                                      bool wait_after)
{
  uint32_t ccr;
  int ret;

  ret = stm32n6_nor_wait_wip_spi();
  if (ret < 0)
    {
      return ret;
    }

  ret = stm32n6_nor_write_enable_spi();
  if (ret < 0)
    {
      return ret;
    }
  ...
  return stm32n6_nor_wait_wip_spi();
}

static int stm32n6_nor_read_cfg2_octal(uint32_t address, uint8_t *value)
{
  uint32_t ccr;
...
static int stm32n6_nor_config_cfg2_spi(uint32_t address, uint8_t value,
                                       uint8_t mask)
{
  uint8_t readback;
  uint8_t writeval;
  ...
  ret = stm32n6_nor_read_cfg2_spi(address, &readback);
  if (ret < 0)
    {
      return ret;
    }
  ...
  ret = stm32n6_nor_write_cfg2_spi(address, writeval, true);
  if (ret < 0)
    {
      return ret;
    }
...
static STM32N6_RAMFUNC int stm32n6_xspi2_enter_memory_mapped(void)
{
  uintptr_t base = STM32N6_XSPI2_BASE;
  ...
  syslog(LOG_INFO, "XSPI2 NOR optional prescaler=%" PRIu32
         " effective=%" PRIu32 "Hz\n", prescaler,
         stm32n6_xspi_effective_hz(g_xspi2_source_hz, prescaler));
#endif
  ...
  putreg32(XSPI_TCR_DCYC(MX66UW1G45G_OCTAL_READ_DUMMY),
           STM32N6_XSPI_TCR(base));
  putreg32(MX66UW1G45G_OCTAL_READ_CMD, STM32N6_XSPI_IR(base));

  putreg32(ccr, STM32N6_XSPI_WCCR(base));
  putreg32(0, STM32N6_XSPI_WTCR(base));
  putreg32(MX66UW1G45G_OCTAL_WRITE_CMD, STM32N6_XSPI_WIR(base));
...
static int stm32n6_xspi1_enter_memory_mapped(void)
{
  ...
  prescaler = g_vddio2_hslv ?
              stm32n6_xspi_prescaler(g_xspi1_source_hz,
                                      STM32N6_XSPI_200MHZ) :
              stm32n6_xspi_prescaler(g_xspi1_source_hz,
                                      STM32N6_XSPI_50MHZ);
  ...
  syslog(LOG_INFO, "XSPI1 PSRAM optional prescaler=%" PRIu32
         " effective=%" PRIu32 "Hz\n", prescaler,
         stm32n6_xspi_effective_hz(g_xspi1_source_hz, prescaler));
  ...
  modifyreg32(STM32N6_XSPI_CR(base), XSPI_CR_FMODE_MASK,
              XSPI_CR_FMODE_INDIRECT_WRITE);
  putreg32(ccr, STM32N6_XSPI_CCR(base));
  putreg32(XSPI_TCR_DCYC(APS256_HEXA_DUMMY), STM32N6_XSPI_TCR(base));
  putreg32(APS256_READ_CMD, STM32N6_XSPI_IR(base));

  putreg32(ccr, STM32N6_XSPI_WCCR(base));
  putreg32(XSPI_TCR_DCYC(APS256_HEXA_DUMMY), STM32N6_XSPI_WTCR(base));
  putreg32(APS256_HEXA_WRITE_CMD, STM32N6_XSPI_WIR(base));
...
static int stm32n6_psram_write_reg(uint32_t address, uint8_t value)
{
  uint32_t ccr;
...
static int stm32n6_psram_read_reg(uint32_t address, uint8_t *value)
{
  uint32_t ccr;
  ...
  return stm32n6_xspi_read_data(STM32N6_XSPI1_BASE,
                                APS256_REG_READ_CMD, address, ccr,
                                XSPI_TCR_DCYC(APS256_REG_DUMMY),
                                2, value);
}

static int stm32n6_psram_config_reg(uint32_t address, uint8_t value,
                                    uint8_t mask)
{
  uint8_t initial;
  uint8_t readback;
  ...
  ret = stm32n6_psram_read_reg(address, &initial);
  if (ret < 0)
    {
      return ret;
    }
  ...
  ret = stm32n6_psram_write_reg(address, writeval);
  if (ret < 0)
    {
      return ret;
    }

  ret = stm32n6_psram_read_reg(address, &readback);
  if (ret < 0)
    {
      return ret;
    }
...
#ifdef CONFIG_NXBOOT_BOOTLOADER
static int stm32n6_psram_selftest(void)
{
  return stm32n6_xspi1_psram_selftest(STM32N6_XSPI1_MEM_BASE);
}
#else
static int stm32n6_psram_selftest(void)
{
...
static STM32N6_RAMFUNC uint32_t stm32n6_nor_octal_command_ccr(void)
{
...
static STM32N6_RAMFUNC uint32_t stm32n6_nor_octal_data_ccr(void)
{
  return stm32n6_nor_octal_command_ccr() |
         XSPI_CCR_DMODE_8_LINES | XSPI_CCR_DDTR | XSPI_CCR_DQSE;
}

static void stm32n6_nor_log_error(FAR const char *op, int ret)
{
  uint8_t status = 0xff;

  (void)stm32n6_nor_read_status_octal(&status);
...
static STM32N6_RAMFUNC int stm32n6_nor_restore_memory_mapped(int opret)
{
  int mapret;

  mapret = stm32n6_xspi2_enter_memory_mapped();
  if (mapret < 0)
    {
#ifdef CONFIG_NXBOOT_BOOTLOADER
...
static STM32N6_RAMFUNC int stm32n6_nor_finish_error(FAR const char *op,
                                                    int opret)
{
  int mapret;
  ...
  if (mapret >= 0)
    {
      stm32n6_nor_log_error(op, opret);
    }
...
int stm32n6_xspi2_nor_initialize(void)
{
  ...
  startup_prescaler =
    stm32n6_xspi_prescaler(g_xspi2_source_hz, STM32N6_XSPI_50MHZ);

  syslog(LOG_INFO, "XSPI2 NOR startup prescaler=%" PRIu32
         " effective=%" PRIu32 "Hz\n", startup_prescaler,
         stm32n6_xspi_effective_hz(g_xspi2_source_hz, startup_prescaler));

  stm32n6_xspi2_gpio_config();
  stm32n6_xspi_controller_config(STM32N6_XSPI2_BASE,
                                 RCC_AHB5RSTR_XSPI2RST,
                                 XSPI_DCR1_MTYP_MACRONIX,
                                 MX66UW1G45G_DEVSIZE, 2,
                                 startup_prescaler, 4, 0);
  ...
  up_mdelay(STM32N6_XSPI_NOR_RESET_DELAY_MS);
  ...
  memcpy(g_nor_jedec, id, sizeof(g_nor_jedec));
  g_nor_jedec_valid = true;

  ret = stm32n6_nor_config_cfg2_spi(MX66UW1G45G_CFG2_DOPI_ADDR,
  ...
  ret = stm32n6_nor_read_cfg2_spi(MX66UW1G45G_CFG2_OPI_ADDR, &cfg2);
  if (ret < 0)
    {
  ...
  ret = stm32n6_nor_write_cfg2_spi(MX66UW1G45G_CFG2_OPI_ADDR, cfg2,
                                   false);
  if (ret < 0)
    {
  ...
  ret = stm32n6_nor_wait_wip_octal();
  if (ret < 0)
    {
  ...
  ret = stm32n6_nor_read_cfg2_octal(MX66UW1G45G_CFG2_OPI_ADDR, &cfg2);
  if (ret < 0)
    {
  ...
  ret = stm32n6_xspi2_enter_memory_mapped();
  if (ret < 0)
    {
...
int stm32n6_xspi2_nor_getinfo(struct stm32n6_xspi2_nor_info_s *info)
{
  ...
  memcpy(info->jedec, g_nor_jedec, sizeof(info->jedec));
  info->jedec_valid = g_nor_jedec_valid;
  info->mapped      = stm32n6_xspi_is_mapped(STM32N6_XSPI2_BASE);
  info->common_ready = g_xspi_common_done;
  info->vddio2_hslv = g_vddio2_hslv;
  info->vddio3_hslv = g_vddio3_hslv;
  info->base        = STM32N6_XSPI2_MEM_BASE;
  info->size        = MX66UW1G45G_SIZE;
  info->erase_size  = MX66UW1G45G_SECTOR_SIZE;
  info->page_size   = MX66UW1G45G_PAGE_SIZE;
  info->source_hz   = g_xspi2_source_hz;

  prescaler = getreg32(STM32N6_XSPI_DCR2(STM32N6_XSPI2_BASE)) &
              XSPI_DCR2_PRESCALER_MASK;
  info->effective_hz = stm32n6_xspi_effective_hz(g_xspi2_source_hz,
                                                 prescaler);

  return OK;
}

STM32N6_RAMFUNC int stm32n6_xspi2_nor_erase(uint32_t offset,
                                            size_t nbytes)
{
  uint32_t ccr;
  int ret;
  ...
  if (ret < 0)
    {
      return stm32n6_nor_finish_error("abort", ret);
    }

  ccr = stm32n6_nor_octal_command_ccr();
  while (nbytes > 0)
    {
      uint32_t cmd;
      size_t erasesize;
      ...
      ret = stm32n6_nor_write_enable_octal();
      if (ret < 0)
        {
          return stm32n6_nor_finish_error("write-enable erase", ret);
        }
      ...
      if (ret < 0)
        {
          return stm32n6_nor_finish_error("erase command", ret);
        }

      ret = stm32n6_nor_wait_wip_octal();
      if (ret < 0)
        {
          return stm32n6_nor_finish_error("erase wait", ret);
        }
  ...
  return stm32n6_nor_restore_memory_mapped(OK);
}

STM32N6_RAMFUNC ssize_t stm32n6_xspi2_nor_write(
  uint32_t offset, FAR const uint8_t *buffer, size_t nbytes)
{
  ...
  ccr = stm32n6_nor_octal_data_ccr();
  while (written < nbytes)
    {
      ...
      if (ret < 0)
        {
          (void)stm32n6_nor_finish_error("write-enable program", ret);
      ...
      if (ret < 0)
        {
          (void)stm32n6_nor_finish_error("page program", ret);
          return written > 0 ? (ssize_t)written : ret;
        }
      ...
      if (ret < 0)
        {
          (void)stm32n6_nor_finish_error("program wait", ret);
          return written > 0 ? (ssize_t)written : ret;
        }
  ...
  ret = stm32n6_nor_restore_memory_mapped(OK);
...
int stm32n6_xspi1_psram_initialize(void)
{
  ...
  startup_prescaler =
    stm32n6_xspi_prescaler(g_xspi1_source_hz, STM32N6_XSPI_50MHZ);

  syslog(LOG_INFO, "XSPI1 PSRAM startup prescaler=%" PRIu32
         " effective=%" PRIu32 "Hz\n", startup_prescaler,
         stm32n6_xspi_effective_hz(g_xspi1_source_hz, startup_prescaler));

  stm32n6_xspi1_gpio_config();
  stm32n6_xspi_controller_config(STM32N6_XSPI1_BASE,
                                 RCC_AHB5RSTR_XSPI1RST,
                                 XSPI_DCR1_MTYP_APMEM_16BIT,
                                 APS256_DEVSIZE, 5,
                                 startup_prescaler, 4, 10);
  ...
  ret = stm32n6_psram_config_reg(APS256_MR0_ADDR, APS256_MR0_VALUE, 0x3f);
  if (ret < 0)
    {
  ...
  ret = stm32n6_psram_config_reg(APS256_MR4_ADDR, APS256_MR4_VALUE, 0xe0);
  if (ret < 0)
    {
  ...
  ret = stm32n6_psram_config_reg(APS256_MR8_ADDR, APS256_MR8_VALUE, 0x40);
  if (ret < 0)
    {
  ...
  ret = stm32n6_xspi1_enter_memory_mapped();
  if (ret < 0)
    {
