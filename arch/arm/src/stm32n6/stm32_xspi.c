...
#define STM32_XSPI_200MHZ              200000000u
#define STM32_XSPI_50MHZ               50000000u
#define STM32_XSPI_IC3_200MHZ_DIV      4u
...
static void stm32_xspi_read_hslv_otp(void)
{
  uint32_t fuse;
...
static void stm32_xspi_probe_clock_state(void)
{
  if (!g_xspi_clock_state_valid)
    {
      stm32_xspi_read_hslv_otp();
    }
}

static void stm32_xspi_config_ic3(void)
{
...
uint32_t stm32_xspi_prescaler(uint32_t source_hz, uint32_t target_hz)
{
  uint32_t div;
...
uint32_t stm32_xspi_effective_hz(uint32_t source_hz,
                                   uint32_t prescaler)
{
  return source_hz / (prescaler + 1u);
}

bool stm32_xspi_common_ready(void)
{
  return g_xspi_common_done;
}

uint32_t stm32_xspi_get_source_hz(uintptr_t base)
{
  stm32_xspi_probe_clock_state();

  return base == STM32_XSPI2_BASE ? g_xspi2_source_hz :
                                      g_xspi1_source_hz;
}

bool stm32_xspi_hslv_enabled(uintptr_t base)
{
  stm32_xspi_probe_clock_state();

  return base == STM32_XSPI2_BASE ? g_vddio3_hslv : g_vddio2_hslv;
}
  ...
  uint32_t tcr, FAR const uint8_t *buffer, size_t nbytes)
{
  ...
  ret = stm32_xspi_wait_idle(base);
  putreg32(XSPI_FCR_CTCF | XSPI_FCR_CTEF, STM32_XSPI_FCR(base));
  return ret;
}
  ...
  FAR const struct stm32_xspi_config_s *config)
{
  ...
  putreg32(config->refresh, STM32_XSPI_DCR4(config->base));
