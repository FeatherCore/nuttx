...
static int stm32_ltdc_interrupt(int irq, FAR void *context,
                                FAR void *arg);
...
static int stm32_ltdc_gpio_config(void)
{
  unsigned int i;
  int ret;
  ...
  for (i = 0; i < sizeof(g_ltdcpins) / sizeof(g_ltdcpins[0]); i++)
    {
      ret = stm32_configgpio(g_ltdcpins[i]);
      if (ret < 0)
        {
          return ret;
        }
...
static int stm32_ltdc_clock_enable(void)
{
...
static void stm32_ltdc_rif_master_config(unsigned int master)
{
  uint32_t regval;
...
static bool stm32_ltdc_layer_reload(uint32_t reload)
{
  uint32_t timeout = 100000;

  putreg32(reload | LTDC_LRCR_GRMSK, STM32_LTDC_LRCR);

  while ((getreg32(STM32_LTDC_LRCR) & reload) != 0)
    {
      if (timeout-- == 0)
        {
          return false;
...
static bool stm32_ltdc_layer_enable(void)
{
  modifyreg32(STM32_LTDC_LCR, 0, LTDC_LCR_LEN);
  if (!stm32_ltdc_layer_reload(LTDC_LRCR_IMR))
    {
      return false;
    }

  return (getreg32(STM32_LTDC_LCR) & LTDC_LCR_LEN) != 0;
}

static void stm32_ltdc_config(void)
{
  ...
  putreg32((hsync << 16) | vsync, STM32_LTDC_SSCR);
  putreg32((hbp << 16) | vbp, STM32_LTDC_BPCR);
  putreg32((activew << 16) | activeh, STM32_LTDC_AWCR);
  putreg32((totalw << 16) | totalh, STM32_LTDC_TWCR);
  putreg32(0, STM32_LTDC_BCCR);

  putreg32(LTDC_LRCR_GRMSK, STM32_LTDC_LRCR);
  putreg32(LTDC_GCR_LTDCEN, STM32_LTDC_GCR);

  putreg32(0, STM32_LTDC_LCR);
  putreg32((xstop << 16) | xstart, STM32_LTDC_LWHPCR);
  putreg32((ystop << 16) | ystart, STM32_LTDC_LWVPCR);
  putreg32(LTDC_PIXEL_FORMAT_RGB565, STM32_LTDC_LPFCR);
  putreg32(0, STM32_LTDC_LFPF0R);
  putreg32(0, STM32_LTDC_LFPF1R);
  putreg32(LTDC_FULL_ALPHA, STM32_LTDC_LCACR);
  putreg32(0, STM32_LTDC_LDCCR);
  putreg32(LTDC_BLENDING_FACTOR1_CA | LTDC_BLENDING_FACTOR2_CA,
           STM32_LTDC_LBFCR);
  putreg32(0, STM32_LTDC_LBLCR);
  putreg32(0, STM32_LTDC_LPCR);
  putreg32(BOARD_LTDC_FB_BASE, STM32_LTDC_LCFBAR);
  putreg32((BOARD_LTDC_STRIDE << 16) | (BOARD_LTDC_STRIDE + 7),
           STM32_LTDC_LCFBLR);
  putreg32(BOARD_LTDC_HEIGHT, STM32_LTDC_LCFBLNR);

  if (!stm32_ltdc_layer_enable())
    {
      syslog(LOG_WARNING,
             "stm32n6: LTDC layer enable did not latch cr=%08lx "
             "rcr=%08lx\n",
             (unsigned long)getreg32(STM32_LTDC_LCR),
             (unsigned long)getreg32(STM32_LTDC_LRCR));
    }
...
static void stm32_ltdc_irq_enable(uint32_t mask)
{
  ...
  modifyreg32(STM32_LTDC_IER2, 0, mask);
}

static void stm32_ltdc_irq_disable(uint32_t mask)
{
  modifyreg32(STM32_LTDC_IER, mask, 0);
  modifyreg32(STM32_LTDC_IER2, mask, 0);
}

static void stm32_ltdc_irq_clear(uint32_t mask)
{
  putreg32(mask, STM32_LTDC_ICR);
  putreg32(mask, STM32_LTDC_ICR2);
}
                                  ...
                                  FAR void *arg)
{
  ...
  isr2 = getreg32(STM32_LTDC_ISR2);
  pending = (isr & getreg32(STM32_LTDC_IER)) |
            (isr2 & getreg32(STM32_LTDC_IER2));

  if ((pending & LTDC_ISR_RRIF) != 0)
    {
      stm32_ltdc_irq_disable(LTDC_IER_RRIE);
  ...
  if ((pending & LTDC_ISR_TERRIF) != 0)
    {
      stm32_ltdc_irq_clear(LTDC_ICR_CTERRIF);
      error = -EIO;
      done = true;
    }
  ...
  if ((pending & LTDC_ISR_FUIF) != 0)
    {
      stm32_ltdc_irq_clear(LTDC_ICR_CFUIF);
      error = -EIO;
      done = true;
    }
...
static int stm32_ltdc_irqconfig(void)
{
  ...
  ret = irq_attach(STM32_IRQ_LTDC_LO, stm32_ltdc_interrupt, NULL);
  if (ret < 0)
    {
  ...
  ret = irq_attach(STM32_IRQ_LTDC_UP, stm32_ltdc_interrupt, NULL);
  if (ret < 0)
    {
                              ...
                              FAR struct fb_planeinfo_s *pinfo)
{
      ...
      if (g_ltdc.sync_warn_count < 4)
        {
          syslog(LOG_WARNING,
                 ...
                 (unsigned long)getreg32(STM32_LTDC_SRCR),
                 (unsigned long)getreg32(STM32_LTDC_ISR),
                 (unsigned long)getreg32(STM32_LTDC_IER),
                 (unsigned long)getreg32(STM32_LTDC_ISR2),
                 (unsigned long)getreg32(STM32_LTDC_IER2));
  ...
  putreg32((uint32_t)fbaddr, STM32_LTDC_LCFBAR);
      ...
      if (g_ltdc.sync_warn_count < 8)
        {
          syslog(LOG_WARNING,
                 ...
                 (unsigned long)getreg32(STM32_LTDC_ISR2),
                 (unsigned long)getreg32(STM32_LTDC_IER2),
                 (unsigned long)getreg32(STM32_LTDC_CPSR),
                 (unsigned long)getreg32(STM32_LTDC_CDSR),
                 (unsigned long)getreg32(STM32_LTDC_LCFBAR));
          g_ltdc.sync_warn_count++;
        }
...
void weak_function stm32_ltdcpower(bool on)
{
}
...
int stm32_ltdcinitialize(void)
{
  ...
  stm32_ltdc_rif_config();

  ret = stm32_ltdc_gpio_config();
  if (ret < 0)
    {
  ...
  if (g_ltdc.initialized)
    {
      ...
      stm32_ltdc_setpower(&g_ltdc.vtable, 0);
