...
bool stm32h7rs_xspi_is_mapped(uintptr_t base)
{
...
int stm32h7rs_xspi_wait_idle(uintptr_t base)
{
  return stm32h7rs_xspi_wait_reg_trace(STM32H7RS_XSPI_SR(base),
                                       XSPI_SR_BUSY, 0, "idle");
}

void stm32h7rs_xspi_config_gpio(uintptr_t portbase, uint32_t pins,
                                uint32_t af)
{
  uint32_t regval;
  int pin;
...
int stm32h7rs_xspi_common_setup(void)
{
  int ret;
...
int stm32h7rs_xspi_command(uintptr_t base, uint32_t instruction,
                           uint32_t ccr)
{
  int ret;
...
int stm32h7rs_xspi_command_addr(uintptr_t base, uint32_t instruction,
...
int stm32h7rs_xspi_read_data(uintptr_t base, uint32_t instruction,
                             uint32_t address, uint32_t ccr,
...
int stm32h7rs_xspi_write_data(uintptr_t base, uint32_t instruction,
                              uint32_t address, uint32_t ccr,
...
void stm32h7rs_xspi_controller_config(
  FAR const struct stm32h7rs_xspi_config_s *config)
{
  uint32_t fthreshold;
  ...
  putreg32(0, STM32H7RS_XSPI_CR(config->base));
  putreg32(config->memory_type | XSPI_DCR1_DEVSIZE(config->device_size) |
           XSPI_DCR1_CSHT(config->csht),
           STM32H7RS_XSPI_DCR1(config->base));
  putreg32(XSPI_DCR2_PRESCALER(config->prescaler),
           STM32H7RS_XSPI_DCR2(config->base));
  putreg32(XSPI_DCR3_CSBOUND(config->csbound) |
           XSPI_DCR3_MAXTRAN(config->maxtran),
           STM32H7RS_XSPI_DCR3(config->base));
  putreg32(XSPI_DCR4_REFRESH(config->refresh),
           STM32H7RS_XSPI_DCR4(config->base));
  putreg32(fthreshold << XSPI_CR_FTHRES_SHIFT,
           STM32H7RS_XSPI_CR(config->base));
  modifyreg32(STM32H7RS_XSPI_CR(config->base), 0, XSPI_CR_EN);
}

int stm32h7rs_xspi_set_prescaler(uintptr_t base, uint32_t prescaler)
{
  int ret;
...
int stm32h7rs_xspi_enter_memory_mapped(uintptr_t base, uint32_t read_ccr,
                                       uint32_t read_tcr,
                                       ...
                                       uint32_t write_instruction)
{
  ...
  putreg32(read_ccr, STM32H7RS_XSPI_CCR(base));
  putreg32(read_tcr, STM32H7RS_XSPI_TCR(base));
  putreg32(read_instruction, STM32H7RS_XSPI_IR(base));

  putreg32(write_ccr, STM32H7RS_XSPI_WCCR(base));
  putreg32(write_tcr, STM32H7RS_XSPI_WTCR(base));
  putreg32(write_instruction, STM32H7RS_XSPI_WIR(base));
