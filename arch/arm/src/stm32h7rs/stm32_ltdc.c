/****************************************************************************
 * arch/arm/src/stm32h7rs/stm32_ltdc.c
 *
...
#  error STM32H7RS LTDC requires RGB565 framebuffer output
#endif
...
static void stm32_gpio_alt(uintptr_t portbase, uint32_t pins,
                               uint32_t af)
{
  uint32_t regval;
  int pin;
  ...
  modifyreg32(portbase + STM32_GPIO_OTYPER_OFFSET, pins, 0);
...
static int stm32_ltdc_gpio_config(void)
{
  ...
  stm32_gpio_alt(STM32_GPIOB_BASE, (1u << 13) | (1u << 15),
                     GPIO_AF_LTDC10);
  stm32_gpio_alt(STM32_GPIOB_BASE,
                     (1u << 3) | (1u << 4) | (1u << 10) |
                     (1u << 11) | (1u << 12) | (1u << 14),
                     GPIO_AF_LTDC13);
  stm32_gpio_alt(STM32_GPIOA_BASE,
                     (1u << 0) | (1u << 1) | (1u << 8) |
                     (1u << 11) | (1u << 12) | (1u << 15),
                     GPIO_AF_LTDC13);
  stm32_gpio_alt(STM32_GPIOA_BASE, 1u << 6, GPIO_AF_LTDC12);
  stm32_gpio_alt(STM32_GPIOA_BASE, (1u << 9) | (1u << 10),
                     GPIO_AF_LTDC14);
  stm32_gpio_alt(STM32_GPIOG_BASE,
                     (1u << 0) | (1u << 1) | (1u << 2) |
                     (1u << 13) | (1u << 14),
                     GPIO_AF_LTDC13);
  stm32_gpio_alt(STM32_GPIOE_BASE, 1u << 11, GPIO_AF_LTDC11);
  stm32_gpio_alt(STM32_GPIOF_BASE, 1u << 0, GPIO_AF_LTDC11);
  stm32_gpio_alt(STM32_GPIOF_BASE, (1u << 10) | (1u << 11),
                     GPIO_AF_LTDC14);
  stm32_gpio_alt(STM32_GPIOF_BASE,
                     (1u << 7) | (1u << 9) | (1u << 15),
                     GPIO_AF_LTDC13);
...
static int stm32_ltdc_clock_enable(void)
{
  ...
  if ((getreg32(STM32_RCC_CR) & RCC_CR_PLL3RDY) == 0)
    {
      ...
      modifyreg32(STM32_RCC_PLLCKSELR, RCC_PLLCKSELR_DIVM3_MASK,
                  RCC_PLLCKSELR_DIVM3(4));
      modifyreg32(STM32_RCC_PLLCFGR,
  ...
  modifyreg32(STM32_RCC_APB5RSTR, 0, RCC_APB5RSTR_LTDCRST);
  up_udelay(1);
  modifyreg32(STM32_RCC_APB5RSTR, RCC_APB5RSTR_LTDCRST, 0);
  return OK;
}
...
static void stm32_ltdc_config(void)
{
  ...
  putreg32(LTDC_SSCR_HSW(BOARD_LTDC_HSYNC - 1) |
           LTDC_SSCR_VSH(BOARD_LTDC_VSYNC - 1), STM32_LTDC_SSCR);
                                ...
                                FAR struct fb_planeinfo_s *pinfo)
{
  ...
  putreg32((uint32_t)fbaddr, STM32_LTDC_L1CFBAR);
...
static int stm32_ltdc_waitforvsync(FAR struct fb_vtable_s *vtable)
{
  ...
  while ((getreg32(STM32_LTDC_SRCR) & LTDC_SRCR_VBR) != 0)
    {
      if (timeout-- == 0)
        {
          timedout = true;
