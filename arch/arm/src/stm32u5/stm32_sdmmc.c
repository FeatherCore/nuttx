...
#if defined(CONFIG_STM32U5_SDMMC1) || defined(CONFIG_STM32U5_SDMMC2)
 ...
 *   CONFIG_STM32U5_SDMMC_IDMA - Enable SDMMC IDMA.
 ...
 *     4 SD data lines).
 *   CONFIG_STM32U5_SDMMC_XFRDEBUG - Enables some very low-level debug
...
#if !defined(CONFIG_STM32U5_SDMMC1)
...
#elif defined(CONFIG_STM32U5_SDMMC1)
#  define SRAM123_START STM32_SRAM1_BASE
#  define STM32U5_SRAM123_SIZE \
          (STM32_SRAM1_SIZE + STM32_SRAM2_SIZE + STM32_SRAM3_SIZE)
#  define SRAM123_END   (SRAM123_START + STM32U5_SRAM123_SIZE)
#  define SRAM4_START   STM32_SRAM4_BASE
#  define SRAM4_END     (SRAM4_START + STM32_SRAM4_SIZE)
#  if CONFIG_STM32U5_SDMMC_IDMA_BUFFER_SIZE < 512
#    error "STM32U5_SDMMC_IDMA_BUFFER_SIZE must cover one MMCSD block"
#  endif
#endif
...
#  undef CONFIG_STM32U5_SDMMC_XFRDEBUG
#endif
...
/* The board enable hook selects SYSCLK as the SDMMC kernel clock. */

#define STM32_SDMMC_CLK  STM32_SYSCLK_FREQUENCY
...
#define STM32_SDMMC_CLCKR_SDXFR     (STM32_SDMMC_SDXFR_CLKDIV     |     \
                                     ...
                                     STM32_SDMMC_CLKCR_PWRSAV     |     \
                                     STM32_SDMMC_CLKCR_WIDBUS_D1)
#define STM32_SDMMC_CLCKR_MMCWIDEXFR (STM32_SDMMC_MMCXFR_CLKDIV    |     \
                                      STM32_SDMMC_CLKCR_EDGE       |     \
                                      STM32_SDMMC_CLKCR_PWRSAV     |     \
                                      STM32_SDMMC_CLKCR_WIDBUS_D8)
...
#if defined(CONFIG_STM32U5_SDMMC_XFRDEBUG)
...
#ifdef CONFIG_STM32U5_SDMMC_IDMA
static FAR uint8_t *stm32_idmabuffer(struct stm32_dev_s *priv);
static bool stm32_idma_needs_bounce(struct stm32_dev_s *priv,
...
#ifndef CONFIG_STM32U5_SDMMC_IDMA
...
#endif
#if defined(CONFIG_STM32U5_SDMMC2)
...
/****************************************************************************
 * Name: stm32_idmabuffer
 ****************************************************************************/

#ifdef CONFIG_STM32U5_SDMMC_IDMA
static FAR uint8_t *stm32_idmabuffer(struct stm32_dev_s *priv)
{
#if CONFIG_STM32U5_SDMMC_IDMA_BUFFER_ADDR != 0
  return (FAR uint8_t *)((uintptr_t)CONFIG_STM32U5_SDMMC_IDMA_BUFFER_ADDR);
#else
  return priv->sdmmc_rxbuffer;
#endif
}

/****************************************************************************
 * Name: stm32_idma_needs_bounce
 ****************************************************************************/
                                    ...
                                    size_t buflen)
{
  ...
  if (priv->base == STM32_SDMMC1_BASE)
    {
      /* STM32U5 SDMMC1 IDMA cannot access SRAM1/2/3 or SRAM4.  Treat any
       * overlapping transfer as a bounce-buffer candidate.
       */

      if ((start < SRAM123_END && end > SRAM123_START) ||
          (start < SRAM4_END && end > SRAM4_START))
        {
          return true;
        }
    }
  ...
  if (priv->unaligned_rx)
    {
      ...
      dmabuffer = stm32_idmabuffer(priv);
      memcpy(priv->buffer, dmabuffer, priv->blocksize);
...
static void stm32_reset(struct sdio_dev_s *dev)
{
  ...
  if (priv->base == STM32_SDMMC1_BASE)
    {
      regaddress = STM32_RCC_AHB2RSTR1;
      restval    = RCC_AHB2RSTR_SDMMC1RST;
    }
#endif
...
#if defined(CONFIG_STM32U5_SDMMC1) && !defined(CONFIG_SDMMC1_WIDTH_D1_ONLY)
  if (priv->base == STM32_SDMMC1_BASE)
    {
      caps |= SDIO_CAPS_4BIT | SDIO_CAPS_8BIT | SDIO_CAPS_MMC_HS_MODE;
    }
#endif
...
static void stm32_clock(struct sdio_dev_s *dev, enum sdio_clock_e rate)
{
  ...
  switch (rate)
  {
    ...
    case CLOCK_MMC_TRANSFER:
      if (priv->widebus && !priv->onebit)
        {
          clckr = STM32_SDMMC_CLCKR_MMCWIDEXFR;
        }
      else
        {
...
#if defined(CONFIG_STM32U5_SDMMC_IDMA) && defined(CONFIG_ARCH_HAVE_SDIO_PREFLIGHT)
static int stm32_dmapreflight(struct sdio_dev_s *dev,
                              const uint8_t *buffer, size_t buflen)
{
  struct stm32_dev_s *priv = (struct stm32_dev_s *)dev;
  ...
  if (stm32_idma_needs_bounce(priv, buffer, buflen))
    {
      if (priv->blocksize > CONFIG_STM32U5_SDMMC_IDMA_BUFFER_SIZE ||
          buflen > CONFIG_STM32U5_SDMMC_IDMA_BUFFER_SIZE)
        {
          mcerr("IDMA bounce buffer too small "
                "buffer:0x%08" PRIxPTR " len:%zu block:%" PRIu32 "\n",
                (uintptr_t)buffer, buflen, priv->blocksize);
          return -EFAULT;
        }
    }
                              ...
                              uint8_t *buffer, size_t buflen)
{
  ...
  dmabuffer = stm32_idmabuffer(priv);
  priv->unaligned_rx = stm32_idma_needs_bounce(priv, buffer, buflen);

  if (priv->unaligned_rx &&
      priv->blocksize > CONFIG_STM32U5_SDMMC_IDMA_BUFFER_SIZE)
    {
      return -EFAULT;
    }
  ...
  if (priv->unaligned_rx)
    {
      up_invalidate_dcache((uintptr_t)dmabuffer,
                           (uintptr_t)dmabuffer + priv->blocksize);
    }
  else
    {
  ...
  if (priv->unaligned_rx)
    {
      sdmmc_putreg32(priv, (uintptr_t)dmabuffer,
                     STM32_SDMMC_IDMABASE0R_OFFSET);
    }
  else
    {
                              ...
                              const uint8_t *buffer, size_t buflen)
{
  ...
  dmabuffer = stm32_idmabuffer(priv);
  bounce    = stm32_idma_needs_bounce(priv, buffer, buflen);

  if (bounce)
    {
      if (buflen > CONFIG_STM32U5_SDMMC_IDMA_BUFFER_SIZE)
        {
          return -EFAULT;
        }
  ...
  sdmmc_putreg32(priv, (uintptr_t)(bounce ? dmabuffer : buffer),
                 STM32_SDMMC_IDMABASE0R_OFFSET);
 ...
 *
 * Description:
 *   Select SYSCLK as the SDMMC kernel clock and enable the SDMMC1 AHB clock.
 *
 ****************************************************************************/
...
{
  modifyreg32(STM32_RCC_CCIPR2, RCC_CCIPR2_SDMMCSEL_MASK,
              RCC_CCIPR2_SDMMCSEL_SYSCLK);
  modifyreg32(STM32_RCC_AHB2ENR1, 0, RCC_AHB2ENR1_SDMMC1EN);
  ...
  if (slotno == 0)
    {
      ...
      /* Configure GPIOs for wide-bus operation. Boards that route eMMC on
       * SDMMC1 may provide D4-D7 for 8-bit transfers.
       */
...
#    if defined(GPIO_SDMMC1_D4) && defined(GPIO_SDMMC1_D5) && \
        defined(GPIO_SDMMC1_D6) && defined(GPIO_SDMMC1_D7)
      stm32_configgpio(SDMMC1_SDIO_PULL(GPIO_SDMMC1_D4));
      stm32_configgpio(SDMMC1_SDIO_PULL(GPIO_SDMMC1_D5));
      stm32_configgpio(SDMMC1_SDIO_PULL(GPIO_SDMMC1_D6));
      stm32_configgpio(SDMMC1_SDIO_PULL(GPIO_SDMMC1_D7));
#    endif
#  endif
...
#endif /* CONFIG_STM32U5_SDMMC1 || CONFIG_STM32U5_SDMMC2 */
