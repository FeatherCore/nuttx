...
#define STM32U5X9J_OSPI_PRESCALER           1u
...
#define STM32U5X9J_HSPI_PRESCALER           0u
...
#define APS512XX_FB_RESERVED                STM32U5X9J_HSPI1_PSRAM_FB_SIZE
...
int stm32u5x9j_ospi1_nor_initialize(void)
{
  ...
  const struct stm32_xspi_config_s dtr_config =
    {
      .port           = STM32_XSPI_PORT_OCTOSPI1,
  ...
  ret = stm32_xspi_configure(&spi_config);
  if (ret < 0)
    {
      return ret;
    }
  ...
  ret = stm32_xspi_configure(&dtr_config);
  if (ret == OK)
    {
  ...
  const struct stm32_xspi_config_s config =
    {
      ...
      .prescaler      = STM32U5X9J_HSPI_PRESCALER,
