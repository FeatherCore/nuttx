/****************************************************************************
 * boards/arm/stm32u5/stm32u5x9j-dk/src/stm32u5x9j_xspi.c
 *
...
static void stm32u5x9j_gpio_hslv_enable(uintptr_t base, uint32_t pinmask)
{
...
static void stm32u5x9j_ospi_gpio_config(void)
{
  ...
  stm32u5x9j_gpio_hslv_enable(STM32_GPIOA_BASE,
                              STM32U5X9J_OSPI_HSLV_PORTA_MASK);
  stm32u5x9j_gpio_hslv_enable(STM32_GPIOC_BASE,
                              STM32U5X9J_OSPI_HSLV_PORTC_MASK);
  stm32u5x9j_gpio_hslv_enable(STM32_GPIOF_BASE,
...
static int
stm32u5x9j_ospi_configure(FAR const struct stm32_xspi_config_s *xspi,
...
static int stm32u5x9j_ospi_dlyb_configure(void)
{
  uint32_t phase;
  uint32_t units;
  int ret;
...
static uint32_t stm32u5x9j_ospi_dtr_ccr(void)
{
...
static int stm32u5x9j_ospi_read_cfg2_spi(uint32_t address,
                                         FAR uint8_t *value)
{
  return stm32_xspi_read_data(STM32_XSPI_PORT_OCTOSPI1,
...
static int stm32u5x9j_ospi_write_cfg2_spi(uint32_t address, uint8_t value)
{
...
static int stm32u5x9j_ospi_read_cfg2_dtr(uint32_t address,
                                         FAR uint8_t *value)
{
  uint8_t buf[2];
  int ret;
...
static int stm32u5x9j_ospi_read_status_spi(FAR uint8_t *status)
{
  return stm32_xspi_read_data(STM32_XSPI_PORT_OCTOSPI1,
...
static int stm32u5x9j_ospi_read_status_dtr(FAR uint8_t *status)
{
  uint8_t buf[2];
  int ret;
...
static int stm32u5x9j_ospi_wait_status(
...
static int stm32u5x9j_ospi_wait_ready_spi(void)
{
  return stm32u5x9j_ospi_wait_status(stm32u5x9j_ospi_read_status_spi,
                                     MX25UM51245G_STATUS_WIP, 0);
}

static int stm32u5x9j_ospi_wait_ready_dtr(void)
{
  return stm32u5x9j_ospi_wait_status(stm32u5x9j_ospi_read_status_dtr,
                                     MX25UM51245G_STATUS_WIP, 0);
}

static void stm32u5x9j_ospi_reset_opi(bool dtr)
{
  uint32_t ccr;
  int ret;
...
static int stm32u5x9j_ospi_reset_spi(void)
{
  int ret;
...
static int
stm32u5x9j_ospi_enter_dopi(FAR const struct stm32_xspi_config_s *config,
                           FAR const struct stm32_xspim_config_s *xspim)
{
  uint8_t cr2;
  int ret;
  ...
  ret = stm32u5x9j_ospi_write_cfg2_spi(MX25UM51245G_CR2_REG3_ADDR,
  ...
  ret = stm32u5x9j_ospi_wait_ready_spi();
  if (ret < 0)
    {
  ...
  ret = stm32u5x9j_ospi_read_cfg2_spi(MX25UM51245G_CR2_REG3_ADDR, &cr2);
  if (ret < 0)
    {
  ...
  ret = stm32u5x9j_ospi_write_cfg2_spi(MX25UM51245G_CR2_REG1_ADDR,
  ...
  ret = stm32u5x9j_ospi_configure(config, xspim);
  if (ret < 0)
    {
      return ret;
    }

  ret = stm32u5x9j_ospi_dlyb_configure();
  if (ret < 0)
    {
      return ret;
    }

  ret = stm32u5x9j_ospi_wait_ready_dtr();
  if (ret < 0)
    {
  ...
  ret = stm32u5x9j_ospi_read_cfg2_dtr(MX25UM51245G_CR2_REG1_ADDR, &cr2);
  if (ret < 0)
    {
...
static int
stm32u5x9j_ospi_switch_dopi_run(FAR const struct stm32_xspi_config_s *config,
                                FAR const struct stm32_xspim_config_s *xspim)
{
  uint8_t cr2;
  int ret;
...
static int stm32u5x9j_ospi_erase_subsector(uint32_t address)
{
  ...
  return stm32u5x9j_ospi_wait_ready_spi();
}
...
#ifdef CONFIG_STM32U5X9J_DK_HSPI_RAM
static uint32_t stm32u5x9j_psram_refresh(uint32_t prescaler)
{
  uint32_t hspi_hz;
  uint32_t cycles;
...
static void stm32u5x9j_hspi_gpio_config(void)
{
  ...
  stm32u5x9j_gpio_hslv_enable(STM32_GPIOH_BASE,
                              STM32U5X9J_HSPI_HSLV_PORTH_MASK);
  stm32u5x9j_gpio_hslv_enable(STM32_GPIOI_BASE,
                              STM32U5X9J_HSPI_HSLV_PORTI_MASK);
  stm32u5x9j_gpio_hslv_enable(STM32_GPIOJ_BASE,
                                       ...
                                       uint32_t new_latency)
{
  ...
  ret = stm32u5x9j_psram_read_reg(address, &initial, old_latency);
  if (ret < 0)
    {
      return ret;
    }
  ...
  ret = stm32u5x9j_psram_read_reg(address, &readback, new_latency);
  if (ret < 0)
    {
      return ret;
    }
                                         ...
                                         FAR char *buffer, size_t buflen)
{
  ...
  len = snprintf(text, sizeof(text),
                 ...
                 stm32u5x9j_hspi1_psram_is_mapped(),
...
int stm32u5x9j_ospi1_nor_initialize(void)
{
  ...
  ret = stm32u5x9j_ospi_configure(&spi_init_config, &xspim_config);
  if (ret < 0)
    {
      return ret;
    }

  stm32u5x9j_ospi_reset_opi(false);

  ret = stm32u5x9j_ospi_configure(&dtr_init_config, &xspim_config);
  if (ret == OK)
    {
      stm32u5x9j_ospi_reset_opi(true);
    }
  ...
  ret = stm32u5x9j_ospi_enter_dopi(&dtr_init_config, &xspim_config);

  if (ret >= 0)
    {
      ret = stm32u5x9j_ospi_switch_dopi_run(&dtr_run_config,
                                            &xspim_config);
    }
  ...
  if (ret < 0)
    {
      ...
      (void)stm32u5x9j_ospi_configure(&dtr_init_config, &xspim_config);
      stm32u5x9j_ospi_reset_opi(true);
      ...
      ret = stm32u5x9j_ospi_readid(g_ospi_jedec);
      if (ret < 0)
        {
          return ret;
        }

      ret = stm32u5x9j_ospi_configure(&spi_run_config, &xspim_config);
      if (ret < 0)
        {
          return ret;
        }
...
int stm32u5x9j_ospi_diag_initialize(void)
{
  ...
  return stm32u5x9j_ospi_diag_register();
}
#endif
#endif /* CONFIG_STM32U5X9J_DK_OSPI_NOR */
  ...
  const struct stm32_xspi_config_s init_config =
    {
      ...
      .refresh        = stm32u5x9j_psram_refresh(
