/****************************************************************************
 * boards/arm/stm32h7rs/stm32h7s78-dk/src/stm32_heap.c
 *
 ...
 *
 * Description:
 *   In the protected KNSh configuration the initial user heap is the whole
 *   configured PSRAM heap window.  Internal SRAM is reserved for kernel
 *   sections, user .data/.bss, and kernel heap.
 *
 ****************************************************************************/

#if defined(CONFIG_BUILD_PROTECTED) && defined(CONFIG_MM_KERNEL_HEAP) && \
    defined(CONFIG_STM32H7RS_PSRAM_HEAP)
void up_allocate_heap(void **heap_start, size_t *heap_size)
{
  ...
  if (ret < 0)
    {
      ferr("ERROR: STM32H7S78-DK PSRAM user heap init failed: %d\n", ret);
      PANIC();
    }
...
{
#if defined(CONFIG_STM32H7RS_PSRAM_HEAP) && \
    (!defined(CONFIG_BUILD_PROTECTED) || !defined(CONFIG_MM_KERNEL_HEAP))
  int ret;
