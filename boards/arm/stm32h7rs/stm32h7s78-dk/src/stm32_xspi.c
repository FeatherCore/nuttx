/****************************************************************************
 * boards/arm/stm32h7rs/stm32h7s78-dk/src/stm32_xspi.c
 *
...
static int stm32_nor_read_status_octal(uint8_t *status)
{
  uint8_t data[2];
  uint32_t ccr;
  int ret;
...
static int stm32_nor_wait_wip_octal(void)
{
...
static int stm32_xspi2_enter_memory_mapped(void)
{
  ...
  for (region = 0; region < sizeof(offsets) / sizeof(offsets[0]); region++)
    {
      ...
      stm32_psram_dcache_flush(ptr, sizeof(pattern));
...
{
  static const struct stm32_xspi_config_s nor_config =
    {
...
{
  const struct stm32_xspi_config_s psram_config =
    {
  ...
  ret = stm32_psram_config_reg(APS256_MR0_ADDR, APS256_MR0_VALUE,
                                    0x3f);
  if (ret < 0)
    {
  ...
  ret = stm32_psram_config_reg(APS256_MR4_ADDR, APS256_MR4_VALUE,
                                    0xe0);
  if (ret < 0)
    {
