...
static int stm32_lcd_backlight_start(uint32_t brightness)
{
  ...
  stm32_gpio_alt(STM32H7RS_GPIOG_BASE, LCD_BACKLIGHT_PIN,
                      GPIO_AF_LPTIM1);
...
fail:
  ...
  stm32_gpio_output(STM32H7RS_GPIOG_BASE, LCD_BACKLIGHT_PIN, true,
                         GPIO_FLOAT);
  return ret;
}
...
static int stm32_lcd_panel_config(void)
{
  ...
  stm32_gpio_output(STM32H7RS_GPIOE_BASE, LCD_DISP_EN_PIN, true,
                         GPIO_PULLUP);
  ...
  if (on)
    {
      ...
      ret = stm32_lcd_backlight_start(LCD_BACKLIGHT_BRIGHTNESS);
      if (ret < 0)
        {
          syslog(LOG_WARNING,
                 ...
                 ret);
          stm32_gpio_output(STM32H7RS_GPIOG_BASE, LCD_BACKLIGHT_PIN,
                                 false, GPIO_FLOAT);
        }
    }
  else
    {
      stm32_lcd_backlight_stop();
      stm32_gpio_output(STM32H7RS_GPIOE_BASE, LCD_DISP_EN_PIN, false,
                             GPIO_PULLUP);
    }
}
