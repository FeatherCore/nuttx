/****************************************************************************
 * boards/arm/stm32n6/stm32n6570-dk/src/stm32_touchscreen.c
 *
...
static int stm32_gt911_gpio_config(void)
{
  int ret;

  ret = stm32_configgpio(GPIO_I2C2_SCL);
  if (ret < 0)
    {
      return ret;
    }

  ret = stm32_configgpio(GPIO_I2C2_SDA);
  if (ret < 0)
    {
      return ret;
    }

  ret = stm32_configgpio(GPIO_GT911_INT);
  if (ret < 0)
    {
      return ret;
    }

  ret = stm32_configgpio(GPIO_GT911_NRST);
  if (ret < 0)
    {
      return ret;
    }
