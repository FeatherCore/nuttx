...
static void stm32n6570_build_log(void)
{
  struct utsname name;
...
static void stm32n6570_clock_log(void)
{
...
static void stm32n6570_board_initialize(void)
{
  int ret;

  syslog(LOG_INFO, "STM32N6570-DK bring-up skeleton\n");
  stm32n6570_build_log();
  stm32n6570_clock_log();
  ...
  ret = stm32n6570_touch_setup();
  if (ret < 0)
    {
      syslog(LOG_ERR, "stm32n6570_touch_setup failed: %d\n", ret);
    }
#endif
