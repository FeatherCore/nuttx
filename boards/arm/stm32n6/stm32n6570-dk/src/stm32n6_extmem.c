/****************************************************************************
 * boards/arm/stm32n6/stm32n6570-dk/src/stm32n6_extmem.c
 *
...
struct stm32n6_ota_partition_s
{
  uint32_t    offset;
  uint32_t    size;
  const char *devpath;
...
static ssize_t stm32n6_xip_bread(FAR struct mtd_dev_s *dev,
                                 off_t startblock, size_t nblocks,
                                 FAR uint8_t *buffer);
static int stm32n6_xip_erase(FAR struct mtd_dev_s *dev, off_t startblock,
                             size_t nblocks);
static ssize_t stm32n6_xip_bwrite(FAR struct mtd_dev_s *dev,
                                  off_t startblock, size_t nblocks,
                                  FAR const uint8_t *buffer);
static ssize_t stm32n6_xip_read(FAR struct mtd_dev_s *dev, off_t offset,
                                size_t nbytes, FAR uint8_t *buffer);
#ifdef CONFIG_MTD_BYTE_WRITE
static ssize_t stm32n6_xip_write(FAR struct mtd_dev_s *dev, off_t offset,
                                 size_t nbytes,
                                 FAR const uint8_t *buffer);
#endif
static int stm32n6_xip_ioctl(FAR struct mtd_dev_s *dev, int cmd,
                             unsigned long arg);
...
static struct stm32n6_xip_mtd_s g_nor_mtd =
{
  .mtd =
    {
      .erase  = stm32n6_xip_erase,
      .bread  = stm32n6_xip_bread,
      .bwrite = stm32n6_xip_bwrite,
      .read   = stm32n6_xip_read,
#ifdef CONFIG_MTD_BYTE_WRITE
      .write  = stm32n6_xip_write,
#endif
...
static const struct stm32n6_ota_partition_s g_ota_partition_table[] =
{
  {
...
static ssize_t stm32n6_xip_bread(FAR struct mtd_dev_s *dev,
                                 ...
                                 FAR uint8_t *buffer)
{
  return stm32n6_xip_read(dev, startblock * STM32N6_NOR_BLOCK_SIZE,
                          nblocks * STM32N6_NOR_BLOCK_SIZE, buffer) /
         STM32N6_NOR_BLOCK_SIZE;
}
                             ...
                             size_t nblocks)
{
  ...
  uint32_t offset = startblock * STM32N6_NOR_ERASE_SIZE;
...
{
#ifdef CONFIG_STM32N6_EXTNOR_WRITE
  uint32_t offset = startblock * STM32N6_NOR_BLOCK_SIZE;
...
static ssize_t stm32n6_xip_read(FAR struct mtd_dev_s *dev, off_t offset,
                                size_t nbytes, FAR uint8_t *buffer)
{
  FAR struct stm32n6_xip_mtd_s *priv =
    (FAR struct stm32n6_xip_mtd_s *)dev;
      ...
      case MTDIOC_GEOMETRY:
        {
          ...
          geo->blocksize    = STM32N6_NOR_BLOCK_SIZE;
          geo->erasesize    = STM32N6_NOR_ERASE_SIZE;
          geo->neraseblocks = priv->size / STM32N6_NOR_ERASE_SIZE;
...
#ifdef CONFIG_STM32N6_EXTNOR_OTA_PARTITION
static int stm32n6_register_ota_partitions(FAR struct mtd_dev_s *mtd)
{
  int ret = OK;
  int i;
  ...
  for (i = 0; i < nitems(g_ota_partition_table); i++)
    {
      FAR const struct stm32n6_ota_partition_s *part =
        &g_ota_partition_table[i];
      FAR struct mtd_dev_s *partmtd;
      off_t firstblock;
      off_t nblocks;

      firstblock = part->offset / STM32N6_NOR_BLOCK_SIZE;
      nblocks    = part->size / STM32N6_NOR_BLOCK_SIZE;
...
#ifdef CONFIG_STM32N6_EXTNOR_OTA_PARTITION
  ret = stm32n6_register_ota_partitions(&g_nor_mtd.mtd);
  if (ret < 0)
    {
      return ret;
    }
