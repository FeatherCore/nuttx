/****************************************************************************
 * boards/arm/stm32n6/stm32n6570-dk/src/stm32_lcd.c
 *
...
static int stm32_lcd_panel_config(void)
{
  ...
  ret = stm32_configgpio(GPIO_LCD_DISP_NRST);
  if (ret < 0)
    {
      return ret;
    }

  ret = stm32_configgpio(GPIO_LCD_DISP_ONOFF);
  if (ret < 0)
    {
      return ret;
    }

  ret = stm32_configgpio(GPIO_LCD_DISP_BL);
  if (ret < 0)
    {
      return ret;
    }

  return stm32_configgpio(GPIO_LCD_DISP_EN);
}
...
{
  stm32_gpiowrite(GPIO_LCD_DISP_EN, on);
  stm32_gpiowrite(GPIO_LCD_DISP_BL, on);
}
